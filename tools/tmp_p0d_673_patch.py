...
def between(start: str, end: str, replacement: str, search_from: int = 0) -> None:
    global text
    a = text.index(start, search_from)
    b = text.index(end, a)
...
new_declarations = r'''fn validate_function_declarations(
    types: &TypeTable,
...
'''
between("fn validate_function_declarations(", "fn validate_structure_and_static_rules(", new_declarations)
...
new_static_terminator = r'''fn validate_static_terminator(
    program: &Program,
    function: &Function,
...
'''
between("fn validate_static_terminator(", "fn require_target(", new_static_terminator)

new_branch_condition = r'''fn validate_branch_condition(
    program: &Program,
    body: &Body,
    operand: &Operand,
    point: &MirPoint,
...
'''
between("fn validate_branch_condition(", "fn is_bool_type(", new_branch_condition)
...
static_start = text.index("fn validate_static_statement(")
static_end = text.index("fn validate_reference_reborrow_shape(", static_start)
static = text[static_start:static_end]
static = static.replace("validate_static_binary_numeric(\n            types,", "validate_static_binary_numeric(\n            program,")
static = static.replace("validate_static_integer_eq(types,", "validate_static_integer_eq(program,")
static = static.replace("validate_static_integer_lt(types,", "validate_static_integer_lt(program,")
static = static.replace(
    "fn validate_static_binary_numeric(\n    types: &TypeTable,",
    "fn validate_static_binary_numeric(\n    program: &Program,",
)
marker = "fn validate_static_binary_numeric("
pos = static.index(marker)
body_pos = static.index(") -> Result<(), MirValidationError> {", pos) + len(") -> Result<(), MirValidationError> {")
static = static[:body_pos] + "\n    let types = &program.types;" + static[body_pos:]
static = static.replace(
    "fn validate_static_integer_eq(\n    types: &TypeTable,",
    "fn validate_static_integer_eq(\n    program: &Program,",
)
pos = static.index("fn validate_static_integer_eq(")
body_pos = static.index(") -> Result<(), MirValidationError> {", pos) + len(") -> Result<(), MirValidationError> {")
static = static[:body_pos] + "\n    let types = &program.types;" + static[body_pos:]
static = static.replace(
...
pos = static.index("fn validate_static_integer_lt(")
body_pos = static.index(") -> Result<(), MirValidationError> {", pos) + len(") -> Result<(), MirValidationError> {")
static = static[:body_pos] + "\n    let types = &program.types;" + static[body_pos:]
static = static.replace(
...
pos = static.index("fn validate_operand_type(")
body_pos = static.index(") -> Result<(), MirValidationError> {", pos) + len(") -> Result<(), MirValidationError> {")
static = static[:body_pos] + "\n    let types = &program.types;" + static[body_pos:]
static = static.replace("validate_operand_type(types, body,", "validate_operand_type(program, body,")

function_value_arm = r'''        Operand::FunctionValue(function) => {
...
'''
anchor = "        Operand::Move(src) => {"
insert_at = static.index(anchor, static.index("fn validate_operand_type("))
static = static[:insert_at] + function_value_arm + static[insert_at:]
text = text[:static_start] + static + text[static_end:]

path_start = text.index("fn validate_path_state(")
call_start = text.index("            Terminator::Call {", path_start)
match_end_marker = "        }\n    }\n\n    Ok(())\n}\n\nfn validate_state_statement"
match_end = text.index(match_end_marker, call_start)
new_call_arms = r'''            Terminator::Call {
...
'''
text = text[:call_start] + new_call_arms + text[match_end:]

call_helper = r'''fn validate_call_state(
    types: &TypeTable,
    body: &Body,
...
'''
insert = text.index("fn validate_state_statement(", path_start)
text = text[:insert] + call_helper + text[insert:]
...
location_helper = r'''fn location_error(
    location: &MirLocation,
...
'''
insert = text.index("fn point_error(")
text = text[:insert] + location_helper + text[insert:]
