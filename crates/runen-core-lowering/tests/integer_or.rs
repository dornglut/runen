...
fn integer_or_statements(function: &runen_core_ir::Function) -> Vec<&CoreStatement> {
    function
        .body
        .blocks
        .iter()
        .flat_map(|block| &block.statements)
        .filter(|statement| matches!(statement, CoreStatement::IntegerOr { .. }))
        .collect()
}
...
#[test]
fn integer_or_lowers_to_one_fresh_result_with_move_operands_and_no_rewrite_or_cfg() {
    let lowered = lower_source("fn f(left: I8, right: I8) -> I8 { return left | right; }");
    let f = function(lowered.as_program(), "f");

    assert_eq!(f.body.blocks.len(), 1, "plain OR adds no Core block");
    let ors = integer_or_statements(f);
    assert_eq!(ors.len(), 1);
    let CoreStatement::IntegerOr { dst, left, right } = ors[0] else {
        unreachable!();
    };
    assert!(dst.projections.is_empty());
    let left_local = moved_local(left).expect("IntegerOr left operand must Move a temporary");
    let right_local = moved_local(right).expect("IntegerOr right operand must Move a temporary");
    ...
    let forbidden_rewrites = f.body.blocks[0]
        .statements
        .iter()
        .filter(|statement| {
            matches!(
                ...
                CoreStatement::IntegerAdd { .. }
                    ...
                    | CoreStatement::IntegerXor { .. }
            )
        })
        .count();
    assert_eq!(forbidden_rewrites, 0);
    assert!(!matches!(
    ...
    let Terminator::Return(Some(returned)) = &f.body.blocks[0].terminator else {
        panic!("OR result must feed the function return");
    };
    assert_eq!(moved_local(returned), Some(dst.local));
}

#[test]
fn call_operands_lower_left_then_right_before_integer_or_result_allocation() {
    ...
    let ors = f.body.blocks[or_block.0 as usize]
        .statements
        .iter()
        .filter(|statement| matches!(statement, CoreStatement::IntegerOr { .. }))
        .collect::<Vec<_>>();
    ...
    assert_eq!(
        f.body
            ...
            .count(),
        0,
        "integer OR adds no branch or join"
    );
}

#[test]
fn grouped_nested_or_emits_one_core_or_per_source_or_in_dependency_order() {
    let lowered = lower_source(
    ...
    for name in ["left", "right"] {
        ...
        let ors = integer_or_statements(function);
        assert_eq!(ors.len(), 2, "{name}");
        assert_eq!(function.body.blocks.len(), 1, "{name}");

        let CoreStatement::IntegerOr { dst: first_dst, .. } = ors[0] else {
            unreachable!();
        };
        let CoreStatement::IntegerOr {
            dst: second_dst,
            left,
            right,
        } = ors[1]
        ...
        assert!(
            ...
            "outer OR must consume the nested OR result"
        );
    }
}

#[test]
fn mixed_xor_or_trees_lower_tighter_xor_before_the_enclosing_or() {
    let lowered = lower_source(
        "fn left(a: I8, b: I8, c: I8) -> I8 { return a ^ b | c; } \
         fn right(a: I8, b: I8, c: I8) -> I8 { return a | b ^ c; }",
    );
    ...
    for name in ["left", "right"] {
        let function = function(lowered.as_program(), name);
        assert_eq!(function.body.blocks.len(), 1, "{name}");
        let operations = function.body.blocks[0]
            .statements
            .iter()
            .filter_map(|statement| match statement {
                CoreStatement::IntegerXor { .. } => Some("xor"),
                CoreStatement::IntegerOr { .. } => Some("or"),
                _ => None,
            })
            .collect::<Vec<_>>();
        assert_eq!(operations, ["xor", "or"], "{name}");
    }
}
...
fn lowering_rejects_malformed_integer_or_retained_type_facts() {
    ...
    let value = left_mismatch.functions[0]
        ...
        .and_then(|returned| returned.value.as_mut())
        .expect("return value");
    let ValueKind::IntegerOr { left, .. } = &mut value.kind else {
    ...
    let value = right_mismatch.functions[0]
        ...
        .and_then(|returned| returned.value.as_mut())
        .expect("return value");
    let ValueKind::IntegerOr { right, .. } = &mut value.kind else {
        panic!("expected integer-OR HIR value");
    };
