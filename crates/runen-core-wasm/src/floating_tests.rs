...
fn floating_arithmetic_adds_no_wasm_sections_or_export_kinds() {
    ...
    let program = validate_program(Program {
        ...
        functions: vec![Function {
            ...
            body: Body {
                locals: vec![
                    LocalDecl::new("f16_result", f16_ty, false),
    ...
    coverage::validate(&program)
        .expect("F16/F32/F64 arithmetic module-shape fixture must be admitted");
    let encoded =
    ...
    assert_eq!(
        section_ids(module),
        vec![1, 3, 7, 10],
        "transient F16/F32/F64 arithmetic must not add imports, tables, memory, globals, elements, or data"
    );
    assert_eq!(
        export_entries(section_payload(module, 7)),
        vec![(0, 0)],
...
fn arithmetic_nan_uses_the_existing_private_carrier_before_provider_decode() {
    for (scalar, kind, zero) in [
        (
            ScalarType::F16,
            ScalarKind::F16,
            Value::F16(BinaryFloatValue::Zero(BinaryFloatSign::Positive)),
        ),
        (
            ScalarType::F32,
            ScalarKind::F32,
        ...
        let instance =
            wasmtime::Instance::new(&mut store, &module, &[wasmtime::Extern::Func(import)])
        ...
        let expected = payload(
            kind.floating_residue(FloatingScalarValue::NaNClass)
                .expect("existing private NaN carrier must encode"),
        );
        assert_eq!(
