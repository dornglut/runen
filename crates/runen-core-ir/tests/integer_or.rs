...
#[test]
fn integer_or_accepts_all_eight_scalar_kinds_and_immutable_vacant_destinations() {
    ...
    for (index, (scalar, left, right)) in cases.into_iter().enumerate() {
        ...
        let program = one_block(
            ...
            vec![Statement::IntegerOr {
                dst: Place::local(LocalId(0)),
                left: Operand::Constant(left),
                right: Operand::Constant(right),
            }],
        );
        validate_program(program).expect("fixed-width integer OR must validate");
    }
}

#[test]
fn integer_or_rejects_non_integer_destination_with_specific_error() {
    for scalar in [
        ScalarType::Bool,
        ScalarType::F32,
        ...
        assert_eq!(
            error.kind,
            MirValidationErrorKind::IntegerOrRequiresInteger(ty)
        );
    }
}

#[test]
fn integer_or_requires_exact_destination_type_identity_for_each_operand() {
...
#[test]
fn integer_or_checks_vacancy_before_operand_state() {
    let mut types = TypeTable::new();
    ...
    assert_eq!(
        error.kind,
        MirValidationErrorKind::IntegerOrRequiresVacant(destination)
    );
}

#[test]
fn shared_and_exclusive_overlapping_loans_block_direct_or_destination_before_vacancy_or_operands() {
    for kind in [BorrowKind::Shared, BorrowKind::Exclusive] {
            ...
            Body {
                ...
                blocks: vec![BasicBlock::new(
                    vec![
                        ...
                        Statement::IntegerOr {
                            dst: destination.clone(),
                            left: Operand::Move(missing.clone().into()),
                            right: Operand::Move(missing.clone().into()),
                        },
                    ],
        ...
        let error = validate_program(program).expect_err("overlapping loan blocks direct OR dst");
        assert_eq!(
            error.kind,
...
#[test]
fn integer_or_evaluates_left_state_before_right_state() {
    let mut types = TypeTable::new();
    let i8_ty = types.push(TypeDef::scalar("i8", ScalarType::I8));
...
#[test]
fn integer_or_result_becomes_live_once_after_both_operands() {
    let mut types = TypeTable::new();
    let i8_ty = types.push(TypeDef::scalar("i8", ScalarType::I8));
    ...
    validate_program(program).expect("successful IntegerOr initializes its destination once");
}
