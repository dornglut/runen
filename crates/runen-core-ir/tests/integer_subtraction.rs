...
#[test]
fn integer_sub_accepts_all_eight_scalar_kinds_and_immutable_vacant_destinations() {
    ...
    for (index, (scalar, left, right)) in cases.into_iter().enumerate() {
        ...
        validate_program(program).expect("fixed-width integer subtraction must validate");
    }
}

#[test]
fn integer_sub_rejects_non_integer_destination_with_specific_error() {
    for scalar in [
        ScalarType::Bool,
        ScalarType::F32,
...
#[test]
fn integer_sub_requires_exact_destination_type_identity_for_both_operands() {
    let mut types = TypeTable::new();
...
#[test]
fn integer_sub_checks_vacancy_before_operand_state() {
    let mut types = TypeTable::new();
    ...
    assert_eq!(
        error.kind,
        MirValidationErrorKind::IntegerSubRequiresVacant(destination)
    );
}

#[test]
fn integer_sub_checks_direct_destination_authority_before_operands() {
    let mut types = TypeTable::new();
    let i8_ty = types.push(TypeDef::scalar("i8", ScalarType::I8));
    let destination = Place::local(LocalId(0));
...
#[test]
fn integer_sub_evaluates_left_state_before_right_state() {
    let mut types = TypeTable::new();
    let i8_ty = types.push(TypeDef::scalar("i8", ScalarType::I8));
...
#[test]
fn integer_sub_result_becomes_live_once_after_both_operands() {
    let mut types = TypeTable::new();
    let i8_ty = types.push(TypeDef::scalar("i8", ScalarType::I8));
    ...
    validate_program(program)
        .expect("successful IntegerSub initializes its destination exactly once");
}
