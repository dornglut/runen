...
fn zero_value(scalar: ScalarType) -> Value {
    ...
    match scalar {
        ...
        _ => unreachable!("floating-div fixture requests only represented floating kinds"),
    }
}

#[test]
fn float_div_accepts_all_three_scalar_kinds_all_contracts_and_immutable_vacant_destinations() {
        ...
        .enumerate()
    {
        for contract in [
            ...
            validate_program(program)
                .expect("represented same-format FloatDiv contract must validate");
        }
    }
}

#[test]
fn float_div_rejects_non_floating_destination_with_specific_error() {
    for scalar in [
        ScalarType::Bool,
        ScalarType::I32,
        ...
        assert_eq!(
            error.kind,
            MirValidationErrorKind::FloatDivRequiresFloat(ty)
        );
    }
}

#[test]
fn float_div_requires_exact_destination_type_identity_for_both_operands() {
    let mut types = TypeTable::new();
...
#[test]
fn float_div_rejects_cross_format_operand() {
    let mut types = TypeTable::new();
    ...
    let error = validate_program(program).expect_err("cross-format FloatDiv must fail");
    assert_eq!(
        error.kind,
...
#[test]
fn float_div_checks_vacancy_before_operand_state() {
    let mut types = TypeTable::new();
    let f32_ty = types.push(TypeDef::scalar("f32", ScalarType::F32));
    ...
    assert_eq!(
        error.kind,
        MirValidationErrorKind::FloatDivRequiresVacant(destination)
    );
}

#[test]
fn float_div_checks_direct_destination_authority_before_operands() {
    let mut types = TypeTable::new();
    let f32_ty = types.push(TypeDef::scalar("f32", ScalarType::F32));
    let destination = Place::local(LocalId(0));
    let zero = Value::F32(BinaryFloatValue::Zero(BinaryFloatSign::Positive));
...
#[test]
fn float_div_evaluates_left_state_before_right_state() {
    let mut types = TypeTable::new();
    let f32_ty = types.push(TypeDef::scalar("f32", ScalarType::F32));
...
#[test]
fn float_div_result_becomes_live_once_after_both_operands() {
    let mut types = TypeTable::new();
    let f32_ty = types.push(TypeDef::scalar("f32", ScalarType::F32));
    ...
    validate_program(program)
        .expect("successful FloatDiv initializes its destination exactly once");
}
