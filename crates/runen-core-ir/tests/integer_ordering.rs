...
fn ordering(dst: Place, operand_type: TypeId, left: Operand, right: Operand) -> Statement {
    Statement::IntegerLt {
        dst,
        operand_type,
        left,
...
#[test]
fn integer_lt_accepts_all_eight_integer_operand_kinds_with_bool_destination() {
    let cases: [(ScalarType, Value, Value); 8] = [
        (ScalarType::I8, Value::I8(-1), Value::I8(1)),
        (ScalarType::I16, Value::I16(-2), Value::I16(-1)),
        (ScalarType::I32, Value::I32(3), Value::I32(4)),
        (ScalarType::I64, Value::I64(-5), Value::I64(6)),
        (ScalarType::U8, Value::U8(7), Value::U8(8)),
        (ScalarType::U16, Value::U16(8), Value::U16(9)),
        (ScalarType::U32, Value::U32(10), Value::U32(11)),
    ...
    for (index, (scalar, left, right)) in cases.into_iter().enumerate() {
        ...
        validate_program(program).expect("fixed-width integer ordering must validate");
    }
}

#[test]
fn integer_lt_rejects_known_non_bool_destination_before_operands() {
    for scalar in [ScalarType::I8, ScalarType::F32, ScalarType::TrackedFixture] {
        ...
        let error = validate_program(program).expect_err("non-Bool ordering destination must fail");
        assert_eq!(
            error.kind,
...
#[test]
fn integer_lt_rejects_unknown_operand_type_before_operands() {
    let mut types = TypeTable::new();
...
#[test]
fn integer_lt_rejects_known_non_integer_operand_types_before_operands() {
    let mut types = TypeTable::new();
...
#[test]
fn integer_lt_uses_explicit_operand_type_for_constants_and_place_constant_pairs() {
            ...
            vec![
                ...
                ordering(
                    ...
                    if place_on_left {
                        ...
                        Operand::Constant(Value::I8(6))
                    },
                    if place_on_left {
                        Operand::Constant(Value::I8(8))
                    } else {
...
#[test]
fn integer_lt_requires_exact_operand_type_identity_for_each_place_operand() {
...
#[test]
fn integer_lt_rejects_constant_mismatch_against_explicit_operand_type() {
    let mut types = TypeTable::new();
    let i8_type = types.push(TypeDef::scalar("i8", ScalarType::I8));
    let bool_type = types.push(TypeDef::scalar("bool", ScalarType::Bool));
    let program = one_block(
...
#[test]
fn integer_lt_checks_vacancy_before_operand_state() {
    let mut types = TypeTable::new();
    let i8_type = types.push(TypeDef::scalar("i8", ScalarType::I8));
    let bool_type = types.push(TypeDef::scalar("bool", ScalarType::Bool));
    ...
    assert_eq!(
        error.kind,
        MirValidationErrorKind::IntegerLtRequiresVacant(destination)
    );
}

#[test]
fn shared_and_exclusive_overlapping_loans_block_direct_lt_destination_first() {
    for kind in [BorrowKind::Shared, BorrowKind::Exclusive] {
        ...
        let error =
            validate_program(program).expect_err("overlapping loan blocks direct ordering dst");
        assert_eq!(
            error.kind,
...
#[test]
fn integer_lt_evaluates_left_state_before_right_state() {
    let mut types = TypeTable::new();
    let i8_type = types.push(TypeDef::scalar("i8", ScalarType::I8));
    let bool_type = types.push(TypeDef::scalar("bool", ScalarType::Bool));
    let source = Place::local(LocalId(0));
...
#[test]
fn integer_lt_result_becomes_live_as_bool() {
    let mut types = TypeTable::new();
    let i8_type = types.push(TypeDef::scalar("i8", ScalarType::I8));
    let bool_type = types.push(TypeDef::scalar("bool", ScalarType::Bool));
    ...
    let program = one_block(
        ...
        vec![
            ordering(
                ...
                Operand::Constant(Value::I8(4)),
            ),
            Statement::Read { src: result.into() },
        ],
    );

    validate_program(program).expect("successful IntegerLt initializes a Bool destination once");
}

#[test]
fn integer_lt_preserves_raw_move_integer_operand_semantics() {
    let mut types = TypeTable::new();
    let i8_type = types.push(TypeDef::scalar("i8", ScalarType::I8));
    let bool_type = types.push(TypeDef::scalar("bool", ScalarType::Bool));
    ...
    validate_program(program).expect("IntegerLt must preserve existing RawMove integer semantics");
}

#[test]
fn integer_lt_preserves_reference_copy_and_move_permission_semantics() {
    let mut types = TypeTable::new();
    let i8_type = types.push(TypeDef::scalar("i8", ScalarType::I8));
    let bool_type = types.push(TypeDef::scalar("bool", ScalarType::Bool));
