...
#[test]
fn parses_projected_replacement_roots_and_reborrows_as_single_reference_values() {
    let source = "fn f(root: I64, r: &mut I64) { let one: &mut I64 = &mut root.value; let nested: &mut I64 = &mut *r.outer.inner; }";
    let parsed = parse(source);
    assert_eq!(parsed.text(), source);
    assert!(parsed.errors().is_empty(), "{:?}", parsed.errors());
    assert_eq!(count_kind(&parsed, SyntaxKind::SafeReferenceValue), 2);
    assert_eq!(count_kind(&parsed, SyntaxKind::FieldValueUse), 0);
    assert_eq!(count_kind(&parsed, SyntaxKind::Dot), 3);
    assert_eq!(count_kind(&parsed, SyntaxKind::Star), 1);
}
