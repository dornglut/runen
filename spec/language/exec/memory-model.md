...
- one group-cohort-scoped exchange and one subgroup-cohort-scoped exchange are **scope-compatible** exactly when the producer of the group-scoped exchange belongs to the subgroup selected by the subgroup-scoped exchange and the producer of the subgroup-scoped exchange belongs to the group selected by the group-scoped exchange; and
- otherwise, that group-cohort/subgroup-cohort pair is **scope-incompatible**.

The group-cohort/subgroup-cohort rule is symmetric and consumes the exact stable producer memberships from the hierarchy instances to which those two exchanges are already bound. It does not infer compatibility from the names or apparent nesting of scope forms, from equal implementation/debug cohort tokens, or from physical topology. In particular, a group-scoped producer in the same selected group but outside the subgroup selected by the subgroup-scoped exchange does not satisfy the mutual-inclusion requirement.

The synchronization-scope relationship of the remaining mixed pairs is **not defined by this revision**. Those open pairs are unscoped/root-cohort, unscoped/group-cohort, unscoped/subgroup-cohort, root-cohort/group-cohort, and root-cohort/subgroup-cohort. In particular, absence of a synchronization edge from the relations defined here is not a normative statement that such a future mixed-scope interaction must be incompatible. Defining those remaining interactions requires additional execution-context or participant-inclusion evidence that this revision does not establish.
...
The complete cross-realization memory model, the complete data-race definition involving synchronization or atomic operations beyond the atomic-exchange and structured-barrier relations above, atomic order semantics beyond the base/release/acquire/acquire-release exchange classes, release sequences, atomic scope semantics beyond the subgroup-cohort form defined above, the remaining mixed interoperability among represented atomic scope forms, mixed atomic/non-atomic access rules, and additional synchronization relations are not defined by this revision. Their absence does not authorize additional conflicting ordinary accesses.
