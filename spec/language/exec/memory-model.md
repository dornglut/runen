...
- a **group-cohort-scoped exchange** is performed by one iteration of a dynamic `each` execution for which an applicable hierarchy instance has already been established and selects exactly the group containing that producer iteration in that hierarchy instance.
...
A group-cohort-scoped exchange requires an already-established hierarchy instance belonging to the producer iteration's dynamic `each`. The operation does not establish a hierarchy instance and MUST NOT treat hierarchy choice as an arbitrary hidden scheduling or realization choice. The selected group is the producer iteration's stable group membership in that established hierarchy; there is no independently supplied group identity that can disagree with the producer's membership. Hierarchy establishment, hierarchy/group identity, and membership remain owned by [Exec parallelism](parallelism.md).

The source or lowering mechanism that identifies the current iteration and requests a scoped exchange form is not defined by this revision. The source mechanism that causes an applicable hierarchy to be established or obtains hierarchy/group identities is likewise not defined here. Those open source/lowering boundaries do not make semantic scope a free-floating label: a root-cohort-scoped exchange belongs to its recorded producer iteration, and a group-cohort-scoped exchange additionally consumes that producer's already-established hierarchy membership.
...
- two group-cohort-scoped exchanges whose selected groups have distinct semantic group identities are **scope-incompatible**, including groups with equal implementation or debug tokens scoped by different hierarchy instances.
...
Subgroup-cohort and broader atomic scope are not defined by this revision. A subgroup form requires its own producer-membership admission and scope-relationship contract before it can affect synchronization.
