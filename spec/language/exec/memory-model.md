...
- two subgroup-cohort-scoped exchanges whose selected subgroups have the same semantic subgroup identity are **scope-compatible**; and
...
The complete cross-realization memory model, the complete data-race definition involving synchronization or atomic operations beyond the atomic-exchange and structured-barrier relations above, atomic order semantics beyond the base/release/acquire/acquire-release exchange classes, release sequences, atomic scope semantics beyond the subgroup-cohort form defined above, mixed interoperability among represented atomic scope forms, mixed atomic/non-atomic access rules, and additional synchronization relations are not defined by this revision. Their absence does not authorize additional conflicting ordinary accesses.
