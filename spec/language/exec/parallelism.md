...
Group-local storage, cohort-local reduction result distribution, group/subgroup collectives beyond the unordered reduction defined above, broadcast, shuffle, scans, broader atomic scope semantics, and hierarchy-sensitive fence scope semantics remain not defined by this revision. Atomic interaction and scope semantics are owned by [Exec memory model](memory-model.md); the hierarchy above defines participant-domain structure where an applicable operation explicitly consumes it, while the structured barrier and unordered reduction define only their own cohort-scoped contracts.
