...
Group-local storage, cohort-local reduction result distribution, group/subgroup collectives beyond the unordered reduction defined above, broadcast, shuffle, scans, and hierarchy-sensitive atomic/fence scope semantics remain not defined by this revision. Atomic interaction semantics are owned by [Exec memory model](memory-model.md); the hierarchy above defines participant-domain structure where an applicable operation explicitly consumes it, while the structured barrier and unordered reduction define only their own cohort-scoped contracts.
