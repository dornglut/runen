...
The source-to-Core refinement itself MUST NOT pre-contract a FloatMul with a consuming FloatAdd, reassociate a multiplication tree, rewrite FloatMul to another arithmetic operation, or erase a `Standard`/`Reproducible` boundary. It emits the represented source operation identities/grouping/use relationships and their occurrence-local contracts so the separately owned P0-C realization permissions remain decidable. Any result-changing finite multiplication reassociation or finite multiply-add contraction is legal only under the exact conditions already owned by `../core/numerics/floating-point.md`; this refinement adds no physical transformation authority.
