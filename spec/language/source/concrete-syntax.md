...
The standalone `&` punctuation token participates only in the bounded Shared-reference forms `SharedReferenceType` and `SharedBorrowValue` and as the address-selection punctuation inside the bounded `RawAddressOfValue` form `raw &x`. The latter role is available only after the reserved `raw` key in that exact production. `&` does not define binary bitwise AND, a general address-of operator, an independent raw-pointer dereference or type operator, reference pattern/binding modes, `&mut`, lifetime syntax, an arbitrary borrow/address operand, or another prefix/infix operation. The distinct longest-match `&&` token remains Boolean short-circuit conjunction and is never decomposed into two root-borrow/address tokens.
...
The standalone `*` punctuation token has exactly two bounded grammar roles. At a value/prefix start, `*` followed by one bare `UserIdentifier` forms `SharedDereferenceValue` below. After one complete left `PrefixValue` in the bounded multiplicative tier, `*` is the existing multiplication operator; under the exact surrounding required-type relation owned by `operators.md` and `function-execution.md`, that binary form maps to plain fixed-width integer multiplication for an admitted exact integer type or to same-format binary floating multiplication for exact `F16`, `F32`, or `F64`. Grammar position, not operand type, distinguishes dereference from multiplication. Thus `*r * x` contains a Shared dereference as its left prefix operand followed by binary multiplication, while `a * r` contains only binary multiplication. The bounded prefix form does not admit `**r`, `*(r)`, a field/path operand, an arbitrary `Value`, raw-pointer dereference, wildcard/pattern syntax, exponentiation, or compound assignment. `*=` tokenizes as standalone `*` followed by standalone `=` and is not represented. The ordinary block-comment delimiters `/*` and matching `*/` are recognized under the comment rules below and are never decomposed into standalone `*` or `/` punctuation.
...
The standalone `=` punctuation token retains only its represented declaration/assignment roles, including the separator in bounded `RawAssignStatement`. `==` instead participates only in the bounded Boolean equality productions below and maps those forms to the equality semantic relation in `operators.md`.
...
The `&&` punctuation token participates only in the bounded Boolean short-circuit-conjunction productions below and maps those forms to the conjunction semantic relation in `operators.md`. Adjacent `&&` is selected by longest match before standalone `&`; consequently `&&=` tokenizes as `&&` followed by standalone `=`, and `&&&` tokenizes as `&&` followed by standalone `&`. Neither sequence is a represented operator/value form. Trivia-separated `& &` is two standalone `&` tokens and is likewise not a represented form because neither the Shared root-borrow nor bounded raw-address form accepts that sequence. Adding the bounded raw-address role to standalone `&` does not define binary bitwise AND, closure syntax, compound assignment, or another operator, and it does not change the existing `&&` token or conjunction grammar.
...
A `SharedReferenceType` or `RawPointerType` is syntactically a `Type`, so the record-field grammar can represent spellings such as `field: &T` or `field: raw T`; `types.md`, `references.md`, and `raw-pointers-unsafe.md` reject Shared-reference and raw-pointer record fields in this first slice. Concrete representability therefore does not widen nominal record value shape or direct-containment semantics.

## Type forms

```text
Type                  = RawPointerType | SharedReferenceType | ReferenceReferentType
...
A `SharedReferenceType` maps to `SharedRef(T)` under `references.md`, where its one `ReferenceReferentType` supplies `T`. A `RawPointerType` maps to `RawPtr(T)` under `raw-pointers-unsafe.md`, where its one `ReferenceReferentType` supplies `T`. Both constructors are syntactically nonrecursive in this slice: neither another `SharedReferenceType` nor another `RawPointerType` is a `ReferenceReferentType`, so spellings such as `&&T`, `raw raw T`, `raw &T`, or `&raw T` are not represented. Referent/pointee admissibility and contextual admission remain semantic requirements owned by `references.md`, `raw-pointers-unsafe.md`, and `types.md`.

This subset has no nested module path, type inference, type alias, generic application, Exclusive/ExclusiveReplace reference type, mutable-reference spelling, lifetime spelling, tuple, array, vector, or other type form beyond the bounded raw-pointer and Shared-reference constructors above.
...
Parameter source order maps directly to callable-signature parameter-slot order. Each concrete parameter identifier establishes the parameter binding corresponding to that slot. Every concrete parameter binding in this subset is immutable for assignment purposes. A source-valid `SharedReferenceType` parameter remains one ordinary value parameter slot; no reference/pass-mode dimension or alternate parameter grammar is introduced. Although the general `Type` production can syntactically place a `RawPointerType` in a parameter, `callables.md` rejects every raw-pointer parameter in this activation-local slice.

When `ResultClause` is present, the callable signature has one result value of that source type. When it is absent, the callable signature has no result value. A `SharedReferenceType` result uses the same existing `-> &T` grammar and is source-valid only when `callables.md` derives exactly one Shared-reference result-origin parameter slot from the ordered parameters: there must be exactly one parameter whose resolved source type is exactly the same `SharedRef(T)`. Zero matching slots and multiple matching slots are source-invalid under that semantic admission rule. A syntactically represented `-> raw T` result is source-invalid under `callables.md`; no raw-pointer result origin, escape, or caller-visible target contract is inferred. The concrete form adds no lifetime annotation or explicit origin-selector syntax, and body implementation dataflow does not choose the Shared origin slot. Absence of a result clause does not introduce Unit, Void, or another source value.
...
Each `BlockStatement` maps to exactly one child lexical scope under `local-bindings.md`. `UnsafeBlockStatement` adds the lexical unsafe-admission fact from `raw-pointers-unsafe.md` to that same contained child scope; it does not create a second hidden scope. Execution order, normal-continuation presence, normal child-scope cleanup, loop-transfer cleanup, return cleanup, explicit-fault/defined-fault cleanup and propagation, raw-operation ordering, and divergence consequences are owned by `function-execution.md`. When a block is a conditional arm or represented `while` body, `control-flow.md` owns its relationship to the applicable selection, successor, backedge, and loop-transfer target/state rules.
...
In contrast, `if (Record { ready: true }) { ... }`, `if ((Record { ready: true })) { ... }`, `if !(Record { ready: true }) { ... }`, `if -(Record { ready: true }) { ... }`, and `if ~(Record { ready: true }) { ... }` are not represented by this conditional grammar because the group recursively requires `ConditionalValue`; grouping does not opt into unrestricted ordinary construction syntax. A standalone construction likewise cannot be introduced merely as an operand of `^`, `|`, or `&&`. Direct `raw &x` and `raw move p` are excluded for the simpler reason that neither production occurs in the conditional grammar at all.

The then arm is always one explicit `BlockStatement`. `else` is optional; when present it is followed by exactly one explicit `BlockStatement`. Each explicit arm therefore maps to one ordinary child lexical scope and may contain ordinary `BodyStatement` entries, including unsafe blocks, raw replacement, `fault;` and, when nested in a represented `while`, `break;` or `continue;`, followed by its own optional terminal `ReturnStatement` only when the preceding body-statement sequence still has a local normal continuation. The omitted-else false outcome and definite normal-successor structural/pointer-origin composition are owned by `control-flow.md`; omission does not synthesize a concrete block or lexical scope.
...
Runtime condition selection, condition producer ordering, represented operator operand execution, grouped-value transparency, arm validation, local normal-continuation composition, normal arm cleanup, loop-transfer behavior, return behavior, explicit-fault behavior, raw-pointer-origin state, other fault/divergence behavior, and exact structural-ownership/raw-pointer-origin equality whenever two normal outcomes meet are owned by `control-flow.md`, `function-execution.md`, `raw-pointers-unsafe.md`, and `operators.md` under their respective boundaries.
...
Without `MutableModifier`, the declaration establishes an immutable binding. With it, the declaration establishes a mutable binding under the assignment-mutability classification owned by `local-bindings.md`. `mut` does not create a second declaration category, a reference/memory value, or a distinct storage identity. `local-bindings.md` and `references.md` reject a mutable ordinary local whose declared type is `SharedReferenceType`; immutable Shared-reference locals are the only represented reference-local category in this slice. A `RawPointerType` local may be immutable or mutable under `local-bindings.md`; `mut` controls ordinary replacement/retargeting of the stored pointer value and grants no raw pointee-mutation authority.

Initializer lookup, owned-value production, transfer, the resulting initial structural ownership state, any exact raw-pointer origin/lexical target-validity requirement, and the point at which the new local enters scope are determined by `local-bindings.md`, `structural-ownership.md`, `references.md` or `raw-pointers-unsafe.md` where applicable, and `function-execution.md`.
...
The target identifier is resolved using the unqualified function-body lookup precedence from `local-bindings.md`. The concrete form maps that selected target and RHS `Value` to the whole-binding assignment relation owned there. A source-valid assignment therefore requires the selected entity, assignment mutability, structural-ownership transition, active Shared-authority restriction where applicable, target/RHS source types, and for a raw-pointer target the incoming pointer-origin lexical validity to satisfy `local-bindings.md`, `references.md`, `raw-pointers-unsafe.md`, `structural-ownership.md`, and `types.md`; concrete syntax does not redefine those requirements.

Assignment is a statement and produces no source value. It does not introduce Unit/Void or participate in `Value` grammar.

RHS evaluation, source-first old-value replacement cleanup, value transfer, successful target structural ownership reset, raw-pointer origin installation where applicable, straight-line sequencing, and fault/divergence consequences are owned by `function-execution.md`.

This form targets only the complete selected binding. A mutable raw-pointer local may therefore be ordinarily assigned/retargeted by this same whole-binding syntax when the incoming exact `RawPtr(T)` value is valid. There is no field/member assignment, destructuring assignment, compound assignment, qualified assignment target, reference-relative assignment, raw pointee replacement through ordinary assignment, or general place/lvalue grammar in this subset.
...
Concrete syntax admits this statement wherever `BodyStatement` is admitted, including outside an unsafe block. Source validity, however, requires an active lexical unsafe-admission region, exact pointer origin/target validity, source-first RHS evaluation, the post-source Shared-authority exclusion, remaining-target-frontier cleanup, and complete-root replacement under `raw-pointers-unsafe.md` and `function-execution.md`. The pointee target's ordinary binding `mut` classification is not a raw-replacement grammar or validity requirement.

`RawAssignStatement` is statement-only, produces no value, and requires its trailing semicolon. It does not define `raw assign` as a general compound/indirect assignment operator, `*p = value`, field/path raw assignment, pointer retargeting, or a general place/lvalue category.
...
Argument source order is the direct-call argument order consumed by `function-execution.md`. A trailing comma is permitted. Because arguments are ordinary `Value`s, an existing Shared-reference binding, root `&x`, bounded `*r`, or unsafe `raw move p` may appear in an argument position when the corresponding ordinary parameter type accepts the exact produced value. `RawPtr(T)` itself is not parameter-admissible under `callables.md`, so `raw &x` or a raw-pointer `IdentifierUse` cannot make a raw-pointer call-transfer interface source-valid. This adds no borrowed/pass-mode/raw-pointer argument grammar.

A result-bearing direct call uses the same `DirectCall` grammar regardless of whether its callable result is ordinary or the bounded contract-bearing `SharedReferenceType`. The callable signature, not syntax at the call site, determines the result type and any advertised Shared-reference result-origin slot. `references.md` and `function-execution.md` own the identity/provenance and cleanup consequences; no call-site lifetime argument, result-origin selector, reborrow marker, raw-pointer escape selector, or alternate result syntax is introduced. Raw-pointer result types are rejected by `callables.md`.
...
A valid no-result call statement produces no source value to discard. A `GroupedValue`, `NumericContractSelectedValue`, `RawAddressOfValue`, or `RawMoveValue` does not become a `CallStatement` or another body-statement starter merely because it can be an ordinary value producer.
...
Because each initializer contains a `Value`, record construction composes recursively with another record construction as well as the other represented value producers and wrappers, including represented operators, bounded Shared-reference producers, bounded raw-pointer producers, bounded contextual grouping, operation-local numeric-contract selection, and bounded producer-backed field-value use. Record fields themselves remain unable to have Shared-reference or raw-pointer type under `types.md`, `references.md`, and `raw-pointers-unsafe.md`, so those producers cannot successfully initialize such a record field in this slice.
...
PrefixValue          = SharedDereferenceValue | BooleanNotValue | IntegerNegValue | IntegerComplementValue | ValueAtom
SharedDereferenceValue = "*" UserIdentifier
...
SharedBorrowValue            = "&" UserIdentifier
...
The existing `XorValue = AdditiveValue XorSuffix?` production is unchanged. `OrValue` is one transparent outer wrapper around `XorValue` when `OrSuffix` is absent. Consequently every source-valid spelling containing no standalone `|` retains its previous tokenization, accepted grammar tree below the new wrapper, operator nesting, and semantic mapping; the wrapper adds no new semantic node when its optional suffix is absent. XOR is structurally tighter than bitwise OR only because the new bounded wrapper consumes complete `XorValue` operands; this revision establishes no family-wide bitwise precedence policy beyond this exact current grammar.

At every `PrefixValue` decision point, a standalone `-` followed across permitted ordinary trivia by `DecimalMagnitude` or `DecimalFloatingMagnitude` MUST be consumed by the complete applicable signed `Literal` production rather than `IntegerNegValue`. Only when neither signed-literal production applies may value-start `-` introduce `IntegerNegValue`. This priority resolves the deliberate shared punctuation without reclassifying signed literals as operators. Standalone `~` has no literal role and therefore introduces `IntegerComplementValue` directly at a prefix start. Standalone `*` at a prefix start selects `SharedDereferenceValue` only when followed by one bare `UserIdentifier`; binary multiplication remains only the `MultiplicativeSuffix` after a complete left prefix value. The reserved `raw` key at an atom start selects only `RawAddressOfValue` when followed by `&` or `RawMoveValue` when followed by the contextual `move` token and then one bare `UserIdentifier`; no type-driven reinterpretation is used.

The eight non-wrapper `ValueAtom` alternatives are the pre-existing literal/identifier/call/construction/field-value producers, the bounded Shared root-borrow producer, and the bounded raw-address/raw-move producers. `GroupedValue` and `NumericContractSelectedValue` are different: each wraps exactly one complete existing `Value` and neither becomes another owned-value producer family, primary-expression semantic category, postfix base, place category, member-receiver system, or call target. `SharedDereferenceValue` is a separate bounded prefix producer rather than an atom wrapper. Grouping is semantically transparent. Numeric-contract selection qualifies one existing governed root operator according to `function-execution.md` and may be erased after that selected contract is retained on the operation.

`SharedBorrowValue` maps exactly to root Shared-borrow formation in `references.md`. Its operand is one bare unqualified `UserIdentifier`; no field path, qualified member, grouped value, dereference result, call result, construction, or arbitrary value may occupy this position. The surrounding required source type supplies the exact `SharedRef(T)` expectation consumed by the reference owner.

`SharedDereferenceValue` maps exactly to the bounded Shared dereference/copy producer in `references.md`. Its operand is one bare unqualified `UserIdentifier` naming an active Shared-reference parameter/local binding. The grammar is deliberately nonrecursive: `**r` and `*(r)` are not represented, while `(*r)` is ordinary grouping around one already complete `SharedDereferenceValue`. No reference-relative field selector or general dereference-expression category follows from this form.

`RawAddressOfValue` maps exactly to complete-root raw address formation in `raw-pointers-unsafe.md`. Its operand is one bare unqualified `UserIdentifier` naming an active parameter or ordinary local binding of a first-slice raw-pointee-admissible type. The surrounding required source type supplies the exact `RawPtr(T)` expectation. The spelling is deliberately `raw &x` rather than a new standalone punctuation token; `&x` without `raw` retains only Shared-borrow meaning.

`RawMoveValue` maps exactly to the unsafe raw ownership-move producer in `raw-pointers-unsafe.md`. Its final operand is one bare unqualified `UserIdentifier` naming an active binding of exact type `RawPtr(T)`. Concrete grammar admits the form both inside and outside unsafe blocks; semantic validity requires an active unsafe-admission region and all exact target availability/Shared-authority conditions. The grammar is nonrecursive and defines no `raw move (p)`, `raw move p.field`, `*p`, arbitrary pointer-expression operand, or non-consuming raw load.

Because either wrapper contains a complete `Value`, wrapper recursion is structurally represented. Their semantic target rules differ. Ordinary `GroupedValue` is transparent to selector root discovery, so `@fast((a * b))` selects the same floating-multiplication root as `@fast(a * b)`, `@fast((a / b))` selects the same floating-division root as `@fast(a / b)`, `@fast((a + b))` selects the same floating-addition root as `@fast(a + b)`, and `@fast((a - b))` selects the same floating-subtraction root as `@fast(a - b)`. A `NumericContractSelectedValue` encountered while discovering the root for another selector is not transparent, so syntactically represented same-root stacked forms equivalent to `@fast(@fast(a * b))`, `@fast((@fast(a * b)))`, `@fast(@fast(a / b))`, `@fast((@fast(a / b)))`, `@fast(@fast(a + b))`, `@fast((@fast(a + b)))`, `@fast(@fast(a - b))`, and `@fast((@fast(a - b)))` are source-invalid under `function-execution.md` rather than establishing duplicate or overriding selections for one root occurrence. That opacity does not prohibit a selector on a distinct governed root nested as an operand of another governed operation. A Shared root-borrow, Shared dereference/copy, raw address formation, or raw ownership move discovered as the selected wrapper's root is not a governed floating operation and therefore fails numeric-contract selector applicability before reference/raw/value effects commit.
...
This distinction MUST remain intact through source validation. Under required `U8`, for example, `-1` is rejected by literal representability while `-(1)` may validate as integer negation and produce the modulo result selected by `operators.md`. A parser, frontend, or optimizer may not normalize either spelling into the other based on host-language unary-minus behavior. The represented floating-subtraction operation does not weaken this boundary: it exists only at the binary additive `-` position after a complete left multiplicative value. The represented floating-multiplication, floating-division, exclusive-or, bitwise-OR, conjunction, selector wrappers, bounded reference forms, and bounded raw-pointer forms likewise do not alter this signed-literal boundary.

An `IdentifierUse` maps to ordinary whole-binding owned-value use under `local-bindings.md`. Its identifier is resolved using the function-local lookup precedence owned there. In this subset, the selected entity MUST be a parameter or ordinary local binding whose complete structural root is fully available under `structural-ownership.md`; another selected entity category does not become a value merely because the context requires one. When that binding has Shared-reference type, ordinary use has the reference-duplication/carrier consequence owned by `references.md`. When it has raw-pointer type, ordinary use duplicates the pointer value and preserves its exact pointer-origin provenance under `raw-pointers-unsafe.md`; it does not access the pointee.

A `SharedBorrowValue`, `SharedDereferenceValue`, `RawAddressOfValue`, and `RawMoveValue` each use the same unqualified function-local lookup precedence and require the selected entity to be an active parameter or ordinary local binding of the exact category/type required by their owner. `RawAssignStatement` uses that precedence for its pointer operand as well. Lookup does not bypass a selected wrong-category module binding or invent a module/static borrow/raw target when no applicable local binding is available.

A `DirectCall` may be used as a `Value` only when its callable signature specifies one result value. The successful call result is the owned value produced by `function-execution.md`. When that result is the bounded contract-bearing `SharedReferenceType`, the same concrete call produces the identity-preserving Shared-reference value described by `references.md`, including the caller-side origin provenance derived from the callee's advertised parameter slot. No distinct reference-result value syntax or call form is introduced. `RawPointerType` results are semantically rejected and therefore no direct call produces a raw-pointer value in this slice. The same result-bearing concrete call form may also appear as the receiver of a `ProducerFieldValueUse` or in the dedicated producer-backed record-pattern scrutinee position when its result type satisfies those independently owned categories. Grouping or selecting a call for an ordinary receiving position does not create an indirect/grouped call target or widen either dedicated category; a selector whose root is the call is source-invalid because direct call is not a currently governed numeric operation.
...
In a result-bearing function, every represented path that reaches a `ReturnStatement` must use `return Value;`, and the returned value's type and ownership transfer are governed by `function-execution.md` and MUST satisfy the callable result type. Because grouping and numeric-contract selection are part of `Value`, `return (value);`, a valid `return @fast(a * b);`, a valid `return @fast(a / b);`, a valid `return @fast(a + b);`, and a valid `return @fast(a - b);` require no return-specific wrapper rule and have exactly the contained producer semantics plus the selected contract fact. For a contract-bearing Shared-reference result, forms such as `return p;`, `return saved;`, or `return forward(p);` are represented by the same grammar and are source-valid only when `references.md` proves the returned Shared-reference flow preserves the enclosing callable's advertised parameter-slot provenance and exact dynamic target/authority. A fresh root form such as `return &local;` is syntactically represented but cannot satisfy that result-origin contract because root borrow creates a different origin. `return raw &x;` is likewise syntactically a `Value` return but is source-invalid because `RawPtr(T)` is not result-admissible. An unsafe `return raw move p;` may return the ordinary pointee value `T` when `T` is the callable's admitted result type and all raw-move preconditions hold; the raw pointer itself does not escape. No return-specific lifetime, origin-selector, or raw escape syntax is introduced.
...
Consequently, when a parameter/local binding has the same key as a module-level function, an unqualified direct-call spelling resolves to the local binding and is invalid as a direct call rather than bypassing it. For assignment, a selected parameter/local binding is validated for assignment mutability, active Shared-authority restrictions, and raw-pointer incoming-origin requirements where applicable; when no local exists and same-module lookup selects a module declaration, that entity is invalid as an assignment target rather than bypassed. A `BindingFieldValueUse` root and direct binding-root record-pattern scrutinee follow the same precedence and require a parameter/local binding under their owners. `SharedBorrowValue`, `SharedDereferenceValue`, `RawAddressOfValue`, `RawMoveValue`, and the pointer operand of `RawAssignStatement` likewise require an applicable active parameter/local binding and do not reinterpret a same-module declaration as source-addressable storage.
...
After qualified lookup selects the target binding, the consuming `ReferenceReferentType`, direct-call, record-construction, or record-pattern-head context validates the entity category. Because both `SharedReferenceType` and `RawPointerType` consume `ReferenceReferentType`, the same qualified nominal record spelling can supply a Shared referent or raw pointee when that type constructor's own semantic admission permits it. Lookup MUST NOT skip a private or wrong-category target to search for another entity.
...
- assignment expressions, assignment-as-value, field assignment, partial-field reinitialization, destructuring assignment, reference-relative assignment, raw field/path assignment, or general place/lvalue syntax beyond represented whole-binding assignment and the bounded raw-pointee replacement statement;
- uninitialized locals, type inference, mutable parameters, mutable Shared-reference locals, or mutable record-pattern binding modifiers;
...
- record-pattern scrutinees beyond the represented bare direct binding root and dedicated `DirectCall`, `RecordConstruction`, and bounded `FieldValueUse` producer-backed forms; in particular no literal (including decimal floating), bare `IdentifierUse`-as-value, represented operator/reference/raw-pointer value, grouping, numeric-contract-selected value, other operator expression, conversion, arbitrary postfix/member expression, or other general expression is admitted there;
...
- arbitrary-receiver member/postfix access beyond the explicit binding-root/direct-call/record-construction field-value forms; reference-relative or raw-pointer-relative field access; field accessibility beyond the represented module-private/exported direct relation; package/friend/protected accessibility; methods; properties; or associated-item lookup;
...
- source reference semantics beyond the bounded Shared `&T`, root `&x`, bare-binding duplicate dereference `*r`, and unique-origin identity-preserving scalar Shared-reference function result described above: no `Exclusive`/`ExclusiveReplace` source references, `&mut`, mutable/exclusive reference spelling, reborrow/child authority, non-root or field/path borrow target, nested reference type, reference-containing record field or aggregate result, multiple/explicit/derived/static result origins, explicit result-origin selector spelling, reference Move/Drop/Assign/InteriorAssign, reference-relative field access, reference pattern/binding mode, named lifetime/parameter/outlives syntax, or non-lexical authority-shortening syntax;
- source raw-pointer semantics beyond the bounded activation-local `raw T`, `raw &x`, contextual `raw move p`, contextual `raw assign p = Value;`, ordinary raw-pointer local duplication/retargeting, and lexical `unsafe` block described above: no raw-pointer parameter/result transfer, pointer-containing record field/aggregate, pointer-to-pointer or pointer-to-Shared-reference type, Shared reference to a raw-pointer value, null/fabricated/integer pointer, source `RawRead`/non-consuming owned raw load, field/path raw address, general raw dereference syntax, pointer arithmetic/offset/one-past rule, pointer equality/ordering/hash/identity observation, pointer/integer conversion, target-sized integer, physical address/layout/alignment/endian/representation/relocation/stability/pinning contract, heap/global/static raw storage, unsafe function/callable/call contract, caller proof obligation, user-written proof contract, or reference-to-raw conversion;
