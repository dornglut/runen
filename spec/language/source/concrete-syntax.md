...
A reserved key is not legal where the grammar requires a user identifier. This revision reserves no other identifier key and defines no escaping mechanism for a reserved key. In particular, `fast` is **not** reserved: an identifier-form token whose lexical identifier key is `fast` remains a user identifier except in the exact contextual selector position defined by `NumericContractSelectedValue` below.
...
The standalone `&` punctuation token participates only in the bounded Shared-reference forms below: `SharedReferenceType` and `SharedBorrowValue`. Before this revision standalone `&` was unrepresented material, so adding those bounded roles reinterprets no previously source-valid spelling. `&` does not define binary bitwise AND, a general address-of operator, raw-pointer syntax, reference pattern/binding modes, `&mut`, lifetime syntax, an arbitrary borrow-expression operand, or another prefix/infix operation. The distinct longest-match `&&` token remains Boolean short-circuit conjunction and is never decomposed into two root-borrow tokens.
...
A `SharedReferenceType` is syntactically a `Type`, so the record-field grammar can represent a spelling such as `field: &T`; `types.md` and `references.md` reject Shared-reference record fields in this first slice. Concrete representability therefore does not widen nominal record value shape or direct-containment semantics.

## Type forms

```text
Type                  = SharedReferenceType | ReferenceReferentType
...
A `SharedReferenceType` maps to `SharedRef(T)` under `references.md`, where its one `ReferenceReferentType` supplies `T`. The nonrecursive grammar is deliberate: another `SharedReferenceType` is not a `ReferenceReferentType`, so nested spellings such as `&&T` are not represented. Referent admissibility and contextual admission remain semantic requirements owned by `references.md` and `types.md`.

This subset has no nested module path, type inference, type alias, generic application, raw-pointer type, Exclusive/ExclusiveReplace reference type, mutable-reference spelling, lifetime spelling, tuple, array, vector, or other type form.
...
Without `MutableModifier`, the declaration establishes an immutable binding. With it, the declaration establishes a mutable binding under the assignment-mutability classification owned by `local-bindings.md`. `mut` does not create a second declaration category, a reference/memory value, or a distinct storage identity. `local-bindings.md` and `references.md` reject a mutable ordinary local whose declared type is `SharedReferenceType`; immutable Shared-reference locals are the only represented reference-local category in this slice.

Initializer lookup, owned-value production, transfer, the resulting initial structural ownership state, and the point at which the new local enters scope are determined by `local-bindings.md`, `structural-ownership.md`, `references.md` where applicable, and `function-execution.md`.
...
This form targets only the complete selected binding. There is no field/member assignment, destructuring assignment, compound assignment, qualified assignment target, reference-relative assignment, raw-pointer assignment, or general place/lvalue grammar in this subset.
...
Argument source order is the direct-call argument order consumed by `function-execution.md`. A trailing comma is permitted. Because arguments are ordinary `Value`s, an existing Shared-reference binding, root `&x`, or bounded `*r` may appear in an argument position when the corresponding ordinary parameter type accepts that exact value type. This adds no borrowed/pass-mode argument grammar.

A result-bearing direct call uses the same `DirectCall` grammar regardless of whether its callable result is ordinary or the bounded contract-bearing `SharedReferenceType`. The callable signature, not syntax at the call site, determines the result type and any advertised Shared-reference result-origin slot. `references.md` and `function-execution.md` own the identity/provenance and cleanup consequences; no call-site lifetime argument, result-origin selector, reborrow marker, or alternate result syntax is introduced.
...
A valid no-result call statement produces no source value to discard. A `GroupedValue` or `NumericContractSelectedValue` does not become a `CallStatement` or another body-statement starter.
...
Because each initializer contains a `Value`, record construction composes recursively with another record construction as well as the other represented value producers and wrappers, including represented operators, bounded Shared-reference producers, bounded contextual grouping, operation-local numeric-contract selection, and bounded producer-backed field-value use. Record fields themselves remain unable to have Shared-reference type under `types.md`/`references.md`, so these producers cannot successfully initialize a reference-containing record field in this slice.
...
`FastContextualKey` is not a reserved key and this production does not define a family of arbitrary `@name(...)` forms. It recognizes exactly a maximal identifier-form token whose lexical identifier key is `fast` after the represented `@` token in this one grammar position. The same token is an ordinary `UserIdentifier` everywhere else that `UserIdentifier` is admitted.
...
In a result-bearing function, every represented path that reaches a `ReturnStatement` must use `return Value;`, and the returned value's type and ownership transfer are governed by `function-execution.md` and MUST satisfy the callable result type. Because grouping and numeric-contract selection are part of `Value`, `return (value);`, a valid `return @fast(a * b);`, a valid `return @fast(a / b);`, a valid `return @fast(a + b);`, and a valid `return @fast(a - b);` require no return-specific wrapper rule and have exactly the contained producer semantics plus the selected contract fact. For a contract-bearing Shared-reference result, forms such as `return p;`, `return saved;`, or `return forward(p);` are represented by the same grammar and are source-valid only when `references.md` proves the returned Shared-reference flow preserves the enclosing callable's advertised parameter-slot provenance and exact dynamic target/authority. A fresh root form such as `return &local;` is syntactically represented but cannot satisfy that result-origin contract because root borrow creates a different origin. No return-specific lifetime or origin-selector syntax is introduced.
...
After qualified lookup selects the target binding, the consuming reference-referent type, direct-call, record-construction, or record-pattern-head context validates the entity category. Lookup MUST NOT skip a private or wrong-category target to search for another entity.
...
- assignment expressions, assignment-as-value, field assignment, partial-field reinitialization, destructuring assignment, reference-relative assignment, or general place/lvalue syntax beyond represented whole-binding assignment;
...
- record-pattern scrutinees beyond the represented bare direct binding root and dedicated `DirectCall`, `RecordConstruction`, and bounded `FieldValueUse` producer-backed forms; in particular no literal (including decimal floating), bare `IdentifierUse`-as-value, represented operator/reference value, grouping, numeric-contract-selected value, other operator expression, conversion, arbitrary postfix/member expression, or other general expression is admitted there;
