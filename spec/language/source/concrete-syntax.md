...
The standalone `@` punctuation token participates only in `NumericContractSelectedValue` below. Before this revision `@` was not represented punctuation and source containing it outside trivia was malformed, so introducing this one bounded role reinterprets no previously source-valid spelling. `@` does not introduce a generic attribute, annotation, decorator, pragma, compiler-directive, macro, metadata, or namespace mechanism. It does not reserve arbitrary following identifier keys. Only an immediately following contextual identifier token with lexical key `fast`, followed by the required parenthesized complete `Value`, forms the selected-value production. Ordinary trivia may occur between these grammar tokens under the general trivia rule. Spellings such as `@name(...)`, `@standard(...)`, and `@reproducible(...)` are not represented selector forms in this revision.
...
The five non-wrapper `ValueAtom` alternatives are exactly the pre-existing value producer families admitted at atom position. `GroupedValue` and `NumericContractSelectedValue` are different: each wraps exactly one complete existing `Value` and neither becomes a seventh owned-value producer family, primary-expression semantic category, postfix base, place category, member-receiver system, or call target. Grouping is semantically transparent. Numeric-contract selection qualifies one existing governed root operator according to `function-execution.md` and may be erased after that selected contract is retained on the operation. The operator producer family remains one existing producer family even though it now contains Boolean negation, plain fixed-width integer negation, plain fixed-width integer bitwise complement, plain fixed-width integer multiplication, plain fixed-width integer addition, same-format binary floating addition, plain fixed-width integer subtraction, plain fixed-width integer exclusive-or, plain fixed-width integer bitwise OR, Boolean equality/inequality, and Boolean short-circuit conjunction.

Because either wrapper contains a complete `Value`, wrapper recursion is structurally represented. Their semantic target rules differ. Ordinary `GroupedValue` is transparent to selector root discovery, so `@fast((a + b))` selects the same floating-addition root as `@fast(a + b)`. A `NumericContractSelectedValue` is not transparent to another selector's root discovery, so syntactically represented forms equivalent to `@fast(@fast(a + b))` and `@fast((@fast(a + b)))` are source-invalid under `function-execution.md` rather than establishing duplicate or overriding selections for one root occurrence.

Selectors on distinct governed occurrences compose without inheritance. `@fast(a + b) + c` places the selected inner value as the left atom of an unqualified outer `+`; when both are floating operations, the inner occurrence is `fast` and the outer occurrence independently receives `standard`. Conversely, `@fast((a + b) + c)` selects the outer addition while the grouped inner unqualified addition independently receives `standard`. A form such as `@fast(a + @fast(b + c))` can select two distinct floating-addition occurrences when each satisfies the exact floating required-type relation. These syntax trees do not by themselves grant result-changing reassociation across a `standard` boundary; the accepted mixed-contract rules in the Core floating owner remain controlling.
...
A `NumericContractSelectedValue` likewise maps to no new owned-value producer. `function-execution.md` passes the required type through unchanged, locates one root governed operation through ordinary grouping only, validates that the current root is same-format floating addition, establishes `fast` for that one occurrence, and then uses the existing operation execution relation. The wrapper may be erased after typed validation only when the selected contract remains retained directly on that floating-addition occurrence. It creates no block/function/module default, caller state, runtime mode, or general annotation mechanism.
