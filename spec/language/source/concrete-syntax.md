...
Each `BlockStatement` maps to exactly one child lexical scope under `local-bindings.md`. Execution order, normal-continuation presence, normal child-scope cleanup, loop-transfer cleanup, return cleanup, explicit-fault/defined-fault cleanup and propagation, divergence consequences are owned by `function-execution.md`. When a block is a conditional arm or represented `while` body, `control-flow.md` owns its relationship to the applicable selection, successor, backedge, and loop-transfer target/state rules.
...
The exclusion is part of normative concrete grammar. The unqualified record-construction form begins with `UserIdentifier "{"`; admitting unrestricted ordinary `Value`, `AdditiveValue`, `MultiplicativeValue`, or `PrefixValue` immediately after `if` would make a spelling such as `if flag { ... }` collide with the record-construction token shape. A leading `!` does not delimit its operand and therefore does not remove that ambiguity: `if !flag { ... }` must still parse `flag` as one conditional `IdentifierUse` and the following `{ ... }` as the then `BlockStatement`, not as a standalone construction operand. The multiplicative and additive tiers likewise do not reset either side to unrestricted ordinary syntax. `ConditionalBooleanNotValue` consequently recurses only through `ConditionalPrefixValue`, `ConditionalMultiplicativeValue` contains only conditional prefix operands, and `ConditionalAdditiveValue` contains only conditional multiplicative operands.

The optional equality suffix requires a second `ConditionalAdditiveValue`, not an unrestricted ordinary value. Consequently the standalone-construction exclusion remains true on both sides of `==` or `!=`, including beneath any number of Boolean-not prefixes, inside either operand of bounded multiplication or addition/subtraction, and inside any number of conditional groups. Forms such as `if flag == other { ... }`, `while !flag != !other { ... }`, and syntactically represented arithmetic forms complete the conditional value before the following block opener.

`ConditionalBooleanNotValue` maps to the same Boolean logical-negation semantic relation in `operators.md` as the ordinary-value `BooleanNotValue` defined below. A `ConditionalMultiplicativeValue` containing `*` maps to the same plain fixed-width integer-multiplication relation as the ordinary multiplicative tier. A `ConditionalAdditiveValue` containing `+` maps to the same plain fixed-width integer-addition relation as the ordinary additive tier; one containing binary `-` maps to the distinct plain fixed-width integer-subtraction relation. `ConditionalEqualityValue` with `==` or `!=` maps to the same Boolean equality/inequality relations as the ordinary equality tier. These concrete placements do not define condition-specific operators. Repeated prefix forms such as `!!flag` or `! ! flag` remain recursively represented and associate from the right by grammar nesting; the multiplicative, additive, and equality tiers are not part of that prefix recursion. A group may explicitly contain a complete conditional equality tier, so `if !(a == b) { ... }`, `if (a == b) == c { ... }`, and `if a == (b != c) { ... }` remain represented according to the same operator semantics and exact-Bool condition requirement.
...
Decimal integer and decimal floating literals remain syntactically represented as conditional atoms. Exact condition and operator typing are owned by `control-flow.md`, `operators.md`, and `function-execution.md`. A bare numeric literal remains syntax-valid as a `ConditionalValue` but is source-invalid because it cannot produce the exact intrinsic `Bool` required by control flow. The same is true of a plain integer-multiplication, integer-addition, or integer-subtraction value used directly as a condition: the condition supplies required type `Bool`, while `operators.md` admits those integer operations only when the surrounding exact required type is one of the eight fixed-width integer types, so the operation is rejected before its operands may commit ownership. Likewise, an integer arithmetic operation appearing as an operand of exact-Bool equality is syntactically represented but source-invalid because equality supplies required type `Bool` to that complete additive operand. A negated or equality operand numeric atom, such as `!-1` or `1 == 1`, may likewise be syntactically represented while failing the exact Bool operand rule owned by the operator semantics.
...
The ordinary represented value grammar has one bounded multiplicative tier between the existing recursive prefix tier and bounded additive tier, with the bounded additive tier below the non-associative equality tier and bounded grouping admitted as one atom wrapper:
...
`BooleanNotValue` maps to the Boolean logical-negation semantic relation in `operators.md`. Its right-recursive `PrefixValue` operand makes repeated negation such as `!!flag` or `! ! flag` unambiguous and right-associated by grammar nesting. Boolean-not recursion does not include `MultiplicativeValue`, `AdditiveValue`, or `EqualityValue`; consequently prefix binds more tightly than multiplication, addition/subtraction, and equality except where grouping explicitly nests a complete `Value`. `!a * b` is one multiplicative value whose left operand is `!a`; `!a + b` is one additive value whose left multiplicative operand contains `!a`; `!a - b` is likewise one additive value whose left multiplicative operand contains `!a`; and `!a == b` remains one equality value whose left additive operand contains `!a`. The prefix form never reparses those ungrouped forms as logical negation of a looser operator tier.
...
- general numeric unary negation, arithmetic beyond bounded plain fixed-width integer multiplication/addition/subtraction, equality/inequality for any source type other than the represented exact-Bool relations, ordering or other comparison, short-circuit logical, compound-assignment, conversion/cast, or other operator forms beyond represented Boolean negation, plain fixed-width integer multiplication/addition/subtraction, and Boolean equality/inequality;
