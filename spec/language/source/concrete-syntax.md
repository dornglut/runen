...
The standalone `.` punctuation token participates only as the existing `FieldSelector` in three bounded contexts: represented `FieldValueUse`, the Shared root target after `&` in `&x.field...`, and the Shared reborrow target after `&*r` in `&*r.field...`. The decimal point inside one `DecimalFloatingMagnitude` is consumed as interior material of that single decimal token and is therefore not a `.` punctuation token. Adjacent `..` is the distinct longest-match punctuation token described next and is never two standalone field selectors. Reusing `FieldSelector` for the two safe-reference target branches does not make a safe-reference value a field-value receiver, create a general postfix/member system, admit field assignment, or give `.` another operator role.
