...
- `DecimalMagnitude`, or standalone `-` followed across permitted ordinary trivia by `DecimalMagnitude`, selects a decimal-integer equality-only literal-test leaf under the existing signed integer-literal grammar;
- standalone `<` followed by `DecimalMagnitude`, or by standalone `-` followed across permitted ordinary trivia by `DecimalMagnitude`, selects a fixed-width-integer strict-upper-bound-only test leaf whose bound is the complete existing `DecimalIntegerLiteral` beginning after `<`;
- `UserIdentifier "=="` followed by `true`, `false`, `DecimalMagnitude`, or standalone `-` followed across permitted ordinary trivia by `DecimalMagnitude` selects one same-path equality bind-and-test target; the leading identifier is the success-binding declaration and the following literal is the existing complete `RefutableRecordLiteralTest`;
- `UserIdentifier "<"` followed by `DecimalMagnitude`, or by standalone `-` followed across permitted ordinary trivia by `DecimalMagnitude`, selects one same-path strict-upper-bound bind-and-test target; the leading identifier is the success-binding declaration and the following integer is the existing complete bound;
