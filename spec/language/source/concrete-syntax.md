...
The standalone `-` punctuation token has exactly two represented grammar roles in this revision. At the start of a `DecimalIntegerLiteral` or `DecimalFloatingLiteral`, it is the existing optional negative sign and retains exactly the literal meaning owned by `literals.md`. After one complete left `PrefixValue` in the bounded additive tier, it may instead be the binary plain fixed-width integer-subtraction operator mapped to `operators.md`. These roles do not define general numeric unary negation, decrement, compound assignment, or another operator. In particular, `-(1)` and `-value` remain unrepresented, while `a - -1` is represented as binary subtraction whose right operand is the existing negative integer literal. Because `--` is not a punctuation token, adjacent `a--1` has the same represented token sequence and grammar meaning as `a - -1`; it is not decrement. `-=` is standalone `-` followed by standalone `=` and is not a represented operator form. Longest-match `->` remains one arrow token and is never reinterpreted as subtraction followed by `>`.
...
ConditionalAdditiveValue =
    ConditionalPrefixValue ConditionalAdditiveSuffix?
ConditionalAdditiveSuffix = AdditiveOperator ConditionalPrefixValue
...
`ConditionalValue` deliberately has its own equality, additive, and prefix tiers rather than reusing unrestricted ordinary `EqualityValue`, `AdditiveValue`, or `PrefixValue`. This preserves the accepted exclusion of a **standalone** `RecordConstruction` at every conditional equality operand, additive operand, and recursive prefix depth.

`ConditionalGroupedValue` is likewise context-preserving: its one inner value is another complete `ConditionalValue`, not unrestricted ordinary `Value`. Parentheses therefore add explicit tree nesting without resetting conditional grammar. The existing standalone-construction exclusion remains true through every grouping depth and through any nested `!`, `+`, or binary `-` reached inside a group. Delimiter-based widening of conditional syntax is not part of this grouping relation.

The exclusion is part of normative concrete grammar. The unqualified record-construction form begins with `UserIdentifier "{"`; admitting unrestricted ordinary `Value`, `AdditiveValue`, or `PrefixValue` immediately after `if` would make a spelling such as `if flag { ... }` collide with the record-construction token shape. A leading `!` does not delimit its operand and therefore does not remove that ambiguity: `if !flag { ... }` must still parse `flag` as one conditional `IdentifierUse` and the following `{ ... }` as the then `BlockStatement`, not as a standalone construction operand. The additive tier likewise does not reset either side to unrestricted ordinary syntax. `ConditionalBooleanNotValue` consequently recurses only through `ConditionalPrefixValue`, and `ConditionalAdditiveValue` contains only `ConditionalPrefixValue` operands.

The optional equality suffix requires a second `ConditionalAdditiveValue`, not an unrestricted ordinary value. Consequently the standalone-construction exclusion remains true on both sides of `==` or `!=`, including beneath any number of Boolean-not prefixes, inside either operand of a bounded additive operation, and inside any number of conditional groups. Forms such as `if flag == other { ... }`, `while !flag != !other { ... }`, and syntactically represented additive forms complete the conditional value before the following block opener.

`ConditionalBooleanNotValue` maps to the same Boolean logical-negation semantic relation in `operators.md` as the ordinary-value `BooleanNotValue` defined below. A `ConditionalAdditiveValue` containing `+` maps to the same plain fixed-width integer-addition relation as the ordinary additive tier; one containing binary `-` maps to the distinct plain fixed-width integer-subtraction relation. `ConditionalEqualityValue` with `==` or `!=` maps to the same Boolean equality/inequality relations as the ordinary equality tier. These concrete placements do not define condition-specific operators. Repeated prefix forms such as `!!flag` or `! ! flag` remain recursively represented and associate from the right by grammar nesting; the additive and equality tiers are not part of that prefix recursion. A group may explicitly contain a complete conditional equality tier, so `if !(a == b) { ... }`, `if (a == b) == c { ... }`, and `if a == (b != c) { ... }` remain represented according to the same operator semantics and exact-Bool condition requirement.

Exactly zero or one ungrouped additive operator is represented at each conditional additive level. Ungrouped `a + b + c`, `a + b - c`, `a - b + c`, and `a - b - c` are therefore not represented. Existing context-preserving grouping can express explicit nested syntax such as `(a + b) - c`, `(a - b) + c`, `a - (b - c)`, or `a + (b - c)` without introducing additive associativity or a general expression hierarchy.

Decimal integer and decimal floating literals remain syntactically represented as conditional atoms. Exact condition and operator typing are owned by `control-flow.md`, `operators.md`, and `function-execution.md`. A bare numeric literal remains syntax-valid as a `ConditionalValue` but is source-invalid because it cannot produce the exact intrinsic `Bool` required by control flow. The same is true of a plain integer-addition or integer-subtraction value used directly as a condition: the condition supplies required type `Bool`, while `operators.md` admits either additive operation only when the surrounding exact required type is one of the eight fixed-width integer types, so the operation is rejected before its operands may commit ownership. Likewise, an additive operation appearing as an operand of exact-Bool equality is syntactically represented but source-invalid because equality supplies required type `Bool` to that complete additive operand. A negated or equality operand numeric atom, such as `!-1` or `1 == 1`, may likewise be syntactically represented while failing the exact Bool operand rule owned by the operator semantics.
...
`WhileStatement` reuses the exact `ConditionalValue` nonterminal above. It therefore admits the same bounded additive and Boolean equality/inequality tiers, recursively prefixed Boolean logical-negation forms, contextual grouped values, and the same literal, identifier-use, direct-call, and bounded field-value atoms while preserving the same standalone-`RecordConstruction` exclusion at every conditional equality operand, additive operand, prefix depth, and grouping depth. The grammar does not introduce a separate loop-condition expression category, truthiness rule, pattern condition, or semantic lookahead rule.
...
The ordinary represented value grammar has one bounded additive tier between the existing recursive prefix tier and bounded non-associative equality tier, with bounded grouping admitted as one atom wrapper:
...
EqualityOperator = "==" | "!="
AdditiveValue    = PrefixValue AdditiveSuffix?
AdditiveSuffix   = AdditiveOperator PrefixValue
AdditiveOperator = "+" | "-"
...
The five non-grouping `ValueAtom` alternatives are exactly the pre-existing value producer families admitted at atom position. `GroupedValue` is different: it is a concrete wrapper around exactly one complete existing `Value` and does not become a seventh owned-value producer family, primary-expression semantic category, postfix base, place category, or member-receiver system. The operator producer family remains one existing producer family even though it now contains Boolean negation, plain fixed-width integer addition, plain fixed-width integer subtraction, and Boolean equality/inequality. Validation and execution transparency are owned by `function-execution.md`.

Because the grouped inner form is a complete `Value`, grouping is recursive: `(((a)))` is represented. A group may contain the complete equality or additive tier and therefore supplies explicit tree nesting without changing the unparenthesized tiers. In particular, `!(a == b)` is logical negation whose prefix operand is one grouped equality value; `(a == b) == c` is an outer equality whose left additive atom contains a grouped inner equality; and `a == (b != c)` is an outer equality whose right additive atom contains a grouped inner inequality. For additive operators, `(a + b) - c`, `(a - b) + c`, `a - (b - c)`, and `a + (b - c)` explicitly nest one complete inner additive value through grouping.
...
`BooleanNotValue` maps to the Boolean logical-negation semantic relation in `operators.md`. Its right-recursive `PrefixValue` operand makes repeated negation such as `!!flag` or `! ! flag` unambiguous and right-associated by grammar nesting. Boolean-not recursion does not include `AdditiveValue` or `EqualityValue`; consequently prefix binds more tightly than both additive operations and equality except where grouping explicitly nests a complete `Value`. `!a + b` is one additive value whose left operand is `!a`, `!a - b` is likewise one additive value whose left operand is `!a`, and `!a == b` remains one equality value whose left additive operand contains `!a`. The prefix form never reparses ungrouped `!a + b`, `!a - b`, or `!a == b` as logical negation of the looser operator tier.

`AdditiveValue` maps `+` to the plain fixed-width integer-addition semantic relation and binary `-` to the distinct plain fixed-width integer-subtraction semantic relation in `operators.md`. Exactly zero or one `AdditiveSuffix` is represented at each additive level, so ungrouped `a + b + c`, `a + b - c`, `a - b + c`, `a - b - c`, and longer mixed/repeated additive chains are not represented. Explicitly grouped inner additive values may participate as atoms of an outer additive operation as described above. The surrounding receiving position supplies the exact required type consumed by the selected additive semantics; syntax alone performs no operand type inference, promotion, conversion, defaulting, overload selection, or generic arithmetic dispatch.
...
The equality tier is looser than the additive tier, and the additive tier is looser than the prefix tier. This structural grammar ordering is the only precedence relation introduced; it requires no precedence number/table or generic binary-expression taxonomy. Forms such as `a + b == c` and `a - b == c` therefore each have one unambiguous syntax tree whose left equality operand is the complete additive value. Current operator typing may still reject such a tree: exact-Bool equality requires each complete operand to produce `Bool`, while plain integer addition/subtraction are admitted only under an exact fixed-width integer required type. Concrete representability does not create numeric equality.

In ordinary `Value` positions, a prefix operand may be any represented `PrefixValue`, including a standalone `RecordConstruction` or a `GroupedValue` containing one. That concrete admission does not make a record value Boolean or numeric. `operators.md` requires the logical-negation operand and both equality/inequality operands to be exactly `Bool`, and requires both integer-addition/subtraction operands and result to have the exact surrounding represented fixed-width integer type, while `function-execution.md` owns transactional operand validation and grouping transparency. Consequently forms such as `!Record { ... }`, `!(Record { ... })`, `Record {} == Record {}`, `(Record {}) == (Record {})`, `Record {} + Record {}`, or `Record {} - Record {}` may be syntactically represented in an ordinary value position but are source-invalid under the applicable exact operator typing rules.

The represented literal forms map to `literals.md`. `true` and `false` denote the boolean literal forms owned there. A `DecimalIntegerLiteral` supplies its concrete sign and decimal magnitude to the exact mathematical-integer and required-type materialization relation owned there. A `DecimalFloatingLiteral` supplies its concrete sign and one contiguous decimal floating magnitude token to the exact decimal-rational and required-type floating materialization relation owned there. This grammar does not assign an integer or floating default type, abstract literal type, conversion, or arithmetic semantics beyond the separately owned plain fixed-width integer-addition/subtraction operator relations.

The optional `-` in either represented numeric literal remains part of that literal grammar and retains exactly its accepted literal semantics. Because the sign and following magnitude are distinct grammar tokens, ordinary trivia may occur between them under the general trivia rule above. No trivia may occur inside `DecimalFloatingMagnitude` because it is one token, including around its token-internal decimal point. A value-start `-` can form a value only through one of these signed literal productions; it does not become a general `PrefixValue` operator merely because binary subtraction is now represented after a complete left `PrefixValue`. In particular, `-!flag`, `-(1)`, and `-value` are not represented `Value`s. A spelling such as `!-1` is Boolean logical negation whose atom is the already represented signed decimal integer literal and is source-invalid because that operand cannot have type `Bool`; `(-1)` is represented only as grouping around that already complete signed literal. After a complete left prefix value, `a - -1` and adjacent `a--1` are instead one binary subtraction whose right `PrefixValue` atom is the existing negative literal; neither spelling introduces unary-negation or decrement semantics.
...
- general numeric unary negation, arithmetic beyond bounded plain fixed-width integer addition/subtraction, equality/inequality for any source type other than the represented exact-Bool relations, ordering or other comparison, short-circuit logical, compound-assignment, conversion/cast, or other operator forms beyond represented Boolean negation, plain fixed-width integer addition/subtraction, and Boolean equality/inequality;
- Unit/empty-group, tuple/comma-expression, parenthesized-type, or general expression grammar beyond the bounded one-value contextual grouping forms above; additive/equality/comparison chaining; or a binary precedence/associativity hierarchy beyond the bounded additive and non-associative equality tiers;
