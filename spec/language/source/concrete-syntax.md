...
`fast` is deliberately **not** a reserved identifier key. Outside the exact numeric-contract selected-value position defined below after standalone `@`, an identifier-form token whose lexical identifier key is `fast` remains an ordinary `UserIdentifier` and participates in ordinary lookup exactly like another user identifier. The contextual role does not alter identifier formation, key equality, or any surrounding identifier namespace.
...
The standalone `@` punctuation token participates only in `NumericContractSelectedValue` below. It has no declaration, attribute, annotation, pragma, macro, module, type, pattern, field, call, or other general role. Current accepted source had no represented `@`, so assigning this one bounded role reinterprets no previously source-valid spelling. An identifier-form token following `@` has the contextual selector-key role only when its lexical identifier key is exactly `fast` in the complete selected-value production; this does not reserve `fast` elsewhere and does not create a generic `@name(...)` namespace. No `@standard(...)`, `@reproducible(...)`, or arbitrary `@name(...)` form is represented in this revision.

The existing `(` and `)` punctuation tokens additionally delimit the bounded grouped-value productions and numeric-contract selected-value production below. This adds no new parenthesis punctuation spelling and does not change their existing parameter-list, direct-call, or argument-list roles. The same delimiters do not by themselves define Unit/empty-group values, tuples, parenthesized `Type`, indirect/grouped call targets, a general expression category, or another postfix system.
...
The standalone `+` punctuation token participates only in the bounded additive productions below. Under the exact surrounding required-type relation owned by `operators.md` and `function-execution.md`, the same concrete `+` form maps to plain fixed-width integer addition for an admitted exact integer type or to same-format binary floating addition for exact `F16`, `F32`, or `F64`. An unqualified floating addition obtains `standard` through the accepted numeric-contract default; an applicable `NumericContractSelectedValue` may instead establish `fast` for exactly that floating-addition occurrence. Syntax itself performs no operand-derived inference, overload search, conversion, promotion, defaulting, generic arithmetic dispatch, or numerical contract interpretation. Standalone `+` is not unary plus, part of a numeric literal, compound assignment, increment, or another operator. Consequently `+1` is not a represented leading-plus literal or unary-plus value, `++` tokenizes as two standalone `+` tokens, and `+=` tokenizes as standalone `+` followed by standalone `=`; none is a represented value/operator form under this revision.
...
This revision defines no standalone `>` token, no standalone `&` token, and no other punctuation or operator token.
...
The productions below use quoted text for reserved keys, punctuation, or an explicitly identified contextual identifier key; `?` for an optional element, `*` for zero or more repetitions, and `|` for alternatives. `UserIdentifier` denotes one user identifier as defined above. `DecimalMagnitude` denotes one decimal magnitude token as defined above. `DecimalFloatingMagnitude` denotes one decimal floating magnitude token as defined above. The sole contextual-key use in this revision is quoted `"fast"` inside `NumericContractSelectedValue`; it matches an identifier-form token whose lexical identifier key is exactly `fast` without adding `fast` to the reserved-key set.
...
`ConditionalValue` deliberately has its own logical-conjunction, equality, bitwise-OR, exclusive-or, additive, multiplicative, and prefix tiers rather than reusing unrestricted ordinary `LogicalAndValue`, `EqualityValue`, `OrValue`, `XorValue`, `AdditiveValue`, `MultiplicativeValue`, or `PrefixValue`. This preserves the accepted exclusion of a **standalone** `RecordConstruction` at every conditional conjunction operand, equality operand, bitwise-OR operand, exclusive-or operand, additive operand, multiplicative operand, and recursive prefix depth. It also deliberately does not admit `NumericContractSelectedValue` directly in this first revision: the only currently eligible governed source operation is floating addition, which cannot satisfy the condition's exact `Bool` result requirement. No parallel conditional selector grammar or broader conditional expression category follows.
...
Boolean, decimal integer, and decimal floating literals, represented operator values, and numeric-contract selected values are not producer-backed record-pattern scrutinees. A bare identifier is not admitted through the producer-backed alternative even though `IdentifierUse` is an ordinary `Value` producer elsewhere. No parenthesized/grouped value, operator value, selected-value qualifier, conversion, arbitrary postfix/member expression, qualified bare module member, or other general `Value` form is admitted as a record-pattern scrutinee.
...
This subset has no indirect call, function-value call, method call, named argument, default argument, variadic argument, nested module path, or arbitrary member-call form. In particular, a grouped value or numeric-contract selected value is not a call target: `(f)(x)` and `@fast(f)(x)` are not represented merely because a grouped or selected value can occur in a receiving `Value` position.
...
Because each initializer contains a `Value`, record construction composes recursively with another record construction as well as the other represented value producers, including represented operators, bounded contextual grouping, bounded operation-local numeric-contract selection, and bounded producer-backed field-value use.
...
Because direct-call arguments and construction initializers contain `Value`, bounded producer-backed field-value uses may compose recursively inside those already represented positions, including inside a `GroupedValue` or inside the contained `Value` of `NumericContractSelectedValue`. This recursion does not make grouped or selected values field receivers and does not create a general precedence or postfix hierarchy.
...
The ordinary represented value grammar has one recursive prefix tier above the bounded multiplicative tier, with a bounded additive tier, a bounded exclusive-or tier, a bounded bitwise-OR tier, the non-associative equality tier, one bounded logical-conjunction tier looser than equality, bounded grouping admitted as one atom wrapper, and one bounded operation-local numeric-contract selected-value qualifier admitted at atom position:
...
ValueAtom            = Literal
                     ...
                     | GroupedValue
NumericContractSelectedValue = "@" "fast" "(" Value ")"
GroupedValue           = "(" Value ")"
...
The five producer `ValueAtom` alternatives remain exactly the pre-existing value producer families admitted at atom position: `Literal`, `IdentifierUse`, `DirectCall`, `RecordConstruction`, and `FieldValueUse`. `NumericContractSelectedValue` and `GroupedValue` are different. Grouping is a transparent concrete wrapper around exactly one complete existing `Value`; numeric-contract selection is a concrete qualifier around one complete ordinary `Value` whose directly contained root governed operation is resolved under `operators.md` and `function-execution.md`. Neither wrapper adds a seventh owned-value producer family, primary-expression semantic family, postfix base, place category, field receiver, call target, or pattern-scrutinee category. The operator producer family remains one existing producer family even though it contains Boolean negation, plain fixed-width integer negation, plain fixed-width integer bitwise complement, plain fixed-width integer multiplication, plain fixed-width integer addition, same-format binary floating addition, plain fixed-width integer subtraction, plain fixed-width integer exclusive-or, plain fixed-width integer bitwise OR, Boolean equality/inequality, and Boolean short-circuit conjunction.

`NumericContractSelectedValue` requires the exact token sequence `@`, an identifier-form token whose lexical identifier key is `fast`, `(`, one complete ordinary `Value`, and `)`, modulo permitted trivia. The parentheses delimit the complete contained value; they do not define a block/function/module scope or an ambient numeric mode. The selected-value form is atomic only with respect to the surrounding ordinary value grammar: its contained `Value` still has its complete existing operator/grouping tree, required-type semantics, ownership, calls, faults/divergence, and evaluation ordering.

Concrete syntax alone does not decide whether the contained root is eligible. `operators.md` and `function-execution.md` locate the target after peeling ordinary `GroupedValue` wrappers only, require the root to be the currently governed same-format floating addition, establish `fast` before the existing operand transaction may commit ownership, and reject every other current root. Thus `@fast(a + b)` and `@fast((a + b))` are represented and may be source-valid when the exact surrounding type selects floating addition. `@fast(1.0)`, `@fast(call())`, `@fast(a + b)` under an integer required type, or another non-governed root is represented by grammar but source-invalid by selector applicability. Literal materialization itself remains unchanged.

A `NumericContractSelectedValue` wrapper is not transparent when another selector searches for its root target. Because the contained `Value` recursively admits the same selected-value atom, forms such as `@fast(@fast(a + b))` and `@fast((@fast(a + b)))` are representable syntax, but source-invalid under the stacked-selection rule in `operators.md` and `function-execution.md`. Ordinary grouping alone remains transparent, so `@fast((a + b))` targets the same directly contained floating addition as `@fast(a + b)`. Syntactic recursion therefore does not create selector precedence, merging, override, or nested-scope inheritance.

Only `fast` has a contextual selector spelling in this revision. Because `fast` is not reserved, forms such as `let fast: F32 = ...;` or `fast` as another applicable `UserIdentifier` remain governed by ordinary identifier lookup. `@other(...)`, `@standard(...)`, and `@reproducible(...)` are not represented selected-value forms. The bounded production establishes no generic annotation/attribute/pragmas facility and pre-authorizes no future selector key or governed operation.

Because the grouped inner form is a complete `Value`, grouping is recursive: `(((a)))` is represented. A group may contain the complete logical-conjunction, equality, bitwise-OR, exclusive-or, additive, or multiplicative tier and therefore supplies explicit tree nesting without changing the unparenthesized tiers. In particular, `!(a == b)` is logical negation whose prefix operand is one grouped equality value; `-(a + b)` is integer negation whose prefix operand is one grouped addition value when the surrounding required type selects integer addition; `~(a + b)` is integer complement whose prefix operand is one grouped addition value when the surrounding required type selects integer addition; `(a == b) == c` is an outer equality whose left bitwise-OR operand contains a grouped inner equality; and `a == (b != c)` is an outer equality whose right bitwise-OR operand contains a grouped inner inequality. For additive operators, `(a + b) - c`, `(a - b) + c`, `a - (b - c)`, and `a + (b - c)` explicitly nest one complete inner additive value through grouping; every concrete `+` selects integer addition or floating addition only from its exact required type. For multiplication, `(a * b) * c` and `a * (b * c)` explicitly nest repeated multiplication; `(a + b) * c` and `a * (b + c)` explicitly override the ordinary multiplicative-over-additive tree. For exclusive-or, `(a ^ b) ^ c` and `a ^ (b ^ c)` explicitly nest repeated exclusive-or. For bitwise OR, `(a | b) | c` and `a | (b | c)` explicitly nest repeated bitwise OR. For conjunction, `(a && b) && c` and `a && (b && c)` explicitly nest repeated conjunction that the ungrouped conjunction tier does not admit.
...
`!a * b` is one multiplicative value whose left operand is `!a`; `-a * b` is one multiplicative value whose left operand is `-a`; `~a * b` is one multiplicative value whose left operand is `~a`; `!a + b`, `-a + b`, and `~a + b` are additive values whose left multiplicative operand contains the prefix form; and `!a == b`, `-a == b`, or `~a == b` remain equality values whose left bitwise-OR operand contains an exclusive-or value containing an additive value containing that prefix form. The prefix grammar never reparses those ungrouped forms as a prefix operator around a looser operator tier.
...
`AdditiveValue` maps `+` to either the plain fixed-width integer-addition semantic relation or the same-format binary floating-addition semantic relation in `operators.md`, selected solely by the exact surrounding required type; binary `-` maps only to the distinct plain fixed-width integer-subtraction semantic relation. Its operands are complete `MultiplicativeValue`s. Exactly zero or one `AdditiveSuffix` is represented at each additive level, so ungrouped `a + b + c`, `a + b - c`, `a - b + c`, `a - b - c`, and longer mixed/repeated additive chains are not represented. The multiplicative tier is structurally tighter, so `a + b * c` contains `b * c` as its right additive operand and `a * b + c` contains `a * b` as its left additive operand. Explicit grouping can override either relation. The surrounding receiving position supplies the exact required type consumed by the selected arithmetic semantics; syntax alone performs no operand type inference, promotion, conversion, defaulting, overload selection, trait dispatch, generic arithmetic dispatch, or integer/floating mixed selection. For `+`, exact fixed-width integer `T` selects integer addition, while exact `F16`, `F32`, or `F64` selects floating addition; no other required type is admitted by either addition relation. An unqualified floating occurrence establishes `standard` through the accepted default, while an applicable enclosing `NumericContractSelectedValue` establishes `fast` for exactly its selected root occurrence; nested floating operations remain independently selected/defaulted by the semantic owners.
...
The conjunction tier is looser than the equality tier, the equality tier is looser than the bitwise-OR tier, the bitwise-OR tier is looser than the exclusive-or tier, the exclusive-or tier is looser than the additive tier, the additive tier is looser than the multiplicative tier, and the multiplicative tier is looser than the prefix tier. This structural grammar ordering is the only precedence relation introduced; it requires no precedence number/table or generic binary-expression taxonomy. Forms such as `a + b * c ^ d | e == f && g`, `a + b ^ c | d`, `a ^ b | c`, `a | b == c`, `a == b && c`, and `a && b == c` therefore each have one unambiguous syntax tree. Current operator typing may still reject such a tree: exact-Bool equality and conjunction require their complete operands to produce `Bool`, while plain integer negation/complement/multiplication/addition/subtraction/exclusive-or/bitwise OR and same-format floating addition are admitted only under their exact numeric required types. Concrete representability does not create numeric equality, truthiness, contract applicability, or type-driven parser dispatch.

In ordinary `Value` positions, a prefix operand may be any represented `PrefixValue`, including a standalone `RecordConstruction`, a `GroupedValue` containing one, or a `NumericContractSelectedValue`. That concrete admission does not make a record value Boolean, integer, or floating and does not make an ineligible selected root governed. `operators.md` requires the logical-negation operand, both equality/inequality operands, and both conjunction operands to be exactly `Bool`; requires the integer-negation and integer-complement operand/result and both integer-multiplication/addition/subtraction/exclusive-or/bitwise-OR operands/result to have the exact surrounding represented fixed-width integer type; and requires floating-addition operands/result to have the exact surrounding represented `F16`, `F32`, or `F64` type with one selected numeric contract. `function-execution.md` owns transactional operand validation, selector applicability before transaction commit, eager/short-circuit sequencing, definite state, held-left lifetime, and grouping/selected-value transparency boundaries. Consequently forms such as `!Record { ... }`, `-Record { ... }`, `~Record { ... }`, `!(Record { ... })`, `-(Record { ... })`, `~(Record { ... })`, `Record {} == Record {}`, `(Record {}) == (Record {})`, `Record {} && Record {}`, `Record {} * Record {}`, `Record {} + Record {}`, `Record {} - Record {}`, `Record {} ^ Record {}`, `Record {} | Record {}`, or `@fast(Record {})` may be syntactically represented in an ordinary value position but are source-invalid under the applicable exact operator/selector typing rules.

The represented literal forms map to `literals.md`. `true` and `false` denote the boolean literal forms owned there. A `DecimalIntegerLiteral` supplies its concrete sign and decimal magnitude to the exact mathematical-integer and required-type materialization relation owned there. A `DecimalFloatingLiteral` supplies its concrete sign and one contiguous decimal floating magnitude token to the exact decimal-rational and required-type floating materialization relation owned there. This grammar does not assign an integer or floating default type, abstract literal type, conversion, or arithmetic/bitwise semantics beyond the separately owned plain fixed-width integer-negation/bitwise-complement/multiplication/addition/subtraction/exclusive-or/bitwise-OR and same-format floating-addition operator relations. Numeric-contract selection does not change literal datum or materialization semantics.
...
A `GroupedValue` maps to no distinct semantic value operation. Its one inner `Value` is validated and executed through the existing producer owner; `function-execution.md` defines required-type propagation, ownership, fault/divergence, typed/lowering erasure boundaries, and its transparency when a numeric-contract selector locates a contained root operation. Parentheses do not make a bare qualified module member into an `IdentifierUse` value.

A `NumericContractSelectedValue` likewise produces no independent value. It qualifies the one contained governed producer selected under `operators.md` and `function-execution.md`, passes the surrounding required type through unchanged, and may be erased after successful typed resolution once the selected contract has been retained on that operation. It creates no runtime wrapper, ambient mode, callable-signature fact, cleanup layer, binding, storage, or second evaluation position.
...
This subset has no string, byte, character, aggregate, pointer, or other additional literal form; no scientific-notation, hexadecimal/binary/octal floating form, explicit infinity/NaN literal, `.5`/`1.` floating shorthand, suffix, separator, alternate numeric radix, or leading-plus numeric form; no Unit/tuple/general expression grouping beyond the bounded one-value `GroupedValue` and context-preserving `ConditionalGroupedValue`; no operator beyond bounded Boolean logical negation, Boolean short-circuit conjunction, plain fixed-width integer negation/bitwise complement/multiplication/addition/subtraction/exclusive-or/bitwise-OR, same-format binary floating addition, and exact-Bool equality/inequality; no numeric-contract selector beyond the one bounded operation-local `@fast(Value)` form, no `@standard` or `@reproducible`, no ambient/lexical/dynamic numeric-contract scope, and no generic annotation namespace; no general unary or binary expression hierarchy beyond the bounded prefix/multiplicative/additive/exclusive-or/bitwise-OR/equality/logical-conjunction tiers; no binary bitwise operation beyond represented exclusive-or and bitwise OR and no shift operator; no short-circuit logical operator beyond represented conjunction; no conversion; no arbitrary-receiver member/postfix access beyond the bounded field receiver categories above; no assignment expression; no block expression; no closure; and no other semantic value producer beyond the represented producers above.
...
- floating unary negation, unary plus, increment/decrement, another numeric unary operator beyond bounded plain fixed-width integer negation and integer bitwise complement, arithmetic beyond bounded plain fixed-width integer negation/multiplication/addition/subtraction and same-format binary floating addition, floating subtraction/multiplication/division/remainder or mixed/cross-format arithmetic, binary bitwise operations beyond bounded plain fixed-width integer exclusive-or and bitwise OR, shifts, equality/inequality for any source type other than the represented exact-Bool relations, ordering or other comparison, short-circuit logical operators beyond represented Boolean conjunction, compound-assignment, conversion/cast, numeric-contract selection beyond the bounded operation-local `@fast(Value)` form, explicit source `standard`/`reproducible`, block/function/module/dynamic numeric-contract scoping or inheritance, generic `@` annotations, or other operator forms beyond represented Boolean negation/conjunction, plain fixed-width integer negation/bitwise complement/multiplication/addition/subtraction/exclusive-or/bitwise-OR, same-format floating addition, and Boolean equality/inequality;
- Unit/empty-group, tuple/comma-expression, parenthesized-type, or general expression grammar beyond the bounded one-value contextual grouping forms and bounded selected-value qualifier above; multiplicative/additive/exclusive-or/bitwise-OR/equality/logical-conjunction/comparison chaining; or a binary precedence/associativity hierarchy beyond the bounded multiplicative, additive, exclusive-or, bitwise-OR, non-associative equality, and bounded logical-conjunction tiers;
...
- record-pattern scrutinees beyond the represented bare direct binding root and dedicated `DirectCall`, `RecordConstruction`, and bounded `FieldValueUse` producer-backed forms; in particular no literal (including decimal floating), bare `IdentifierUse`-as-value, represented operator value, grouping, numeric-contract selected value, other operator expression, conversion, arbitrary postfix/member expression, or other general expression is admitted there;
