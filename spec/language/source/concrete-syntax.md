...
The existing `(` and `)` punctuation tokens additionally delimit the bounded grouped-value and numeric-contract-selected-value productions below and the statement-local scrutinee delimiter in bounded refutable record selection. This adds no new delimiter spelling and does not change their existing parameter-list, direct-call, argument-list, or value-wrapper roles. In the refutable selector, the parentheses contain exactly one `RecordPatternScrutinee`; they do not select `GroupedValue`, do not widen that scrutinee to `Value`, and add no grouping/evaluation/ownership semantics. The same delimiters do not by themselves define Unit/empty-group values, tuples, parenthesized `Type`, indirect/grouped call targets, a general expression category, or another postfix system.
...
The standalone `&` punctuation token participates only in the bounded safe-reference type/root/reborrow forms and as the address-selection punctuation inside the bounded `RawAddressOfValue` form `raw &x`. The Shared root branch admits one unqualified root identifier followed by zero or more existing `FieldSelector`s, so `&x` and `&x.field...` are bounded Shared root-reference forms. The Shared reborrow branch admits `*`, one unqualified parent safe-reference identifier, and zero or more existing `FieldSelector`s, so `&*r` is the zero-selector complete-referent child and `&*r.field...` is bounded Shared field-relative child reborrow. The reserved `mut` key following `&` selects the replacement-capable forms `&mut T`, `&mut x`/`&mut x.field...`, or `&mut *r`/`&mut *r.field...` only in the exact grammar positions below; `&mut` is not a distinct punctuation token. Both replacement-capable target branches admit zero or more existing `FieldSelector`s after their one bare identifier: the root branch resolves from the selected binding root, while the reborrow branch resolves relative to the selected parent reference's referent. Zero selectors after `&mut *r` preserve the existing complete-referent child; one or more selectors select the bounded projected replacement-capable child. The raw-address role is available only after the reserved `raw` key in its exact production and remains complete-root-only. `&` does not define binary bitwise AND, a general address-of operator, reference pattern/binding modes, lifetime syntax, an arbitrary borrow/address operand, plain-Exclusive syntax, or another prefix/infix operation. The distinct longest-match `&&` token remains Boolean short-circuit conjunction and is never decomposed into two root-borrow/reborrow/address tokens.
...
The standalone `.` punctuation token participates only as the existing `FieldSelector` in six bounded contexts: represented `FieldValueUse`, the bounded `FieldAssignmentTarget`, the Shared root target after `&` in `&x.field...`, the replacement-capable root target after `&mut` in `&mut x.field...`, the Shared reborrow target after `&*r` in `&*r.field...`, and the replacement-capable reborrow target after `&mut *r` in `&mut *r.field...`. The decimal point inside one `DecimalFloatingMagnitude` is consumed as interior material of that single decimal token and is therefore not a `.` punctuation token. Adjacent `..` is the distinct longest-match punctuation token described next and is never two standalone field selectors. Reusing `FieldSelector` across these six bounded contexts does not make a field-value use a place, make a safe-reference value a field-value receiver, create a general postfix/member system, or create assignability outside the exact `FieldAssignmentTarget` grammar.

The `..` punctuation token participates only as `RecordPatternRest` in the bounded recursive record-pattern grammars below. It is node-local rest/omission syntax, not a field selector, range, spread/update, constructor-rest form, arbitrary wildcard, value, type, operator, module path, or general punctuation facility. Before this revision adjacent `..` tokenized as two standalone `.` punctuation tokens, but no represented grammar admitted two adjacent field selectors or another source-valid `..` spelling, so longest-match recognition of `..` reinterprets no previously source-valid source. Decimal floating tokenization remains unchanged: when token processing begins at a digit, the decimal-floating selection rule below consumes a qualifying digit-run `.` digit-run as one numeric token before ordinary punctuation tokenization can see that decimal point.

The standalone `=` punctuation token retains only its represented declaration/assignment roles, including the separator in bounded refutable record selection and the separators in bounded `ReferenceReplaceStatement` and `RawAssignStatement`. The longest-match `==` punctuation token has exactly two bounded grammar roles in this revision. In ordinary and conditional comparison productions it maps to the equality semantic-selection relation in `operators.md`. Inside one `RefutableRecordBindTest`, after the target's leading success-binding `UserIdentifier`, it instead selects the existing pattern-local equality-test relation owned by `patterns.md`; the identifier is a binding declaration rather than the left operand of a `ComparisonValue`. Grammar position and complete token shape distinguish the roles without type-driven parsing. This pattern-local role does not create a general pattern comparison, general `Value`, binding initializer, assignment, alias, type-ascription, guard, or predicate grammar.

The standalone `!` punctuation token participates only in the represented Boolean logical-negation prefix productions below. `!=` instead participates only in the bounded comparison productions and maps those forms to the inequality semantic-selection relation in `operators.md`. None of these punctuation tokens reserves an identifier key.

The standalone `<` punctuation token has exactly three bounded grammar roles in this revision. In the ordinary and conditional comparison productions below it maps to the plain fixed-width integer strict-less-than semantic-selection relation in `operators.md`. After the `:` of one `RefutableRecordPatternField`, it may begin the exact test-only `RefutableRecordStrictUpperBoundTest` production `"<" DecimalIntegerLiteral`; that pattern-local form maps to the strict-upper-bound relation owned by `patterns.md`. Inside one `RefutableRecordBindTest`, after the target's leading success-binding `UserIdentifier`, `<` selects the same existing strict-upper-bound relation over the selected field value with the following `DecimalIntegerLiteral` as bound. Neither pattern-local role is a `ComparisonValue`, general `Value`, producer operand, or range endpoint grammar; the binding identifier in the composite role is not the semantic left comparison operand. Grammar position and complete token shape distinguish all three roles without type-driven parsing. `<=` tokenizes as standalone `<` followed by standalone `=` and is not a represented operator or pattern-test form. These roles define no `>`, `>=`, three-way comparison, lower-bound or two-sided range syntax, generic ordering, type-parameter syntax, markup, or other punctuation meaning.

The standalone `~` punctuation token participates only in the represented plain fixed-width integer-bitwise-complement prefix productions below and maps that form to the complement semantic relation in `operators.md`. It is not a reserved identifier key, binary bitwise operator, shift, pointer/reference form, destructor marker, pattern marker, type form, or another prefix/postfix operation. Because `~` was not a represented punctuation token before this revision, adding it reinterprets no previously source-valid spelling. `~~value` is two standalone `~` tokens and is represented as nested right-recursive complement; `~=` tokenizes as standalone `~` followed by standalone `=` and is not a represented operator form.
...
Trivia MAY occur around and between the tokens shown by these productions. Line boundaries have no statement-termination role. Semicolons are required exactly where a grammar production includes `;`; a represented `BlockStatement`, `UnsafeBlockStatement`, `IfStatement`, `RefutableRecordSelectionStatement`, or `WhileStatement` terminates at its final closing `}` and has no trailing semicolon.
...
The represented body grammar delimits the function root lexical scope and admits recursively nested child lexical scopes through `BlockStatement`, `UnsafeBlockStatement`, represented conditional arms, bounded refutable-selection arms, and represented `while` bodies:
...
A terminal return inside a nested ordinary or unsafe block terminates the current source function activation under `function-execution.md`; it does not merely exit that block. A `fault;` reached inside either likewise terminates the current activation abnormally through the defined-fault relation. A source-valid `break;` or `continue;` reached inside a represented loop exits the active child lexical scopes required by its nearest enclosing `while` target under `function-execution.md` and `control-flow.md`, including any intervening unsafe block, Boolean-conditional arm, or bounded refutable-selection arm scope. The block forms themselves still do not create a block expression, tail value, Unit/Void value, label, or catch form. Boolean conditional selection is introduced only by `IfStatement`, bounded refutable selection only by `RefutableRecordSelectionStatement`, bounded-loop selection only by `WhileStatement`, loop transfer only by the explicit statements defined below, and unsafe admission only by `UnsafeBlockStatement`.

Each `BlockStatement` maps to exactly one child lexical scope under `local-bindings.md`. `UnsafeBlockStatement` adds the lexical unsafe-admission fact from `raw-pointers-unsafe.md` to that same contained child scope; it does not create a second hidden scope. Execution order, normal-continuation presence, normal child-scope cleanup, loop-transfer cleanup, return cleanup, explicit-fault/defined-fault cleanup and propagation, whole-binding and bounded field-assignment ordering, complete-referent replacement ordering, raw-operation ordering, safe-reference carrier/authority consequences, external-referent restoration, and divergence consequences are owned by `function-execution.md` and `references.md`. When a block is a conditional arm, bounded refutable-selection arm, or represented `while` body, `control-flow.md` owns its relationship to the applicable selection, successor, backedge, and loop-transfer target/state rules.

## Conditional statements

The represented statement-level Boolean conditional has this grammar:
...
ConditionalLogicalAndValue =
    ConditionalComparisonValue ConditionalLogicalAndSuffix?
ConditionalLogicalAndSuffix = "&&" ConditionalComparisonValue
ConditionalComparisonValue =
    ConditionalOrValue (ComparisonOperator ConditionalOrValue)?
ConditionalOrValue =
...
The separate bounded `if let` statement defined below does not add `let` or record-pattern syntax to `ConditionalValue`. Because `let` is a reserved key and is not a valid start of any `ConditionalValue`, the token following initial `if` distinguishes `RefutableRecordSelectionStatement` from `IfStatement` without type lookup or widening the existing conditional grammar.
...
Exactly zero or one ungrouped bitwise-OR operator is represented at each conditional bitwise-OR level. Ungrouped `a | b | c` and longer bitwise-OR chains are therefore not represented. Existing context-preserving grouping can express `(a | b) | c` or `a | (b | c)` without introducing bitwise-OR associativity. Exclusive-or is structurally tighter than bitwise OR, while comparison is structurally looser: `a ^ b | c` has `a ^ b` as its left bitwise-OR operand, and `a | b < c` has the complete bitwise OR as its left comparison operand.

Exactly zero or one ungrouped comparison operator is represented at each conditional comparison level. Ungrouped `a < b < c`, `a < b == c`, `a == b < c`, `a != b < c`, and all other repeated or mixed comparison chains are therefore not represented. Existing context-preserving grouping can express `(a < b) == flag`, `flag == (a < b)`, `(a == b) == c`, or `a == (b != c)` without introducing comparison associativity or chaining. Bitwise OR is structurally tighter than comparison, while conjunction is structurally looser.

Exactly zero or one ungrouped conjunction operator is represented at each conditional conjunction level. Ungrouped `a && b && c` and longer conjunction chains are therefore not represented. Existing context-preserving grouping can express `(a && b) && c` or `a && (b && c)` without introducing conjunction associativity. Comparison is structurally tighter than conjunction: `a < b && ready` has the complete strict comparison as its left conjunction operand, while `ready && a < b` has the complete strict comparison as its right conjunction operand.

Decimal integer and decimal floating literals remain syntactically represented as conditional atoms. Exact condition and operator typing are owned by `control-flow.md`, `operators.md`, and `function-execution.md`. A bare numeric literal remains syntax-valid as a `ConditionalValue` but is source-invalid because it cannot produce the exact intrinsic `Bool` required by control flow. The same is true of a plain integer-negation, integer-complement, integer-multiplication, floating-multiplication, floating-division, integer-addition, floating-addition, integer-subtraction, floating-subtraction, integer-exclusive-or, or integer-bitwise-OR value used directly as a condition: the condition supplies required type `Bool`, while `operators.md` admits those numeric operations only under their exact integer or floating required-type domains, so the operation is rejected before its operand or operands may commit ownership. Comparisons are different because their results are intrinsically `Bool`: `if x == 1 { ... }` and `if x < 1 { ... }` can be source-valid when static exact evidence from `x` selects an admitted integer operand type `T` and the literal materializes under that `T`; the symmetric literal-on-left forms behave identically. `1 == 1` and `1 < 1` remain source-invalid because both top-level operands are contextual and the comparison is unanchored, not because the condition supplies a Bool operand requirement. Likewise, a contextual top-level numeric producer such as `(x + 1)` may be validated under `T` selected by an exact opposite comparison operand, while `(x + 1) < (y + 2)` remains unanchored even when nested leaves have declared types; comparison evidence does not recursively mine a contextual producer's children. Boolean conjunction continues to require exact Bool operands. A Boolean-negated numeric atom such as `!-1` therefore remains source-invalid, while integer comparison itself supplies no numeric truthiness rule.
...
An `IfStatement` produces no source value and has no trailing semicolon. It does not add a conditional expression, block value, Unit/Void value, pattern condition, guard, truthiness relation, ordering comparison beyond represented fixed-width integer strict `<`, or short-circuit logical operator beyond the represented conjunction producer. The separate `RefutableRecordSelectionStatement` is a statement category rather than a pattern condition inside this `ConditionalValue`. Fixed-width integer equality/inequality/strict-less-than, when source-valid under the ordinary comparison relation, are already Bool-producing operator values rather than a condition-specific numeric rule.

Because `ReturnStatement` is an optional terminal element of `BlockStatement`, a conditional arm may return from the current function. Return remains absent from `BodyStatement`, so this grammar still does not admit an arbitrary nonterminal return followed by another statement in that same arm block. Because `FaultStatement`, `BreakStatement`, and `ContinueStatement` are `BodyStatement` forms, the grammar may represent a following sibling after them; the no-local-normal-continuation rule rejects that sibling semantically.
...
`WhileStatement` reuses the exact `ConditionalValue` nonterminal above. It therefore admits the same bounded Boolean short-circuit-conjunction, comparison, bitwise-OR, exclusive-or, multiplicative, and additive tiers, recursively prefixed Boolean logical-negation, integer-negation, and integer-complement forms, contextual grouped values, and the same literal, identifier-use, direct-call, and bounded field-value atoms while preserving the same standalone-`RecordConstruction`, direct-`NumericContractSelectedValue`, direct-safe-reference, direct-`RawAddressOfValue`, and direct-`RawMoveValue` exclusions at every conditional conjunction operand, comparison operand, bitwise-OR operand, exclusive-or operand, additive operand, multiplicative operand, prefix depth, and grouping depth. The grammar does not introduce a separate loop-condition expression category, truthiness rule, pattern condition, `while let` form, or semantic lookahead rule.

The loop body is exactly one ordinary `BlockStatement` and therefore one child lexical scope under `local-bindings.md`. `WhileStatement` is itself one `BodyStatement`, so loops may nest recursively and may appear inside represented conditional arms, bounded refutable-selection arms, unsafe blocks, or other blocks. The closing body `}` terminates the complete `WhileStatement`; no trailing semicolon is present.
...
An inner represented `while` is the nearest target for transfers lexically inside its body. Ordinary blocks, unsafe blocks, Boolean-conditional arms, and bounded refutable-selection arms do not establish transfer targets.
...
## Recursive irrefutable record destructuring with bounded rest
...
This irrefutable declaration grammar defines no `let mut Record { ... }`, shorthand field pattern, wildcard/ignore binding, tuple/array/enum pattern, literal/alternative/guard target, qualified binding leaf, qualified field name, nested module path beyond the represented alias/member pair, destructuring assignment, reference-binding mode, raw-pointer-binding pattern mode, mutable pattern-binding modifier, range pattern, constructor spread/update/rest, or general spread syntax. In particular, Boolean/decimal-integer equality literal-test leaves, fixed-width-integer strict-upper-bound test leaves, and the bounded same-path bind-and-test targets below belong only to the separate refutable grammar and do not become `RecordPatternTarget` alternatives.

## Bounded single-success refutable record selection

The represented refutable-selection statement has exactly this grammar:

```text
RefutableRecordSelectionStatement =
    "if" "let" RefutableRecordPattern "=" "(" RecordPatternScrutinee ")"
    BlockStatement ("else" BlockStatement)?
RefutableRecordPattern =
    RecordPatternHead "{" RefutableRecordPatternEntries? "}"
RefutableRecordPatternEntries =
    RecordPatternRest ","?
  | RefutableRecordPatternField ("," RefutableRecordPatternField)* ("," RecordPatternRest)? ","?
RefutableRecordPatternField =
    UserIdentifier ":" RefutableRecordPatternTarget
RefutableRecordPatternTarget =
    UserIdentifier
  | RefutableRecordPattern
  | RefutableRecordLiteralTest
  | RefutableRecordStrictUpperBoundTest
  | RefutableRecordBindTest
RefutableRecordLiteralTest =
    BooleanLiteral | DecimalIntegerLiteral
RefutableRecordStrictUpperBoundTest =
    "<" DecimalIntegerLiteral
RefutableRecordBindTest =
    UserIdentifier "==" RefutableRecordLiteralTest
  | UserIdentifier "<" DecimalIntegerLiteral
```

This grammar reuses the existing `RecordPatternHead`, `RecordPatternRest`, and exact `RecordPatternScrutinee` nonterminals but does not reuse or widen `RecordPatternTarget`. The required parentheses delimit exactly that existing scrutinee category and are not `GroupedValue` syntax. Consequently ordinary `let RecordPattern = ...;` remains unable to contain any bounded refutable-test or same-path bind-and-test target, while the refutable selector gains no general-expression scrutinee.

After initial reserved key `if`, reserved key `let` selects this statement before any `ConditionalValue` parse. Since `let` is not a `ConditionalValue` start, no type or semantic lookup is required to distinguish this form from `IfStatement`, and the existing `if flag { ... }` / record-construction ambiguity boundary remains unchanged. The later mandatory `(` / `)` independently makes the complete scrutinee/success-block boundary token-unambiguous: `=(root) { ... }` is direct-root plus success block, while `=(R { ... }) { ... }` contains a complete record-construction scrutinee. No new reserved key or punctuation token is introduced.

Every `RefutableRecordPattern` begins with the same unqualified or two-part qualified nominal record head as the irrefutable pattern. Explicit field/rest token shape is likewise the same, including node-local optional final `..` and optional trailing comma. `patterns.md` owns exact nominal head/type equality, explicit field identity/uniqueness/accessibility, no-rest exhaustiveness, rest-authorized omission, and recursive source ordering for this separate semantic family.

A `RefutableRecordPatternTarget` is classified entirely from token shape:

- `true` or `false` selects a Boolean equality-only literal-test leaf;
- `DecimalMagnitude`, or standalone `-` followed across permitted trivia by `DecimalMagnitude`, selects a decimal-integer equality-only literal-test leaf under the existing signed integer-literal grammar;
- standalone `<` followed by `DecimalMagnitude`, or by standalone `-` followed across permitted trivia by `DecimalMagnitude`, selects a fixed-width-integer strict-upper-bound-only test leaf whose bound is the complete existing `DecimalIntegerLiteral` beginning after `<`;
- `UserIdentifier "=="` followed by `true`, `false`, `DecimalMagnitude`, or standalone `-` followed across permitted trivia by `DecimalMagnitude` selects one same-path equality bind-and-test target; the leading identifier is the success-binding declaration and the following literal is the existing complete `RefutableRecordLiteralTest`;
- `UserIdentifier "<"` followed by `DecimalMagnitude`, or by standalone `-` followed across permitted trivia by `DecimalMagnitude`, selects one same-path strict-upper-bound bind-and-test target; the leading identifier is the success-binding declaration and the following integer is the existing complete bound;
- a bare `UserIdentifier` with no nested-pattern or bind-and-test continuation selects an ordinary binding leaf;
- `UserIdentifier "{"` begins an unqualified nested refutable record pattern; and
- `UserIdentifier "::" UserIdentifier "{"` begins a qualified nested refutable record pattern.

The leading `UserIdentifier` of `RefutableRecordBindTest` is never parsed as an ordinary `IdentifierUse` or as a left `ComparisonValue` operand. It declares the success binding owned by `patterns.md`; the selected record field value remains the semantic test operand. The following `==` or `<` selects exactly one already represented pattern-local test relation. This bounded composition therefore creates no general comparison/test `Value`, binding initializer, assignment/ascription, alias, guard, predicate expression, or arbitrary bind-and-test grammar.

`DecimalFloatingLiteral` is neither a `RefutableRecordLiteralTest` nor a strict-upper-bound bound and therefore cannot follow either composite relation token. Neither a positive nor signed decimal floating spelling is admitted as a refutable test leaf or same-path bind-and-test test. `RefutableRecordStrictUpperBoundTest` and the `<` branch of `RefutableRecordBindTest` contain only standalone `<` followed by one complete `DecimalIntegerLiteral`; neither is an embedded `ComparisonValue`, general `Value`, producer operand, lower-bound test, or range endpoint form. The `==` branch likewise accepts only the exact existing `RefutableRecordLiteralTest`, not a general right comparison operand. No grouping, other operator value, safe-reference/raw-pointer value, wildcard, range, guard, alternative, shorthand, reference/mutable binding mode, non-duplicable composite binding mode, nested-record alias, multiple-test target, or wider bind-and-test target is introduced. The pattern remains nominal-record-shaped at its top; no top-level scalar literal pattern is represented.

Consequently nearby spellings such as `binding <= 1`, `binding > 1`, `binding >= 1`, `binding == 1.0`, `binding == other`, `binding < other`, `binding < 1.0`, `binding == (1)`, `binding < (1)`, `binding == 1 < 2`, or `binding < 1 == 1` are not represented composite targets. `<=` remains standalone `<` followed by standalone `=`; `>` remains unrepresented punctuation. Trivia may separate the displayed tokens exactly under the general trivia rule, but it does not widen any target nonterminal.

The grammar can syntactically represent a refutable pattern tree whose targets are all ordinary bindings/nested nodes and contain zero refutable tests. `patterns.md` rejects that semantic case because this statement requires at least one refutable test, satisfied by an equality-only target, a strict-upper-bound-only target, or the test component of one same-path bind-and-test target; concrete grammar does not duplicate that recursive semantic count merely to distinguish an otherwise well-formed tree.

Inside the mandatory statement-local parentheses, `RecordPatternScrutinee` is reused unchanged. The statement therefore admits exactly the same direct bare binding root or producer-backed result-bearing `DirectCall`, `RecordConstruction`, or bounded `FieldValueUse`, and no grouping/general expression, standalone literal, operator, safe-reference/raw-pointer producer, or other scrutinee category. A producer-backed field-value scrutinee still completes its field-receiver lifecycle before the resulting record enters the distinct pattern transient relation. The delimiter itself performs no evaluation and creates no semantic wrapper.

The success arm is exactly one `BlockStatement`. `else` is optional and, when present, is followed by exactly one mismatch `BlockStatement`. Each explicit arm is therefore an ordinary child lexical scope. Success-only bindings, including same-path composite bindings, are established and scoped according to `patterns.md` and `control-flow.md`; they are not introduced into the mismatch block and do not participate in lookup while their own or any other refutable test executes. Omitted mismatch creates no concrete block or synthetic lexical scope.

`RefutableRecordSelectionStatement` is statement-only, produces no source value or Unit/Void value, and has no trailing semicolon. Its pattern/static test-and-binding validation, scrutinee exact-type/evaluation relation, two-phase refutable-test-before-binding execution, match/mismatch transient cleanup, success binding establishment, abnormal completion, omitted mismatch, exact normal-successor state composition, and source-to-Core refinement boundaries are owned by `patterns.md` and `control-flow.md` while consuming the existing producer/cleanup/equality/strict-less-than primitives from their canonical owners.

This bounded form defines no `while let`, `let else`, standalone refutable declaration, multi-arm `match`/`case`, variants/enums, exhaustiveness over alternatives, guard, two-sided or one-sided range pattern, lower-bound test, `<=`, `>`, or `>=` pattern relation, pattern alternative, floating literal/ordering test, scalar top pattern, wildcard/shorthand/ref/mutable binding mode, non-duplicable or nested-record bind-and-test mode, multiple tests on one field, destructuring assignment, general expression scrutinee, general comparison/test `Value`, pattern coercion/default numeric type, or new equality/ordering/predicate operation.
...
The complete RHS is one ordinary `Value` whose required type is exact referent type `T`. `references.md` and `function-execution.md` own source-first evaluation, the required live/full replacement-capable destination-authority admission before RHS producer consequences may commit, post-RHS revalidation after normal RHS success, then-current applicable referent-frontier cleanup, exact-`T` installation, and either complete-root reset or canonical bounded non-empty subpath installation according to the stored reference's exact target domain. If the RHS faults or diverges, no outer referent replacement occurs.

`ReferenceReplaceStatement` is statement-only and requires its trailing semicolon. It does not consume the destination reference carrier, define a value result, expose a stale snapshotted destination when the RHS disables that carrier, admit Shared or plain-Exclusive replacement, admit a field selector after the reference operand, or establish a general dereference-place/lvalue grammar. A replacement-capable root or projected child may itself denote one previously selected structural region; the statement still replaces that reference's complete referent and does not add `*r.field... = Value;` syntax.
...
- a bare `UserIdentifier` remains `IdentifierUse` in an ordinary `Value` position and remains the distinct direct binding-root scrutinee in a `RecordDestructuringDeclaration` or `RefutableRecordSelectionStatement`;
...
The same `.` selector spelling represents both duplicate and consume outcomes for field-value production. The `FieldSelector` nonterminal is also reused after the root identifier in the bounded `FieldAssignmentTarget`, after the root identifier in the bounded Shared root-reference target, after the root identifier in the bounded replacement-capable root-reference target, and after the parent safe-reference identifier in bounded Shared or replacement-capable field-relative reborrow. Those reuses select an assignment target or reference target under their corresponding owners rather than producing field values under this section.

The `.` punctuation token in these bounded selector roles has no decimal-literal, method, direct reference-relative value-access, raw-pointer-relative, general postfix/member, or other operator meaning. Its assignment-target role exists only through `FieldAssignmentTarget` and does not make `FieldValueUse` or another completed value generally assignable. The decimal point inside a `DecimalFloatingMagnitude` is instead part of that one decimal token and never reaches this punctuation production. The distinct longest-match `..` punctuation token never enters `FieldSelector` and has only its bounded record-pattern roles above.

## Value forms

The ordinary represented value grammar has one recursive prefix tier above the bounded multiplicative tier, with a bounded additive tier, a bounded exclusive-or tier, a bounded bitwise-OR tier, one non-associative comparison tier, one bounded logical-conjunction tier looser than comparison, and bounded atom wrappers for contextual grouping and operation-local numeric-contract selection. Shared complete-root/binding-field root formation, replacement-capable complete-root/binding-field root formation, and Shared/replacement-capable complete/field-relative reborrow are bounded atom producers; complete-referent dereference is one bounded nonrecursive prefix producer. Raw address formation and raw ownership move are bounded atom producers introduced by reserved key `raw`.

```text
Value                = LogicalAndValue
LogicalAndValue      = ComparisonValue LogicalAndSuffix?
LogicalAndSuffix     = "&&" ComparisonValue
ComparisonValue      = OrValue ComparisonSuffix?
ComparisonSuffix     = ComparisonOperator OrValue
ComparisonOperator   = "==" | "!=" | "<"
...
ReplacementReferenceReborrowTarget    = "*" UserIdentifier FieldSelector*
ReplacementReferenceRootTarget        = UserIdentifier FieldSelector*
...
`SafeReferenceValue` is classified entirely from concrete token shape after the leading standalone `&`: after the reserved `mut` key, `*` plus one bare `UserIdentifier` and zero or more `FieldSelector`s selects the replacement-capable reborrow branch, while a `UserIdentifier` plus zero or more `FieldSelector`s selects the replacement-capable root branch. Otherwise `*` followed by one `UserIdentifier` and zero or more `FieldSelector`s takes the Shared reborrow branch, while a `UserIdentifier` followed by zero or more `FieldSelector`s takes the Shared root branch. Thus `&*r` is the zero-selector Shared reborrow, `&*r.field...` is bounded Shared field-relative reborrow, `&x` is the zero-selector Shared root, `&x.field...` is the bounded Shared field-root form, `&mut *r` is the zero-selector complete-referent replacement-capable reborrow, `&mut *r.field...` is bounded replacement-capable field-relative reborrow, `&mut x` is the zero-selector replacement-capable root, and `&mut x.field...` is the bounded replacement-capable field-root form. The grammar remains disjoint because `mut` is reserved and unavailable as `UserIdentifier`; within either post-`mut` branch, leading `*` versus a bare `UserIdentifier` selects reborrow versus root before any selector sequence. No generic postfix or field-value receiver rule continues a completed safe-reference value, and no semantic type lookup is used to choose the syntactic branch.
...
The non-wrapper `ValueAtom` alternatives consist of the pre-existing literal/identifier/call/construction/field-value producers, the bounded `SafeReferenceValue` family with Shared complete-root/field-root formation, replacement-capable complete-root/field-root formation, and Shared/replacement-capable complete/field-relative reborrow, plus the bounded raw-address/raw-move producers. `GroupedValue` and `NumericContractSelectedValue` are wrappers around one complete existing `Value`; neither becomes another owned-value producer family, primary-expression semantic category, postfix base, place category, member-receiver system, or call target. `SafeDereferenceValue` is a separate bounded prefix producer rather than an atom wrapper. Grouping is semantically transparent. Numeric-contract selection qualifies one existing governed root operator according to `function-execution.md` and may be erased after that selected contract is retained on the operation.
...
- `&mut x` or `&mut x.field...` requests fresh replacement-capable root formation. The first operand is one bare unqualified `UserIdentifier`, followed by zero or more existing `FieldSelector`s. Zero selectors preserves the existing complete-root case; one or more selectors supply the exact bounded structural field path resolved and accessibility-checked under `references.md` and `field-access.md`. Semantic validity additionally requires the root to be one mutable ordinary local, the final selected type to be replacement-reference-referent-admissible, the exact selected path to be fully available, and canonical Exclusive compatibility at that path. Parameters and immutable/non-replaceable roots are rejected semantically. The surrounding required source type supplies exact `ExclusiveReplaceRef(T)` for the final selected root/field type. No field-level mutability property or general place target is introduced.
...
- `&mut *r` or `&mut *r.field...` requests a fresh replacement-capable child of the replacement-capable parent safe-reference binding named by `r`. Zero selectors preserves the existing complete-referent child; one or more selectors supply the exact bounded relative structural field path resolved and accessibility-checked from the parent's referent type. The operation leaves the parent carrier in place, creates one fresh direct child authority/provenance targeting exactly the selected region with `ExclusiveReplace` permission, and is valid only when the parent retains replacement-capable Exclusive authority over that selected target, the target is fully available, the final selected type is replacement-reference-referent-admissible, and the surrounding required source type is exact `ExclusiveReplaceRef(T)`.

No safe-reference form permits permission strengthening, implicit reborrow, plain-Exclusive child, arbitrary dereference operand, qualified target, producer/transient target, or a general address/borrow expression. Bounded Shared and replacement-capable field-root formations remain independent root formations, while bounded `&*r.field...` and `&mut *r.field...` are explicitly fresh child/reborrow forms relative to an existing parent authority.

`SafeDereferenceValue` maps exactly to bounded complete-referent dereference in `references.md`. Its operand is one bare unqualified `UserIdentifier` naming an active safe-reference parameter/local binding. For exact `SharedRef(T)` it retains Shared duplicate behavior. For exact `ExclusiveReplaceRef(T)`, `references.md` selects duplicate behavior when `T` is duplicable and complete-referent ownership Move when `T` is non-duplicable. The complete referent is the exact structural region selected by the stored reference, including a projected binding-field root or projected child when applicable. The grammar is deliberately nonrecursive: `**r` and `*(r)` are not represented, while `(*r)` is ordinary grouping around one already complete `SafeDereferenceValue`. No direct reference-relative field selector or general dereference-expression category follows from this form.
...
Because the grouped inner form is a complete `Value`, grouping is recursive: `(((a)))` is represented. A group may contain the complete logical-conjunction, comparison, bitwise-OR, exclusive-or, additive, or multiplicative tier and therefore supplies explicit tree nesting without changing the unparenthesized tiers. In particular, `!(a == b)` is logical negation whose prefix operand is one grouped comparison value; `-(a + b)` is integer negation whose prefix operand is one grouped addition value when the surrounding required type selects integer addition; `~(a + b)` is integer complement whose prefix operand is one grouped addition value when the surrounding required type selects integer addition; `(a < b) == flag` is an outer equality whose left bitwise-OR operand contains a grouped inner strict comparison; and `a == (b != c)` is an outer equality whose right bitwise-OR operand contains a grouped inner inequality. For additive operators, `(a + b) - c`, `(a - b) + c`, `a - (b - c)`, and `a + (b - c)` explicitly nest one complete inner additive value through grouping; every concrete `+` or binary `-` selects its plain fixed-width integer or same-format floating semantic operation only from its exact required type. For multiplicative operators, `(a * b) * c`, `a * (b / c)`, `(a / b) * c`, and `a / (b / c)` explicitly nest repeated or mixed multiplicative structure; each concrete `*` independently selects plain fixed-width integer multiplication or same-format floating multiplication only from its exact required type, while each concrete `/` maps only to same-format floating division under exact floating required type. `(a + b) * c`, `a * (b + c)`, `(a + b) / c`, and `a / (b + c)` explicitly override the ordinary multiplicative-over-additive tree. For exclusive-or, `(a ^ b) ^ c` and `a ^ (b ^ c)` explicitly nest repeated exclusive-or. For bitwise OR, `(a | b) | c` and `a | (b | c)` explicitly nest repeated bitwise OR. For comparison, grouping alone may nest complete comparisons such as `(a < b) == flag` or `flag == (a < b)`; ungrouped comparison chains remain excluded. For conjunction, `(a && b) && c` and `a && (b && c)` explicitly nest repeated conjunction that the ungrouped conjunction tier does not admit. Because safe-reference and raw forms are ordinary bounded producers, `(&x)`, `(&x.field)`, `(&mut x)`, `(&mut x.field)`, `(&*r)`, `(&*r.field)`, `(&mut *r)`, `(&mut *r.field)`, `(*r)`, `(raw &x)`, and `(raw move p)` are ordinary grouping around one complete producer; grouping changes neither their target syntax nor their authority/unsafe admission and does not make the grouped form a field-value receiver or reference target.
...
`!a * b` is one multiplicative value whose left operand is `!a`; `-a / b` is one multiplicative value whose left operand is `-a`; `~a * b` is one multiplicative value whose left operand is `~a`; `*r * b` is one multiplicative value whose left operand is bounded safe-reference dereference `*r`; `!a + b`, `-a + b`, and `~a + b` are additive values whose left multiplicative operand contains the prefix form; and `!a < b`, `-a == b`, or `~a != b` are comparison values whose left bitwise-OR operand contains an exclusive-or value containing an additive value containing that prefix form. The prefix grammar never reparses those ungrouped forms as a prefix operator around a looser operator tier.
...
`OrValue` maps `|` to the plain fixed-width integer-bitwise-OR semantic relation in `operators.md`. Its operands are complete `XorValue`s. Exactly zero or one `OrSuffix` is represented at each bitwise-OR level, so ungrouped `a | b | c` and longer bitwise-OR chains are not represented. Explicit grouping can represent `(a | b) | c` or `a | (b | c)` without introducing associativity. Exclusive-or is structurally tighter than bitwise OR: `a ^ b | c` contains `a ^ b` as its left bitwise-OR operand, while `a | b ^ c` contains `b ^ c` as its right bitwise-OR operand. Comparison is structurally looser than bitwise OR. The surrounding receiving position supplies the exact required type consumed by bitwise-OR semantics; syntax alone performs no operand type inference, promotion, conversion, defaulting, overload selection, physical bit-pattern reinterpretation, pointer/reference interpretation, generic bitwise dispatch, closure interpretation, or pattern-alternative interpretation.

`ComparisonValue` maps `==`, `!=`, and `<` to the comparison-local operation-selection relation in `operators.md`; syntax does not choose an operand type. Its operands are complete `OrValue`s. Exactly zero or one `ComparisonSuffix` is represented at each comparison level, so ungrouped forms such as `a == b == c`, `a < b < c`, `a < b == c`, `a == b < c`, and any other repeated or mixed comparison sequence are not represented. Explicitly grouped inner comparison values may participate as atoms of an outer comparison. This introduces explicit syntax-tree nesting, not associativity or a comparison-chain relation. Bitwise OR is structurally tighter than comparison: `a | b < c` contains `a | b` as its left comparison operand, while `a < b | c` contains `b | c` as its right comparison operand. Before effectful operand validation, `function-execution.md` classifies each complete top-level operand as `Exact(T)`, `Contextual`, or `Invalid`, with ordinary grouping transparent and without recursively mining a contextual producer's children. One symmetric exact `T` must be established. For `==` or `!=`, exact `Bool` selects the existing Boolean relation and exactly `I8`, `I16`, `I32`, `I64`, `U8`, `U16`, `U32`, or `U64` selects the corresponding fixed-width integer relation. For `<`, only those eight fixed-width integer types are admitted; exact `Bool` rejects. Every successful comparison result is intrinsically exactly `Bool`, and the surrounding required result type does not supply or alter operand type `T`. A grouped inner strict comparison contributes `Exact(Bool)` to an enclosing comparison only after `function-execution.md` can establish that inner `<` occurrence's own static comparison admission; an unanchored or otherwise statically invalid inner `<` contributes `Invalid`, never speculative Bool evidence. Thus same-type record, safe-reference, raw-pointer, floating, or other evidence does not create comparison operations for those families, and syntax introduces no default numeric type, recursive/bidirectional constraint solving, conversion, promotion, coercion, type ascription, or type-driven parse decision.

`LogicalAndValue` maps `&&` to the Boolean short-circuit-conjunction semantic relation in `operators.md`. Its operands are complete `ComparisonValue`s. Exactly zero or one `LogicalAndSuffix` is represented at each conjunction level, so ungrouped `a && b && c` and longer conjunction chains are not represented. Explicit grouping can represent `(a && b) && c` or `a && (b && c)` without introducing associativity. Comparison is structurally tighter: `a < b && ready` contains `a < b` as its left conjunction operand, while `ready && a < b` contains `a < b` as its right conjunction operand. Syntax alone does not prune, speculate, or eagerly evaluate the right producer; short-circuit validation/execution and exact successful producer-state equality are owned by `function-execution.md`.

The conjunction tier is looser than the comparison tier, the comparison tier is looser than the bitwise-OR tier, the bitwise-OR tier is looser than the exclusive-or tier, the exclusive-or tier is looser than the additive tier, the additive tier is looser than the multiplicative tier, and the multiplicative tier is looser than the prefix tier. This structural grammar ordering is the only precedence relation introduced; it requires no precedence number/table or generic binary-expression taxonomy. Forms such as `a + b / c ^ d | e < f && g`, `a + b * c ^ d | e == f && g`, `a + b ^ c | d`, `a ^ b | c`, `a | b < c`, `a < b && c`, and `a && b == c` therefore each have one unambiguous syntax tree. Current operator typing may still reject such a tree: conjunction requires complete operands to produce `Bool`; comparison independently selects one exact operand type `T` from effect-free top-level evidence and admits only the relation/type combinations above; plain integer negation/complement/multiplication/addition/subtraction/exclusive-or/bitwise OR and same-format floating multiplication/division/addition/subtraction remain admitted only under their exact numeric required types. Concrete representability therefore does not create floating, record, pointer, safe-reference, structural, or generic comparison; pointer/reference arithmetic; truthiness; type-driven parser dispatch; or contract inheritance.

In ordinary `Value` positions, an operator prefix operand may be any represented `PrefixValue`, including a standalone `RecordConstruction`, a `GroupedValue`, a `NumericContractSelectedValue`, a bounded safe-reference/raw atom, or the bounded `SafeDereferenceValue`. That concrete admission does not make a record/reference/raw-pointer value Boolean, integer, or floating and does not make every selected wrapper or unsafe raw operation source-valid. `operators.md` requires the logical-negation operand and both conjunction operands to be exactly `Bool`; requires every comparison's operands to have exactly the one comparison-selected admitted type `T`, with `==`/`!=` admitting `Bool` or one of the eight represented fixed-width integer types and `<` admitting only one of those eight integer types; requires the integer-negation and integer-complement operand/result and both integer-multiplication/addition/subtraction/exclusive-or/bitwise-OR operands/result to have the exact surrounding represented fixed-width integer type; and requires floating-multiplication, floating-division, floating-addition, and floating-subtraction operands/results to have exact surrounding represented `F16`, `F32`, or `F64`. `references.md` separately owns safe-reference producer typing and authority/structural consequences, and `raw-pointers-unsafe.md` owns raw address/move typing and unsafe preconditions. `function-execution.md` owns comparison evidence classification/selection, transactional producer validation, eager/short-circuit sequencing, definite state, held-left lifetime, grouping transparency, and selector applicability. Consequently syntactically represented type-invalid combinations remain rejected at those semantic boundaries.
...
This subset has no string, byte, character, aggregate, raw-pointer, or other additional literal form; no scientific-notation, hexadecimal/binary/octal floating form, explicit infinity/NaN literal, `.5`/`1.` floating shorthand, suffix, separator, alternate numeric radix, or leading-plus numeric form; no Unit/tuple/general expression grouping beyond the bounded one-value `GroupedValue`, context-preserving `ConditionalGroupedValue`, and operation-local `NumericContractSelectedValue`; no operator beyond bounded Boolean logical negation, Boolean short-circuit conjunction, plain fixed-width integer negation/bitwise complement/multiplication/addition/subtraction/exclusive-or/bitwise-OR, same-format binary floating multiplication/division/addition/subtraction, exact-Bool equality/inequality, same-exact-type fixed-width integer equality/inequality, and same-exact-type fixed-width integer strict less-than; no source numeric-contract spelling beyond operation-local `fast`; no general unary or binary expression hierarchy beyond the bounded safe-reference dereference/root/reborrow forms, raw address/move atoms, operator prefix/multiplicative/additive/exclusive-or/bitwise-OR/comparison/logical-conjunction tiers; no binary bitwise operation beyond represented exclusive-or and bitwise OR and no shift operator; no short-circuit logical operator beyond represented conjunction; no conversion; no arbitrary-receiver member/postfix access beyond the bounded field receiver categories above and the dedicated Shared/replacement-capable root and Shared/replacement-capable reborrow target field-selector chains; no assignment expression; no block expression; no closure; and no other semantic value producer beyond the represented producers above.
...
A producer-backed pattern scrutinee in either the irrefutable declaration or bounded refutable selector likewise applies the lookup relation of its concrete producer before the pattern consumes the produced value. When that producer is a `ProducerFieldValueUse`, its receiver lookup and complete field-value production occur before the resulting owned value enters the pattern transient relation. Pattern-introduced bindings, including same-path bind-and-test declarations, are not yet in scope during any of those lookups; bounded refutable success bindings enter scope only after full match and applicable transient cleanup.

Every unqualified record-construction target and unqualified recursive record-pattern head, including a refutable-pattern head, is an explicit same-module declaration lookup. Active parameter/locals of equal key do not participate in those head/target lookups, and the selected module binding must be a record declaration. Qualified record-construction targets and qualified record-pattern heads instead use only the represented source-unit module-alias lookup relation described below.
...
- floating unary negation, unary plus, increment/decrement, another numeric unary operator beyond bounded plain fixed-width integer negation and integer bitwise complement, arithmetic beyond bounded plain fixed-width integer negation/multiplication/addition/subtraction and same-format binary floating multiplication/division/addition/subtraction, floating remainder or mixed/cross-format arithmetic, binary bitwise operations beyond bounded plain fixed-width integer exclusive-or and bitwise OR, shifts, equality/inequality for any source type other than the represented exact-Bool and same-exact-type fixed-width integer relations, ordering beyond same-exact-type fixed-width integer strict `<`, `<=`, `>`, `>=`, three-way comparison, floating/record/reference/pointer/structural ordering, short-circuit logical operators beyond represented Boolean conjunction, compound-assignment, conversion/cast, source `standard`/`reproducible` selector spellings, block/function/module numeric-contract selection scopes or defaults, lexical/dynamic contract inheritance, caller-to-callee contract propagation, generic `@` annotations/attributes/pragmas, or other operator forms beyond represented Boolean negation/conjunction, plain fixed-width integer negation/bitwise complement/multiplication/addition/subtraction/exclusive-or/bitwise-OR/equality/inequality/strict-less-than, same-format floating multiplication/division/addition/subtraction with the bounded operation-local `fast` selector, and Boolean equality/inequality;
- Unit/empty-group, tuple/comma-expression, parenthesized-type, or general expression grammar beyond the bounded one-value contextual grouping and selected-value forms above; multiplicative/additive/exclusive-or/bitwise-OR/comparison/logical-conjunction chaining; or a binary precedence/associativity hierarchy beyond the bounded multiplicative, additive, exclusive-or, bitwise-OR, non-associative comparison, and bounded logical-conjunction tiers;
...
- conditional expressions, direct `else if`, unrestricted nonterminal-within-block return or arbitrary unreachable tails, additional loop forms (`loop`, `for`, do/while), loop `else`, `while let`, labels or a label namespace, labeled `break`/`continue`, transfer values, loop values, refutable/literal patterns beyond the bounded single-success record selector above, pattern alternatives, guards, multi-arm `match`/`case`, variants/enums/tagged unions, wildcard/ignore/shorthand patterns, range patterns, non-duplicable/nested/multi-test same-path composites, catch/recovery, or other control-transfer forms beyond represented Boolean statement-level `if`, bounded single-success refutable record selection, bounded statement-level `while`, bounded unlabeled `break;`/`continue;`, terminal return, payload-free explicit `fault;`, and lexical unsafe blocks;
...
- arbitrary-receiver member/postfix access beyond the explicit binding-root/direct-call/record-construction field-value forms, the bounded binding-root field-assignment target, the bounded Shared root-target selector chain after `&`, the bounded replacement-capable root-target selector chain after `&mut`, the bounded Shared reborrow-target selector chain after `&*r`, and the bounded replacement-capable reborrow-target selector chain after `&mut *r`; direct safe-reference-relative or raw-pointer-relative field value access; field accessibility beyond the represented module-private/exported direct relation; package/friend/protected accessibility; methods; properties; or associated-item lookup;
...
- pattern alternatives or any pattern-alternative role for standalone `|`;
