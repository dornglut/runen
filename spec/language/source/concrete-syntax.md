...
An `IfStatement` produces no source value and has no trailing semicolon. It does not add a conditional expression, block value, Unit/Void value, pattern condition, guard, truthiness relation, ordering comparison, or short-circuit logical operator beyond the represented conjunction producer. The separate `RefutableRecordSelectionStatement` is a statement category rather than a pattern condition inside this `ConditionalValue`. Fixed-width integer equality/inequality, when source-valid under the ordinary comparison relation, are already Bool-producing operator values rather than a condition-specific numeric rule.
...
`EqualityValue` maps `==` and `!=` to the comparison-local equality/inequality operation-selection relation in `operators.md`; syntax does not choose Boolean versus fixed-width integer semantics. Its operands are complete `OrValue`s. Exactly zero or one `EqualitySuffix` is represented at each equality level, so ungrouped forms such as `a == b == c`, `a != b == c`, and `a == b != c` are not represented. Explicitly grouped inner equality values may participate as atoms of an outer equality as described above. This introduces explicit syntax-tree nesting, not equality associativity or a comparison-chain relation. Bitwise OR is structurally tighter than equality: `a | b == c` contains `a | b` as its left equality operand, while `a == b | c` contains `b | c` as its right equality operand. Before effectful operand validation, `function-execution.md` classifies each complete top-level operand as `Exact(T)`, `Contextual`, or `Invalid`, with ordinary grouping transparent and without recursively mining a contextual producer's children. One symmetric exact `T` must be established; exact `Bool` selects the existing Boolean relation and exactly `I8`, `I16`, `I32`, `I64`, `U8`, `U16`, `U32`, or `U64` selects the corresponding fixed-width integer relation. Any other selected exact type rejects the operation even if both operands supplied same-type static evidence. The result is intrinsically exactly `Bool`, and the surrounding required result type does not supply or alter operand type `T`. Thus same-type record, safe-reference, or raw-pointer evidence does not create equality for those families, and syntax introduces no floating equality, ordering, default numeric type, recursive/bidirectional constraint solving, conversion, promotion, coercion, type ascription, or type-driven parse decision.
