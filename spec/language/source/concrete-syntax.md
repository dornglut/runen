...
The standalone `=` punctuation token retains only its represented declaration/assignment roles, including the separator in bounded refutable record selection and the separators in bounded `ReferenceReplaceStatement` and `RawAssignStatement`. `==` instead participates only in the bounded comparison productions below and maps those forms to the equality semantic-selection relation in `operators.md`.
...
The standalone `<` punctuation token has exactly two bounded grammar roles in this revision. In the ordinary and conditional comparison productions below it maps to the plain fixed-width integer strict-less-than semantic-selection relation in `operators.md`. After the `:` of one `RefutableRecordPatternField`, it may instead begin the exact `RefutableRecordStrictUpperBoundTest` production `"<" DecimalIntegerLiteral`; that pattern-local form maps to the strict-upper-bound relation owned by `patterns.md` and is not a `ComparisonValue`, general `Value`, or second comparison operand grammar. Grammar position and complete token shape distinguish the roles without type-driven parsing. `<=` tokenizes as standalone `<` followed by standalone `=` and is not a represented operator or pattern-test form. These roles define no `>`, `>=`, three-way comparison, lower-bound or two-sided range syntax, generic ordering, type-parameter syntax, markup, or other punctuation meaning.
...
This irrefutable declaration grammar defines no `let mut Record { ... }`, shorthand field pattern, wildcard/ignore binding, tuple/array/enum pattern, literal/alternative/guard target, qualified binding leaf, qualified field name, nested module path beyond the represented alias/member pair, destructuring assignment, reference-binding mode, raw-pointer-binding pattern mode, mutable pattern-binding modifier, range pattern, constructor spread/update/rest, or general spread syntax. In particular, Boolean/decimal-integer equality literal-test leaves and fixed-width-integer strict-upper-bound test leaves belong only to the separate bounded refutable grammar below and do not become `RecordPatternTarget` alternatives.
...
- `true` or `false` selects a Boolean equality literal-test leaf;
- `DecimalMagnitude`, or standalone `-` followed across permitted trivia by `DecimalMagnitude`, selects a decimal-integer equality literal-test leaf under the existing signed integer-literal grammar;
- standalone `<` followed by `DecimalMagnitude`, or by standalone `-` followed across permitted trivia by `DecimalMagnitude`, selects a fixed-width-integer strict-upper-bound test leaf whose bound is the complete existing `DecimalIntegerLiteral` beginning after `<`;
- a bare `UserIdentifier` selects a binding leaf;
...
`DecimalFloatingLiteral` is neither a `RefutableRecordLiteralTest` nor a strict-upper-bound bound; neither a positive nor signed decimal floating spelling is admitted as a refutable test leaf. `RefutableRecordStrictUpperBoundTest` contains only standalone `<` followed by one complete `DecimalIntegerLiteral`; it is not an embedded `ComparisonValue`, general `Value`, producer operand, lower-bound test, or range endpoint form. No grouping, other operator value, safe-reference/raw-pointer value, wildcard, range, guard, alternative, shorthand, reference/mutable binding mode, or bind-and-test target is introduced. The pattern remains nominal-record-shaped at its top; no top-level scalar literal pattern is represented.

The grammar can syntactically represent a refutable pattern tree whose targets are all bindings/nested nodes and contain zero refutable-test leaves. `patterns.md` rejects that semantic case because this statement requires at least one refutable-test leaf, satisfied by either an existing equality literal-test leaf or a strict-upper-bound test leaf; concrete grammar does not duplicate that recursive semantic count merely to distinguish an otherwise well-formed tree.
...
The success arm is exactly one `BlockStatement`. `else` is optional and, when present, is followed by exactly one mismatch `BlockStatement`. Each explicit arm is therefore an ordinary child lexical scope. Success-only bindings are established and scoped according to `patterns.md` and `control-flow.md`; they are not introduced into the mismatch block. Omitted mismatch creates no concrete block or synthetic lexical scope.

`RefutableRecordSelectionStatement` is statement-only, produces no source value or Unit/Void value, and has no trailing semicolon. Its pattern/static literal-or-bound validation, scrutinee exact-type/evaluation relation, two-phase refutable-test-before-binding execution, match/mismatch transient cleanup, success binding establishment, abnormal completion, omitted mismatch, exact normal-successor state composition, and source-to-Core refinement boundaries are owned by `patterns.md` and `control-flow.md` while consuming the existing producer/cleanup/equality/strict-less-than primitives from their canonical owners.

This bounded form defines no `while let`, `let else`, standalone refutable declaration, multi-arm `match`/`case`, variants/enums, exhaustiveness over alternatives, guard, two-sided or one-sided range pattern, lower-bound test, `<=`, `>`, or `>=` pattern relation, pattern alternative, floating literal/ordering test, scalar top pattern, wildcard/shorthand/ref/mutable binding mode, destructuring assignment, general expression scrutinee, general comparison/test `Value`, pattern coercion/default numeric type, or new equality/ordering/predicate operation.
