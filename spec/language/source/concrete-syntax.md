...
Decimal integer and decimal floating literals remain syntactically represented as conditional atoms. Exact condition and operator typing are owned by `control-flow.md`, `operators.md`, and `function-execution.md`. A bare numeric literal remains syntax-valid as a `ConditionalValue` but is source-invalid because it cannot produce the exact intrinsic `Bool` required by control flow. The same is true of a plain integer-negation, integer-multiplication, integer-addition, or integer-subtraction value used directly as a condition: the condition supplies required type `Bool`, while `operators.md` admits those integer operations only when the surrounding exact required type is one of the eight fixed-width integer types, so the operation is rejected before its operand or operands may commit ownership. Likewise, an integer arithmetic operation appearing as an operand of exact-Bool equality is syntactically represented but source-invalid because equality supplies required type `Bool` to that complete additive operand. A Boolean-negated numeric atom, such as `!-1`, or numeric equality such as `1 == 1`, may likewise be syntactically represented while failing the exact Bool operand rule owned by the operator semantics.
...
PrefixValue          = BooleanNotValue | IntegerNegValue | ValueAtom
...
At every `PrefixValue` decision point, a standalone `-` followed across permitted trivia by `DecimalMagnitude` or `DecimalFloatingMagnitude` MUST be consumed by the complete applicable signed `Literal` production rather than `IntegerNegValue`. Only when neither signed-literal production applies may value-start `-` introduce `IntegerNegValue`. This priority resolves the deliberate shared punctuation without reclassifying signed literals as operators.

The five non-grouping `ValueAtom` alternatives are exactly the pre-existing value producer families admitted at atom position. `GroupedValue` is different: it is a concrete wrapper around exactly one complete existing `Value` and does not become a seventh owned-value producer family, primary-expression semantic category, postfix base, place category, or member-receiver system. The operator producer family remains one existing producer family even though it now contains Boolean negation, plain fixed-width integer negation, plain fixed-width integer multiplication, plain fixed-width integer addition, plain fixed-width integer subtraction, and Boolean equality/inequality. Validation and execution transparency are owned by `function-execution.md`.
...
`BooleanNotValue` maps to the Boolean logical-negation semantic relation in `operators.md`. `IntegerNegValue` maps to the plain fixed-width integer-negation semantic relation there. Both use right-recursive `PrefixValue` operands, so repeated or mixed prefix forms are unambiguous and right-associated by grammar nesting after signed-literal priority is applied. For example, `!!flag` is nested Boolean negation and `--1` is outer integer negation whose operand is the complete signed literal `-1`; there is no decrement token or semantic relation. Prefix recursion does not include `MultiplicativeValue`, `AdditiveValue`, or `EqualityValue`; consequently prefix binds more tightly than multiplication, addition/subtraction, and equality except where grouping explicitly nests a complete `Value`.

Syntactic prefix composition does not imply type validity. `-!flag` is structurally integer negation of Boolean negation, `!-value` is structurally Boolean negation of integer negation, and `-(1.0)` is structurally integer negation of a grouped floating literal, but each must satisfy the exact surrounding and nested required-type relations in `operators.md` and `function-execution.md`. This revision introduces no floating integer-negation fallback, truthiness, implicit conversion, or type-driven parser reinterpretation.

`!a * b` is one multiplicative value whose left operand is `!a`; `-a * b` is one multiplicative value whose left operand is `-a`; `!a + b` and `-a + b` are additive values whose left multiplicative operand contains the prefix form; and `!a == b` or `-a == b` remain equality values whose left additive operand contains that prefix form. The prefix grammar never reparses those ungrouped forms as a prefix operator around a looser operator tier.
...
- floating unary negation, unary plus, increment/decrement, another numeric unary operator beyond bounded plain fixed-width integer negation, arithmetic beyond bounded plain fixed-width integer negation/multiplication/addition/subtraction, equality/inequality for any source type other than the represented exact-Bool relations, ordering or other comparison, short-circuit logical, compound-assignment, conversion/cast, or other operator forms beyond represented Boolean negation, plain fixed-width integer negation/multiplication/addition/subtraction, and Boolean equality/inequality;
