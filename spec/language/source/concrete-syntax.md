...
ConditionalAdditiveValue =
    ConditionalPrefixValue ("+" ConditionalPrefixValue)?
...
`ConditionalGroupedValue` is likewise context-preserving: its one inner value is another complete `ConditionalValue`, not unrestricted ordinary `Value`. Parentheses therefore add explicit tree nesting without resetting conditional grammar. The existing standalone-construction exclusion remains true through every grouping depth and through any nested `!` or `+` reached inside a group. Delimiter-based widening of conditional syntax is not part of this grouping relation.
...
The optional equality suffix requires a second `ConditionalAdditiveValue`, not an unrestricted ordinary value. Consequently the standalone-construction exclusion remains true on both sides of `==` or `!=`, including beneath any number of Boolean-not prefixes, inside either side of a bounded addition, and inside any number of conditional groups. Forms such as `if flag == other { ... }`, `while !flag != !other { ... }`, and syntactically represented additive forms complete the conditional value before the following block opener.

`ConditionalBooleanNotValue` maps to the same Boolean logical-negation semantic relation in `operators.md` as the ordinary-value `BooleanNotValue` defined below. `ConditionalAdditiveValue` containing `+` maps to the same plain fixed-width integer-addition relation as the ordinary additive tier. `ConditionalEqualityValue` with `==` or `!=` maps to the same Boolean equality/inequality relations as the ordinary equality tier. These concrete placements do not define condition-specific operators. Repeated prefix forms such as `!!flag` or `! ! flag` remain recursively represented and associate from the right by grammar nesting; the additive and equality tiers are not part of that prefix recursion. A group may explicitly contain a complete conditional equality tier, so `if !(a == b) { ... }`, `if (a == b) == c { ... }`, and `if a == (b != c) { ... }` remain represented according to the same operator semantics and exact-Bool condition requirement.

Exactly zero or one ungrouped `+` is represented at each conditional additive level. `a + b + c` is therefore not represented ungrouped. Existing context-preserving grouping can express explicit nested syntax such as `(a + b) + c` or `a + (b + c)` without introducing addition associativity or a general expression hierarchy.

Decimal integer and decimal floating literals remain syntactically represented as conditional atoms. Exact condition and operator typing are owned by `control-flow.md`, `operators.md`, and `function-execution.md`. A bare numeric literal remains syntax-valid as a `ConditionalValue` but is source-invalid because it cannot produce the exact intrinsic `Bool` required by control flow. The same is true of a plain integer-addition value used directly as a condition: the condition supplies required type `Bool`, while `operators.md` admits `+` only when the surrounding exact required type is one of the eight fixed-width integer types, so the addition is rejected before its operands may commit ownership. Likewise, an addition appearing as an operand of exact-Bool equality is syntactically represented but source-invalid because equality supplies required type `Bool` to that complete additive operand. A negated or equality operand numeric atom, such as `!-1` or `1 == 1`, may likewise be syntactically represented while failing the exact Bool operand rule owned by the operator semantics.
...
`BooleanNotValue` maps to the Boolean logical-negation semantic relation in `operators.md`. Its right-recursive `PrefixValue` operand makes repeated negation such as `!!flag` or `! ! flag` unambiguous and right-associated by grammar nesting. Boolean-not recursion does not include `AdditiveValue` or `EqualityValue`; consequently prefix binds more tightly than both addition and equality except where grouping explicitly nests a complete `Value`. `!a + b` is one additive value whose left operand is `!a`, and `!a == b` remains one equality value whose left additive operand contains `!a`. The prefix form never reparses ungrouped `!a + b` or `!a == b` as logical negation of the looser operator tier.

`AdditiveValue` maps `+` to the plain fixed-width integer-addition semantic relation in `operators.md`. Exactly zero or one `AdditiveSuffix` is represented at each additive level, so ungrouped `a + b + c` is not represented. Explicitly grouped inner additions may participate as atoms of an outer addition as described above. The surrounding receiving position supplies the exact required type consumed by addition semantics; syntax alone performs no operand type inference, promotion, conversion, defaulting, or overload selection.
...
The equality tier is looser than the additive tier, and the additive tier is looser than the prefix tier. This structural grammar ordering is the only precedence relation introduced; it requires no precedence number/table or generic binary-expression taxonomy. A form such as `a + b == c` therefore has one unambiguous syntax tree whose left equality operand is the complete `a + b` additive value. Current operator typing may still reject such a tree: exact-Bool equality requires each complete operand to produce `Bool`, while plain integer addition is admitted only under an exact fixed-width integer required type. Concrete representability does not create numeric equality.

In ordinary `Value` positions, a prefix operand may be any represented `PrefixValue`, including a standalone `RecordConstruction` or a `GroupedValue` containing one. That concrete admission does not make a record value Boolean or numeric. `operators.md` requires the logical-negation operand and both equality/inequality operands to be exactly `Bool`, and requires both addition operands/result to have the exact surrounding represented fixed-width integer type, while `function-execution.md` owns transactional operand validation and grouping transparency. Consequently forms such as `!Record { ... }`, `!(Record { ... })`, `Record {} == Record {}`, `(Record {}) == (Record {})`, or `Record {} + Record {}` may be syntactically represented in an ordinary value position but are source-invalid under the applicable exact operator typing rules.

The represented literal forms map to `literals.md`. `true` and `false` denote the boolean literal forms owned there. A `DecimalIntegerLiteral` supplies its concrete sign and decimal magnitude to the exact mathematical-integer and required-type materialization relation owned there. A `DecimalFloatingLiteral` supplies its concrete sign and one contiguous decimal floating magnitude token to the exact decimal-rational and required-type floating materialization relation owned there. This grammar does not assign an integer or floating default type, abstract literal type, conversion, or arithmetic semantics beyond the separately owned plain fixed-width integer-addition operator relation.

The optional `-` in either represented numeric literal is part of that literal grammar only. Because the sign and following magnitude are distinct grammar tokens, ordinary trivia may occur between them under the general trivia rule above. No trivia may occur inside `DecimalFloatingMagnitude` because it is one token, including around its token-internal decimal point. `-` does not become a general prefix operator merely because `!`, `+`, grouping, and the equality tier are represented separately. In particular, `-!flag` and `-(1)` are not represented `Value`s. A spelling such as `!-1` is instead Boolean logical negation whose atom is the already represented signed decimal integer literal and is source-invalid because that operand cannot have type `Bool`; `(-1)` is represented only as grouping around that already complete signed literal.
...
- numeric unary negation, subtraction, arithmetic beyond bounded plain fixed-width integer addition, equality/inequality for any source type other than the represented exact-Bool relations, ordering or other comparison, short-circuit logical, compound-assignment, conversion/cast, or other operator forms beyond represented Boolean negation, plain fixed-width integer addition, and Boolean equality/inequality;
- Unit/empty-group, tuple/comma-expression, parenthesized-type, or general expression grammar beyond the bounded one-value contextual grouping forms above; addition/equality/comparison chaining; or a binary precedence/associativity hierarchy beyond the bounded additive and non-associative equality tiers;
