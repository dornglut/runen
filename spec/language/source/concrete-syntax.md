...
The standalone `<` punctuation token participates only in the bounded comparison productions below and maps that form to the plain fixed-width integer strict-less-than semantic-selection relation in `operators.md`. Before this revision `<` was not represented punctuation, so source containing it outside trivia was malformed; adding this one role reinterprets no previously source-valid spelling. `<=` tokenizes as standalone `<` followed by standalone `=` and is not a represented operator form. This role defines no `>`, `>=`, three-way comparison, generic ordering, type-parameter syntax, markup, or other punctuation meaning.
...
This irrefutable declaration grammar defines no `let mut Record { ... }`, shorthand field pattern, wildcard/ignore binding, tuple/array/enum pattern, literal/alternative/guard target, qualified binding leaf, qualified field name, nested module path beyond the represented alias/member pair, destructuring assignment, reference-binding mode, raw-pointer-binding pattern mode, mutable pattern-binding modifier, range pattern, constructor spread/update/rest, or general spread syntax. In particular, Boolean and decimal-integer literal-test leaves belong only to the separate bounded refutable grammar below and do not become `RecordPatternTarget` alternatives.
...
- `true` or `false` selects a Boolean literal-test leaf;
- `DecimalMagnitude`, or standalone `-` followed across permitted trivia by `DecimalMagnitude`, selects a decimal-integer literal-test leaf under the existing signed integer-literal grammar;
...
`DecimalFloatingLiteral` is not a `RefutableRecordLiteralTest`; neither a positive nor signed decimal floating spelling is admitted as a test leaf. No grouping, operator value, safe-reference/raw-pointer value, wildcard, range, guard, alternative, shorthand, reference/mutable binding mode, or bind-and-test target is introduced. The pattern remains nominal-record-shaped at its top; no top-level scalar literal pattern is represented.

The grammar can syntactically represent a refutable pattern tree whose targets are all bindings/nested nodes and contain zero literal-test leaves. `patterns.md` rejects that semantic case because this statement requires at least one literal-test leaf; concrete grammar does not duplicate that recursive semantic count merely to distinguish an otherwise well-formed tree.

Inside the mandatory statement-local parentheses, `RecordPatternScrutinee` is reused unchanged. The new statement therefore admits exactly the same direct bare binding root or producer-backed result-bearing `DirectCall`, `RecordConstruction`, or bounded `FieldValueUse`, and no grouping/general expression, standalone literal, operator, safe-reference/raw-pointer producer, or other scrutinee category. A producer-backed field-value scrutinee still completes its field-receiver lifecycle before the resulting record enters the distinct pattern transient relation. The delimiter itself performs no evaluation and creates no semantic wrapper.
...
`RefutableRecordSelectionStatement` is statement-only, produces no source value or Unit/Void value, and has no trailing semicolon. Its pattern/static literal validation, scrutinee exact-type/evaluation relation, two-phase literal-test-before-binding execution, match/mismatch transient cleanup, success binding establishment, abnormal completion, omitted mismatch, exact normal-successor state composition, and source-to-Core refinement boundaries are owned by `patterns.md` and `control-flow.md` while consuming the existing producer/cleanup/equality primitives from their canonical owners.

This bounded form defines no `while let`, `let else`, standalone refutable declaration, multi-arm `match`/`case`, variants/enums, exhaustiveness over alternatives, guard, range, pattern alternative, floating literal test, scalar top pattern, wildcard/shorthand/ref/mutable binding mode, destructuring assignment, general expression scrutinee, pattern coercion/default numeric type, or new equality/predicate operation.
