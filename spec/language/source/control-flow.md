...
Safe-reference authority parent/child/carrier state is validated sequentially by `references.md` and remains subject to lexical cleanup. It is **not** converted into a generic branch-state lattice, authority-graph union, join, or fixed point by this document. The represented exclusions on reference fields, mutable/rebindable reference locals, replacement-capable results, and freshly reborrowed Shared results mean persistent child authority remains determined by ordinary owned-carrier lifecycle rather than by a new control-flow merge algebra.
