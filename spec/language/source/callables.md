...
Represented source function body attachment, Runen-body source-function activations, the common represented call-site argument/result transaction after target classification, and Runen-body call execution including safe-reference carrier/result/external-referent consequences, recursion, cleanup, return, divergence, and defined-fault propagation are owned by [Source function execution](function-execution.md). Provider admission, invocation, provider observations, and provider outcome selection for declaration-only external calls are owned by [Source external callable imports](external-callables.md). The function-value branch of bounded-indirect target classification and function-value formation are owned by `function-values.md`; the dedicated-closure-binding branch is owned by `closures.md`. The non-observable generic activation substitution context carried by a generic call and marker-obligation validation ordering are owned separately by [Source generics](generics.md). Function-local parameter binding identity, scope, mutability, availability, and ordinary owned use are owned by [Source function-local bindings](local-bindings.md). The represented concrete function-definition, external-function-declaration, closure declaration, generic parameter/application, parameter, result, safe-reference type, and raw-pointer type spellings are owned by [Source concrete syntax](concrete-syntax.md).
...
## External declaration integration

[Source external callable imports](external-callables.md) maps one declaration-only external source form to the same source function-entity and callable-signature identity relations owned by this document. It does not define a competing function identity or ABI signature.

A Runen-body function definition and an external function declaration are distinct declarations and therefore distinct function entities. The former has a represented body under `function-execution.md`; the latter has the external execution origin and provider requirement owned by `external-callables.md`. This execution-origin fact is not a callable-signature equality dimension.

`external-callables.md` imposes its stricter non-generic intrinsic-scalar-only interface domain after this document establishes the ordinary callable structure. The resulting external interface has result contract `None`. No calling convention, ABI, physical symbol, unsafe-callable qualifier, or external-provider identity becomes part of the canonical callable signature through this integration.
...
Any additional concrete declaration-only or alternate definition form accepted later must map to these semantic entities without creating a competing identity relation.
