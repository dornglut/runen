...
- root Shared formation `&x` or `&x.field...` from an active binding begins with **RootOrigin(x)** for the selected binding identity, including an admitted closure capture binding, while the produced authority independently retains its exact structural target path;
- fresh Shared static-root formation `&S` or `&alias::S` begins with **StaticRootOrigin(S)** for the resolved source static binding identity;
...
A source safe-reference target is one dynamic structural region. Root formation can establish such a region from storage owned by the current activation, including admissible closure capture-binding storage, or from one execution-persistent static scalar root under `statics.md`; reference transport can carry that same region into another activation; and explicit reborrow can derive a child region by composing the parent target with a bounded relative structural field path. None of these target relations exposes a physical address, Core place, source binding name, closure environment field, or implementation storage token as program data.
...
### Execution-persistent static scalar target

A Shared root may target one complete execution-persistent static scalar root owned by `statics.md`.

For unqualified formation, local-first lookup is applied before same-module static lookup. For qualified `&alias::name`, the existing one-hop exported module lookup must select a source static. In both cases the selected static's exact scalar type must equal the requested `SharedRef(T)` referent type and satisfy Shared-referent admission.

A static root has no source binding structural-ownership state and exposes no field path in this first scalar slice. Its persistent extent and Live state are owned by `statics.md` / Core `persistent-storage.md`. Shared authority compatibility remains required.

Successful formation creates one fresh Shared authority/carrier and `StaticRootOrigin(staticBinding)` provenance. It does not read the static value. A fresh static-root authority is not `ParameterOrigin` and cannot directly satisfy either represented callable safe-reference result contract.

No replacement-capable static root, raw static root, or ownership-moving static target is represented.
...
- `*r` ownership Move through the parameter's complete target may consume its complete root;
- `*r = value` replacement through the parameter's complete target restores complete ownership with an empty consumed-path set;
...
6. identify the reference target's exact backing structural root/path `q`; when `q` is empty, select and end that root's then-current complete-root remaining ownership frontier, and when `q` is non-empty, require the canonical bounded non-empty subpath-installation admission and select and end exactly the then-current `frontier(q)`;
7. install the produced exact-`T` value into the complete referent; and
8. when `q` is empty, establish fresh complete ownership with an empty consumed-path set for that backing root, and when `q` is non-empty, apply the canonical successful bounded subpath-installation transition at `q`, removing exactly consumed paths equal to or below `q` while preserving every structurally disjoint consumed path.
...
This slice represents bounded Shared and replacement-capable reborrow over one stored safe-reference parent plus zero or more existing relative field selectors:
...
A Shared-reference explicit parameter receives one valid Shared carrier targeting storage whose extent remains live across the call. That target may belong to a still-live suspended caller/ancestor activation or to execution-persistent static storage. It remains governed by its exact authority and may be explicitly reborrowed where allowed. Its target may already be one projected structural field region because parameter transfer preserves the exact target of the caller-produced Shared value. A bounded Shared relative reborrow composes its selector path from that exact transported target; it does not rediscover the caller's source root.
...
The exact complete-target requirement for `SharedDirectChild` remains normative. Bounded field-relative reborrow therefore does not add a projected result contract: only the zero-selector Shared child `&*r` can satisfy the existing direct-child target requirement when every other requirement holds. A non-empty Shared child is projected, while every replacement-capable child remains result-invalid by permission.
...
## Persistent static refinement boundary

A fresh source Shared static root refines to the Core execution-persistent Shared-root relation in `../core/persistent-storage.md` and `../core/references.md`. The source `StaticRootOrigin` validation fact may erase after the resolved persistent target and authority are established; it is not a runtime tag. Ordinary Shared transport preserves the same target/authority exactly as for activation-local roots.

This refinement requires no physical static address, relocation guarantee, raw-pointer provenance, result-origin extension, or ABI/linkage identity.
