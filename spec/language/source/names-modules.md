...
Function-local lexical scopes, local binding identity, local lookup precedence, and local shadowing are owned by [Source function-local bindings](local-bindings.md). The represented concrete record/Runen-body-function/external-function/marker-trait/constant/static accessibility, module-import alias, and two-part qualified module-member forms, including their use as record-pattern heads, marker references, constant values, static values, bounded qualified Shared static roots, and direct call targets, are owned by [Source concrete syntax](concrete-syntax.md). Declaration-only external function execution-origin and provider semantics are owned by [Source external callable imports](external-callables.md). Marker trait entity/category semantics, unnamed implementation propositions, and coherence are owned by [Source marker traits](traits.md). Constant declaration/value/use semantics are owned by [Source constants](constants.md). Persistent static declaration/storage/use semantics are owned by [Source immutable execution statics](statics.md). Captureless function-value formation and bounded indirect-call target classification consume these lookup relations through [Source function values and indirect calls](function-values.md). This document does not define member lookup, overload resolution, package discovery, marker implementation selection, constant evaluation beyond that owner, or an implementation representation.
...
The represented record definition, Runen-body function definition, and external function declaration in `concrete-syntax.md` each establish their accepted declaration and therefore contribute one module binding through this relation. Runen-body and external function declarations share the existing function-entity binding category; external origin does not create a second function namespace.
...
The represented source constant declaration owned semantically by `constants.md` and the represented source static declaration owned by `statics.md`, both spelled by `concrete-syntax.md`, likewise contribute ordinary module bindings through this relation. Record, function, marker-trait, constant, and static bindings share this one declaration namespace and the same duplicate-key prohibition.
...
Within represented function bodies, `local-bindings.md` owns when active function-local bindings are consulted before this same-module relation. The consuming source form validates the category of the selected entity; same-module lookup does not skip bindings based on the category desired by that context. Explicit unqualified record-construction targets and unqualified record-pattern heads consume same-module declaration lookup directly under their own owners without introducing local-binding participation. A bare constant or static scalar value use consumes the local-first relation from `local-bindings.md` and reaches this same-module relation only when no active body-local value binding selects the key; `constants.md` or `statics.md` then requires the selected module binding to have the applicable category. An unqualified Shared static-root spelling follows that same local-first boundary: an active body binding remains category-final, and only absence of a local selection permits `statics.md` to select a same-module static. A bare function-value formation under an exact required function-value type uses the same local-first boundary and reaches this same-module relation only when no active local resolves; `function-values.md` then requires the selected module binding to be one exact non-generic function entity. The same same-module function lookup continues to select the direct-call target when no active local resolves an unqualified call target. After one function entity is selected, `function-execution.md` owns Runen-body execution while `external-callables.md` owns the distinct external-provider execution origin; lookup does not search by execution origin.
...
A qualified constant value consumes this same one-hop exported-binding relation. `constants.md` separately requires the selected binding to denote one source constant and defines its effect-free owned-value production. A qualified static scalar value or bounded `&alias::member` Shared static root consumes the same exported-binding relation. `statics.md` separately requires the selected binding to denote one source static and owns its persistent read/root semantics. A qualified function-value formation consumes the same exported-binding relation and `function-values.md` separately requires one eligible non-generic Runen-body function entity of the exact required function-value type. A qualified call target likewise remains the direct module-function call category because function-local bindings do not participate in this explicitly qualified lookup; after selection, Runen-body versus external execution origin is owned by `function-execution.md` or `external-callables.md` respectively.
...
This permission concerns source name resolution only. The represented literal-initialized constants from `constants.md` and literal-initialized immutable statics from `statics.md` create no declaration dependency edge: static persistent instances begin from already materialized literal values rather than runtime declaration evaluation. Therefore this slice adds no initialization/evaluation cycle rule. A later constant-expression dependency, runtime-static initialization extension, linking, package/build dependency, environment-admission, or other canonical owner may impose an independently justified cycle restriction for its own semantics. Physical compilation or build order does not by itself create a source name-resolution restriction.

## Deliberate boundaries

This revision defines module declaration namespaces, binding accessibility, same-module lookup, source-unit module-alias scopes, and qualified cross-module lookup. Represented function-local value-binding scopes and precedence are owned by `local-bindings.md`. The current concrete record/function/marker-trait/constant/static/import/export/qualification forms are owned by `concrete-syntax.md`.
...
- constant expressions/dependencies beyond the represented self-contained literal initializer, runtime static initialization/dependency ordering beyond the already-materialized first-slice static values, or runtime module initialization;
- binary ABI, physical linkage/symbol visibility, external Runen export, or physical external-provider resolution; declaration-only semantic external callable imports are owned separately by `external-callables.md`;
- parser, lossless syntax, HIR, Core MIR lowering, backend, or another implementation representation.

Top-level marker trait, constant, and static bindings use the ordinary declaration lookup represented above. Exact marker implementation/coherence and compilation-global evidence are owned by `traits.md` and introduce no lookup namespace. Exact constant value/initializer/use semantics are owned by `constants.md`; exact persistent static storage/read/Shared-root semantics are owned by `statics.md`. Neither creates a runtime module object or a second lookup namespace.
