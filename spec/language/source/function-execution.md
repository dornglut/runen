...
- under `SharedIdentity`, when slot `j` receives a caller-created Shared child `&*parent`, valid return preserves that exact child authority rather than deriving a new child in the callee;
- under `SharedDirectChild`, the designated replacement-capable argument carrier transfers into the callee, the valid result child is produced within the callee's represented execution or by a nested result producer, that child survives cleanup, and no carrier for the designated parent authority returns to the caller through this result; and
...
A temporary Shared root `&x` or `&x.field...`, a temporary complete-root replacement-capable `&mut x`, and a temporary reborrow `&*r` or `&mut *r`, is ordinary producer ownership containing the produced safe-reference carrier until it transfers into its receiving local/argument position or is cleaned by that receiving relation. Reborrow carries a fresh child authority while root formation carries a fresh root authority. Under `SharedIdentity`, a valid Shared root or child passed to the advertised origin slot may round-trip only when the callee preserves that exact incoming authority and target. Under `SharedDirectChild`, the advertised origin slot is replacement-capable; a valid callee result may expose an exact Shared direct child already created from that designated activation authority, or an identity-preserving transport/forwarding of such a child, but neither the Return boundary nor call boundary creates a synthetic child. `*r` produces an ordinary referent value; when its referent is non-duplicable through a replacement-capable reference it additionally consumes the complete referent structural root, but creates no new reference carrier category.
...
Safe root formation creates one fresh source authority/carrier at its producer position without reading, moving, mutating, or consuming the target value. Complete-referent dereference accesses the referent under the retained capability selected by `references.md`; for replacement-capable non-duplicable referents it performs the structural ownership Move at that producer position. Explicit reborrow creates a fresh child authority/carrier and delegates capability without moving/copying the parent carrier. Under `SharedIdentity`, result transfer preserves a previously produced authority/carrier relationship across the Return/call boundary and introduces no additional producer/effect position. Under `SharedDirectChild`, the child authority is created only at its actual source-valid reborrow producer position, or arrives as the already-produced result of a nested call whose own contract has completed; Return and the outer call boundary only preserve and expose that already-existing child across cleanup. They do not create a child after parent-carrier cleanup, whether that carrier ended earlier through ordinary execution or during final activation cleanup. Those are semantic alias/value-production consequences but introduce no source-visible physical addressing, runtime pointer arithmetic, or source effect system.
