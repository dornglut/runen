...
8. for a complete external-referent root or local-binding empty-path target, select and clean the then-current complete-root remaining ownership frontier in canonical frontier order; for a local-binding non-empty target path `p`, require the canonical bounded non-empty subpath-installation admission and select and clean exactly the then-current `frontier(p)`;
9. transfer the produced exact-`T` value into exactly the complete referent selected by the reference; and
10. for a complete external-referent root or local-binding empty path, establish fresh complete structural ownership for that domain; for a local-binding non-empty path `p`, apply the canonical successful bounded subpath-installation transition, preserving structurally disjoint consumed state.

The destination reference carrier is neither consumed nor replaced by this statement. The reference may denote an empty-path local root, a non-empty projected local path, or a replacement-capable parameter's external referent root. For an already valid non-empty projected local target `p`, the post-RHS structural admission may accept a fully available, exactly consumed, or partially available descendant-consumed `p`, but rejects a consumed strict ancestor exactly as owned by `references.md` and `structural-ownership.md`. The RHS may itself ownership-move the referent—for example through `*r` when `T` is non-duplicable—so long as it does not consume, move away, or otherwise disable the destination reference carrier/authority required at step 6. This relation deliberately rejects both bypass of the pre-RHS authority admission and any design that snapshots a dereference destination, lets the RHS consume the destination reference, and then commits through stale authority.
...
Safe root formation, explicit bounded reborrow, and an already admitted complete-referent dereference have no divergence point after source validation. A non-duplicable complete-referent dereference performs its Move at that producer position and then yields the owned value. Divergence may occur in a surrounding receiving producer after such a value or reference carrier has already become a transient; in that case its ownership/authority remains exactly where transfer had reached and is not implicitly restored or shortened.
...
Safe root formation creates one fresh source authority/carrier at its producer position without reading, moving, mutating, or consuming the target value. Complete-referent dereference accesses the referent under the retained capability selected by `references.md`; for replacement-capable non-duplicable referents it performs the structural ownership Move at that producer position. Explicit bounded reborrow creates exactly one fresh child authority/carrier at its producer position and delegates capability only over that child's exact target without moving/copying the parent carrier or accessing the target value. A non-empty Shared selector path changes only the child's selected target/delegation region; it creates no field-value read or ownership transition. Under `SharedIdentity`, result transfer preserves a previously produced authority/carrier relationship across the Return/call boundary and introduces no additional producer/effect position. Under `SharedDirectChild`, the child authority is created only at its actual source-valid zero-selector reborrow producer position, or arrives as the already-produced result of a nested call whose own contract has completed; a non-empty projected child does not satisfy that contract. Return and the outer call boundary only preserve and expose an already-existing valid result child across cleanup. They do not create a child after parent-carrier cleanup, whether that carrier ended earlier through ordinary execution or during final activation cleanup. Those are semantic alias/value-production consequences but introduce no source-visible physical addressing, runtime pointer arithmetic, or source effect system.
