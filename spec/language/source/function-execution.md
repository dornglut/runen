...
The exact required type supplied by an existing receiving position—local initializer, whole-binding assignment RHS, direct-call argument, record initializer, return value, or conditional/loop condition—also passes unchanged to a represented plain integer-neg, integer-complement, integer-mul, integer-add, or integer-sub producer. The selected integer operator producer consumes that one required type under its operation-specific context-directed rule in `operators.md`; no receiving position gains a separate integer-operator typing rule. Represented Boolean operators instead consume their intrinsic exact-`Bool` result/operand relations from `operators.md`; the receiving required type must accept that intrinsic result before the applicable Boolean-operator transaction may commit operand ownership.
...
Left-to-right constructor evaluation, left-to-right argument evaluation, source-first assignment RHS evaluation, Boolean logical-negation, plain fixed-width integer-negation, and plain fixed-width integer-complement operand evaluation before their respective result transformations, eager left-to-right plain integer-multiplication/addition/subtraction and Boolean equality/inequality operand evaluation, Boolean short-circuit conjunction left evaluation and selection before any permitted right evaluation, producer-backed field receiver evaluation before selected-field production, producer-before-pattern evaluation, **depth-first pattern binding-leaf source order**, and concrete body/block statement sequencing fix relative source ordering for any effects that future accepted operation owners make observable.
...
Additional operators beyond Boolean logical negation, plain fixed-width integer negation/bitwise-complement/multiplication/addition/subtraction, Boolean equality/inequality, and Boolean short-circuit conjunction, general expressions beyond the bounded grouping wrapper and prefix/multiplicative/additive/equality/logical-conjunction tiers, arbitrary assignment places, additional loop forms, labeled transfers, transfer values, unrestricted nonterminal-within-block return, arbitrary-receiver members, refutable/rest/shorthand pattern categories, additional producer-backed scrutinee families, and other source forms remain outside this execution relation.
