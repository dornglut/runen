...
On normal callee completion, callee-local and parameter cleanup removes every callee-owned Shared-reference carrier before control returns to the caller. Because this first source slice exposes only non-mutating Shared dereference/copy and permits only duplicable reference-free referents, the external referent remains fully available/live for the caller continuation. On defined fault, the same carrier cleanup occurs while unwinding the callee before the fault continues into the caller. If the callee diverges, the caller remains suspended and its target storage plus the transferred Shared authority/carrier relation remain live. These rules compose identically for nested and recursive calls.
...
After transfer, the local begins with complete initial structural ownership of its value under `local-bindings.md` and `structural-ownership.md`. Transfer does not duplicate the produced value. When the local has Shared-reference type, source validity has already required the local to be immutable; transfer moves the produced reference carrier into that binding without creating another carrier or authority, and the carrier remains until ordinary lexical cleanup of that binding.
...
Function parameters belong to the root activation but are not local declarations. On activation termination, after root lexical locals, process parameters in **reverse callable-signature parameter-slot order**, each using its then-current frontier. A Shared-reference parameter loses its transferred carrier when that parameter is cleaned. A loop transfer is not activation termination and therefore does not process parameters merely because it exits loop-body child scopes.

This cleanup order is semantic and independent of physical stack layout, ABI passing, compiler/Core local numbering, or backend strategy. Together with the source reference lifetime rules, it ensures a callee parameter carrier ends before control returns/faults into a caller whose target binding remains live, and ensures a reference local declared after its borrow target is cleaned before that target's enclosing storage extent ends.
...
For a source function with one result type, that result type is the required type supplied to the return-value producer. Represented decimal integer and decimal floating literal returns therefore materialize under the declared result type through `literals.md`. `callables.md` and `references.md` prohibit every Shared-reference result type in this first slice, so no source-valid return can escape a reference carrier from the terminating activation.
...
Result evaluation, including any structural ownership transition, Shared-reference consequence internal to nested producers, or producer-specific transient cleanup, completes before return-induced scope/activation cleanup.
...
3. process parameters in reverse slot order, cleaning each current frontier and ending any Shared-reference parameter carriers;
...
An owned transient return result is outside callee activation-local cleanup after successful result evaluation because ownership has been separated for caller transfer. A source-valid transient return result cannot have Shared-reference type in this first slice.
...
The temporary produced value of root `&x` is ordinary producer ownership containing one Shared carrier until it transfers into its receiving local/argument position or is cleaned by that receiving relation. `*r` produces an ordinary duplicate referent value and no new reference transient/carrier category of its own.
...
Root Shared-borrow formation creates one source Shared authority/carrier at its producer position without reading, moving, mutating, or consuming the target value. Shared dereference/copy duplicates the referent value at its producer position without consuming the carrier or target. Those are semantic alias/value-production consequences but do not introduce source-visible physical addressing, runtime pointer arithmetic, or a source effect system.
