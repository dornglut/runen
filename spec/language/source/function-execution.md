...
On normal callee completion, every incoming replacement-capable external referent has already satisfied the normal restoration obligation below before activation cleanup. Every callee-owned safe-reference carrier not preserved as a valid contract-bearing Shared result is then removed by ordinary local/parameter cleanup. A valid Shared-reference result carrier is instead preserved outside that cleanup and transferred to the caller under the direct-call result relation below. Under `SharedDirectChild`, preserving the child result does not preserve the designated replacement-capable parent parameter carrier: that carrier is cleaned in the ordinary parameter-cleanup phase before caller control resumes. Ending a reborrowed child parameter branch restores its parent's retained capability according to `references.md` when that was the final descendant branch. On defined fault, there is no external-referent restoration obligation; no result is produced and ordinary fault cleanup removes callee-owned carriers before the same fault continues into the caller. If the callee diverges, the caller remains suspended, transferred safe authorities/carriers and then-current external referent states remain live, and no result or synthetic restoration is produced. These rules compose identically for nested and recursive calls.
...
For `SharedDirectChild { origin: j }`, slot `j` has exact type `ExclusiveReplaceRef(T)`. At successful call entry `references.md` establishes the designated incoming activation authority `A` and complete target `R` independently of the later location of the parameter carrier. Every source-valid normal callee result has already produced and preserved one Shared authority/carrier `C` with the exact direct-parent/target relation required by that owner. Successful-call completion accounts for `C` and the active `A -> C` relation before ordinary end-of-call carrier consequences are applied. Callee lexical cleanup then proceeds, and parameter cleanup releases the transferred carrier for `A` in ordinary reverse-slot order. Only after that cleanup does caller control resume and receive the same already-preserved `C`; the call boundary does not create, reborrow, or synthesize a child after the parent carrier has been released. `A` may therefore remain active carrierless while `C` or descendants remain, and every pre-existing ancestor relation remains unchanged under `references.md`.
...
- under `SharedDirectChild`, the designated replacement-capable argument carrier transfers into the callee, the valid result child is produced within the callee's represented execution or by a nested result producer, that child survives cleanup, and the designated parent carrier itself does not return to the caller through this result; and
...
Function parameters belong to the root activation but are not local declarations. On activation termination, after root lexical locals, process parameters in **reverse callable-signature parameter-slot order**, each using its then-current binding frontier. Every safe-reference parameter loses its transferred carrier when that parameter is cleaned. Under `SharedIdentity`, a valid result may already own a separately produced carrier for the same exact incoming Shared authority outside activation cleanup. Under `SharedDirectChild`, the valid result instead owns its already-produced Shared child carrier; the designated `ExclusiveReplaceRef(T)` parent parameter carrier is still cleaned normally before caller continuation, after which `references.md` may keep that parent authority active carrierless while the child or descendants remain. Raw-pointer parameters do not exist in this source slice. A loop transfer is not activation termination and therefore does not process parameters merely because it exits loop-body child scopes.
...
2. for `SharedDirectChild`, account for the preserved child `C` and its exact active direct-parent relation to `A` before any cleanup can remove the designated parent parameter carrier;
3. clean all active lexical scopes innermost through root;
4. process parameters in reverse slot order, cleaning each current binding frontier. Under `SharedIdentity`, parameter cleanup does not clean the separately preserved result carrier. Under `SharedDirectChild`, cleanup removes the transferred designated parent carrier normally; it does not remove the separately preserved child result carrier, and `references.md` keeps `A` active carrierless while that child or descendants remain;
...
Argument transients produced before a later argument fault are cleaned in reverse production order before the fault continues in the caller. When an argument transient has safe-reference type, cleaning it removes exactly its carrier and applies ordinary authority/child-branch ending. After successful transfer into a callee parameter, that carrier is no longer owned by the caller transient and is cleaned with the callee parameter at activation termination. `SharedIdentity` may separately preserve a result carrier naming the same exact incoming Shared authority. `SharedDirectChild` may separately preserve a distinct already-created Shared child carrier, but it does not preserve the designated replacement-capable parent parameter carrier; that parent carrier is cleaned before caller control resumes. Raw-pointer argument transients do not arise because raw-pointer parameters are invalid.
