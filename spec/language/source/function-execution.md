...
## Equality and inequality producer validation and execution

A represented equality or inequality producer consumes from `operators.md` its intrinsic result type `Bool`, the comparison-local operation-selection relation, exact selected operand type `T`, the applicable Boolean or fixed-width integer successful semantic value relation, exactly-once consumption of both successful owned operand values, and absence of any operator-local structural-ownership transition.

The surrounding receiving position's required source type applies first to the operator's intrinsic result type. Source validation MUST establish that the surrounding required type is exactly `Bool` before comparison evidence classification or either operand is validated in a way that may commit producer state. If the surrounding required type is not `Bool`, the operator is source-invalid with result type `Bool`, and no comparison evidence or left/right operand consequence commits producer-validation state merely while diagnosing that mismatch.

For each complete top-level equality operand, classify its represented producer **statically and effect-free** as exactly one of:

1. `Exact(T)` — accepted static source facts determine one exact result source type `T` independently of the enclosing equality operand requirement and without evaluating the producer;
...
This selection introduces no default numeric type, recursive or bidirectional constraint solver, recursive anchor mining, promotion, conversion, coercion, widening, narrowing, type ascription, overload resolution, trait dispatch, or generic equality inference.

After one exact `T` is selected, admit the operation only as follows:

- exact `Bool` selects the existing Boolean equality or Boolean inequality relation according to the concrete operation;
- exact `I8`, `I16`, `I32`, `I64`, `U8`, `U16`, `U32`, or `U64` selects the corresponding plain fixed-width integer equality or inequality relation; and
- every other selected exact source type rejects the comparison before speculative operand validation.

Evidence availability therefore does not itself authorize equality. Two exact same-type record, safe-reference, or raw-pointer operands may establish `T`, but the operation remains source-invalid because this revision admits no equality relation for those selected types.
...
4. commit the final speculative state to the containing source-validation state only when both complete operands are source-valid; and
5. add no further source-state transition for equality/inequality itself.
...
Existing literal rules compose without extension. For example, an exact `I32` binding compared with an in-range decimal integer literal selects `T = I32`, after which that literal materializes under its existing required-type relation. The symmetric literal-on-left/binding-on-right form selects the same `I32`. Two decimal integer literals remain unanchored because both top-level producers are contextual. A result-bearing call or resolved field-value producer may anchor `T` from static facts without executing the call or receiver during evidence classification. In `(x + 1) == y`, top-level addition is contextual and exact `y` may select `T`, after which the complete addition validates under that `T`; `(x + 1) == (y + 2)` remains unanchored because both complete top-level additions are contextual.
...
9. if right evaluation diverges, retain ownership of the held left operand as part of the suspended in-progress operation and perform no cleanup merely because execution remains suspended;
10. after both operands succeed, consume the held left `T` value and the successfully produced right `T` value exactly once by applying the selected Boolean or fixed-width integer equality/inequality semantic value relation from `operators.md`; and
...
After both operand values have been produced successfully, equality/inequality itself is finite, deterministic, non-faulting, and non-diverging and adds no source-visible side effect, structural transition, storage identity, cleanup phase, or runtime state.

When equality/inequality is used as a represented `if` or bounded-`while` condition, its intrinsic successful result already has exact source type `Bool`; the condition's exact-`Bool` requirement therefore admits that result but never supplies or alters integer operand type `T`. The successful post-condition producer-validation state is exactly the sequential successful state after right producer evaluation. The comparison transformation changes only the two successfully produced values into one Bool result and introduces no successful path-dependent structural state, source state set, join, meet, widening, implicit restoration, or normalization rule.

Nested represented equality/inequality operators apply their own complete relations recursively. A completed nested equality/inequality producer has intrinsic result type `Bool` and therefore supplies `Exact(Bool)` evidence when it is itself one complete top-level operand of an outer comparison; that static evidence fact does not validate or execute the nested comparison during the outer evidence pass. The nested comparison performs its own operand evidence selection and complete transactional validation when its producer is later validated under the outer selected required type.
...
The held left operand of an in-progress represented eager binary operator is a scalar operation-owned transient rather than a structural-frontier transient. For equality/inequality it has the comparison-selected exact operand type `T`, which is `Bool` for the Boolean relation or one admitted fixed-width integer type for the integer relation; for integer multiplication/addition/subtraction/exclusive-or/bitwise OR it has the selected fixed-width integer type `T`; for same-format floating multiplication, division, addition, and subtraction it has the selected exact `F16`/`F32`/`F64` type `T`. Cleaning it on right-operand fault ends that one owned value exactly once. If right evaluation succeeds, the applicable operator relation consumes it instead; if right evaluation diverges, it remains owned by the suspended operation. A floating operation's selected contract is not part of the held value and adds no cleanup-bearing state. Unary Boolean negation, integer negation, and integer complement create no held-left transient or other separately cleanup-bearing operator transient. Boolean short-circuit conjunction likewise creates no held-left transient across right evaluation because the successful left Bool is consumed for selection before the right producer can begin.
...
If the right operand of a represented eager binary operator diverges after left success, the operator remains suspended in that right producer. The held left value and completed source-state consequences remain owned/effective; no operator result or held-left cleanup occurs merely because time passes. If the left operand diverges, right evaluation never begins and no held left operand exists. This applies equally to equality/inequality under its comparison-selected Bool or fixed-width integer `T`, plain fixed-width integer multiplication/addition/subtraction/exclusive-or/bitwise OR, and same-format floating multiplication/division/addition/subtraction under any selected contract.
