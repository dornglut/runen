...
- under `SharedIdentity`, when slot `j` receives a temporary root `&x`, successful return preserves a carrier for that same root authority after the callee parameter carrier would otherwise be cleaned, so the caller may transfer the result into an immutable Shared-reference local while `x` remains in its valid lexical extent;
