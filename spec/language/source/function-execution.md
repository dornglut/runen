...
- a source-valid explicit bounded reborrow producer from `references.md`;
...
The represented whole-binding assignment, bounded binding-root field assignment, complete-referent replacement, raw replacement, lexical unsafe block, `fault;`, `break;`, and `continue;` forms are body statements rather than owned value producers. Whole-binding assignment, bounded binding-root field assignment, reference replacement, raw replacement, and an unsafe block may complete locally normally; `fault;`, `break;`, and `continue;` have no local fallthrough. None becomes a `Value`, `ConditionalValue`, call, or return-value form merely because it is a body statement.
...
A safe root-reference producer receives its surrounding exact required safe-reference type and applies the root-target, referent-admission, availability, mutability where applicable, lexical-extent, and canonical authority-compatibility rules from `references.md`. For either Shared or replacement-capable root formation, source validation completes the root lookup and every present field-selector resolution/accessibility step, requires exact final referent type/admission and selected-path full availability, requires the containing binding extent to remain active, and applies the canonical authority requirement for exactly that selected target before committing any fresh authority/carrier. `&x`/`&x.field...` select Shared permission; `&mut x`/`&mut x.field...` select replacement-capable exclusive permission and additionally require the root to be one admitted mutable ordinary local. Zero selectors preserve the existing complete-root cases. Successful root formation creates one fresh authority/carrier targeting exactly the resolved structural path, reads/copies/moves/replaces no target value, changes no structural ownership state, and is non-faulting/non-diverging after source validation.

A bounded complete-referent dereference producer resolves its stored reference binding under `references.md` and requires surrounding exact required type equal to the referent `T`. For `SharedRef(T)` it duplicates the complete referent and leaves target ownership unchanged. For `ExclusiveReplaceRef(T)`, it duplicates a duplicable `T` under retained Shared reference-relative capability, while a non-duplicable `T` is ownership-moved under retained Exclusive reference-relative capability and consumes exactly the complete structural region denoted by that reference. When the reference target is backed by a current-activation local binding root at structural path `q`, a non-duplicable Move consumes exactly `q`; when it is backed by an incoming replacement-capable parameter's external referent root at structural path `q`, the Move consumes exactly that same external-root path `q`. In either case `q` may be empty or non-empty, so dereference remains complete relative to the stored reference's exact selected target, including a projected root or projected replacement-capable child. Structurally disjoint backing paths remain unchanged, and ending the reference authority later does not restore ownership consumed by that Move. Dereference itself neither moves nor duplicates the stored reference carrier merely to select the target.

An explicit bounded reborrow producer resolves its parent safe-reference binding and applies the exact selector-path, selected-type/referent admission, child-permission, target availability, retained-parent target-relative capability, lexical-validity, and fresh-authority rules from `references.md`. `&*r` is the zero-selector Shared case and creates a fresh Shared child over the parent's complete referent only when the resulting `SharedRef(T)` is represented and the parent retains Shared capability there. `&*r.field...` resolves every selector statically from the parent referent type, requires the exact final selected type to match the receiving `SharedRef(T)`, and creates the same kind of fresh Shared child over exactly the composed descendant target only when the parent retains Shared capability at that selected region. `&mut *r` is the zero-selector replacement-capable case and preserves the existing complete-referent relation. `&mut *r.field...` requires a parent of exact type `ExclusiveReplaceRef(P)`, resolves every selector statically from exact referent type `P`, requires the exact final selected type `T` to be replacement-reference-referent-admissible and to match the receiving `ExclusiveReplaceRef(T)`, requires the exact composed target to be fully available, and requires the parent to retain replacement-capable Exclusive capability over exactly that selected target. A Shared parent never strengthens to a replacement-capable child.

All selector/accessibility/type/availability/delegation requirements are source validity and complete before child creation. After they succeed, reborrow evaluates no nested producer, reads/copies/moves no parent carrier or target value, changes no referent structural ownership state, and creates exactly one fresh child authority/carrier with fresh reborrow provenance and exact direct parent/target relation. When the parent target is backed by a current-activation local binding at path `q`, a non-empty relative selector path `p` preserves that same backing root at `q + p`; when the parent target is backed by an incoming replacement-capable parameter's external referent root at path `q`, the child remains in that same external root at `q + p`. No second structural ownership root is created. The formation tail is finite, non-faulting, and non-diverging. Structurally disjoint child delegation remains independent and overlapping delegation remains governed only by the target-relative authority relation in `references.md`. Ending a projected child restores only delegated alias capability; it does not restore structural ownership previously consumed through that child.
...
When a `GroupedValue` or `ConditionalGroupedValue` is one complete top-level operand of a represented comparison, grouping is also transparent to the comparison-local static exact-result-type evidence relation below. The grouped operand has exactly the evidence classification of its complete contained producer: `Exact(T)`, `Contextual`, or `Invalid`. Grouping supplies no anchor, changes no selected exact type, and does not permit recursive anchor mining into the children of a contextual contained producer. In particular, grouping around `(x + 1)` does not make the top-level context-directed addition exact merely because nested `x` has a declared exact type.
...
9. if right evaluation diverges, retain ownership of the held left operand as part of the suspended operation and perform no cleanup merely because execution remains suspended;
...
## Comparison producer validation and execution

A represented equality, inequality, or strict-less-than producer consumes from `operators.md` its intrinsic result type `Bool`, the comparison-local operation-selection relation, exact selected operand type `T`, the applicable Boolean equality/inequality, fixed-width integer equality/inequality, or fixed-width integer strict-order successful semantic value relation, exactly-once consumption of both successful owned operand values, and absence of any operator-local structural-ownership transition.

The surrounding receiving position's required source type applies first to the comparison's intrinsic result type. Source validation MUST establish that the surrounding required type is exactly `Bool` before comparison evidence classification or either operand is validated in a way that may commit producer state. If the surrounding required type is not `Bool`, the comparison is source-invalid with result type `Bool`, and no comparison evidence or left/right operand consequence commits producer-validation state merely while diagnosing that mismatch.

For each complete top-level comparison operand, classify its represented producer **statically and effect-free** as exactly one of:

1. `Exact(T)` — accepted static source facts determine one exact result source type `T` independently of the enclosing comparison operand requirement and without evaluating the producer;
2. `Contextual` — the producer deliberately requires an external exact required type and supplies no comparison anchor itself; or
3. `Invalid` — the static lookup/category/type/admission facts required even to establish the producer result category fail.

This classification establishes only static producer result-type evidence. It performs no source evaluation, binding `Move` or `Copy`, direct call, constructor initializer evaluation or record assembly, field receiver evaluation or field-value read/consumption, safe-reference formation/dereference/reborrow, raw operation, defined-fault/divergence observation, cleanup, runtime query, or producer-state transition. It does not prematurely require ordinary ownership availability, safe-authority compatibility, raw unsafe admission, or another producer validity fact merely because that fact will later be required to execute or validate the producer under selected `T`; those requirements remain inside the speculative complete operand transaction below. A producer is `Invalid` here only when its already-owned static lookup/category/type facts cannot establish its result category/type at all, or when the special nested strict-less-than rule below cannot establish that nested comparison's own static integer admission.

Current represented producers classify as follows when the named static facts resolve:

- a Boolean literal is `Exact(Bool)`;
- ordinary whole-binding use is `Exact(T)` from the selected binding's declared exact type;
...
- a record construction is `Exact(T)` from its statically resolved nominal target type, without evaluating any initializer;
- a binding-root or represented producer-backed field-value use is `Exact(T)` from its statically resolved final field type, without reading a field or evaluating the producer receiver;
- Boolean logical negation, equality/inequality, and Boolean short-circuit conjunction are `Exact(Bool)` from their intrinsic result type; classifying such a nested operator does not validate or execute its own operands at this evidence step;
- a strict-less-than comparison is `Exact(Bool)` only when this same effect-free evidence relation can establish that nested occurrence's own exact selected operand type and `operators.md` admits that selected type for strict less-than; an unanchored nested strict comparison, an `Invalid` nested operand, or a nested comparison whose selected exact type is not one of the eight fixed-width integer types is `Invalid` rather than speculative `Exact(Bool)` evidence;
- a safe root-reference formation is `Exact(SharedRef(T))` or `Exact(ExclusiveReplaceRef(T))` from its concrete permission form plus statically resolved referent/path type;
- a complete-referent safe-reference dereference is `Exact(T)` from the stored safe-reference binding's exact referent type;
- an explicit bounded safe-reference reborrow is `Exact(SharedRef(T))` or `Exact(ExclusiveReplaceRef(T))` from the stored parent referent/path plus concrete permission form;
- raw address formation is `Exact(RawPtr(T))` from the statically resolved target type; and
- raw ownership move is `Exact(T)` from the stored raw-pointer binding's exact pointee type.

The nested strict-less-than evidence check is structural and effect-free. It recursively classifies only that nested comparison's complete top-level operands, applies the same symmetric exact-type selection rule below, and checks only strict-less-than operation admission for the selected type. It does not validate/evaluate either nested operand producer, commit source producer-validation state, use an outer comparison's selected type as an inward requirement, or recursively mine a contextual producer's children for anchors. The recursion therefore follows explicit finite nested comparison syntax rather than creating a bidirectional constraint solver.

Current top-level contextual producers are:

- decimal integer literals;
- decimal floating literals;
- context-directed plain fixed-width integer negation, complement, multiplication, addition, subtraction, exclusive-or, and bitwise OR;
- context-directed same-format floating multiplication, division, addition, and subtraction; and
- a numeric-contract-selected wrapper whose governed root producer remains contextual.

`GroupedValue` and `ConditionalGroupedValue` use the evidence-transparency rule above. In particular, classification does not recursively mine a contextual producer's children for exact anchors: `(x + 1)` remains `Contextual` as a complete top-level comparison operand even when nested `x` has a declared exact type.

After classifying both complete operands, establish one exact selected operand type `T` symmetrically:

- if either classification is `Invalid`, reject the complete comparison before operand producer-validation state may commit;
- if neither operand is `Exact`, reject the comparison as unanchored;
- if exactly one operand is `Exact(T)`, select exactly that `T`;
- if both operands are `Exact(T)` for the same exact source type, select that `T`; and
- if both operands are exact but their exact source types differ, reject before operand producer-validation state may commit.

This selection introduces no default numeric type, recursive or bidirectional constraint solver, recursive anchor mining into contextual producers, promotion, conversion, coercion, widening, narrowing, type ascription, overload resolution, trait dispatch, or generic comparison inference.

After one exact `T` is selected, admit the concrete comparison operation only as follows:

- for `==` or `!=`, exact `Bool` selects the existing Boolean equality or Boolean inequality relation according to the concrete operation;
- for `==` or `!=`, exact `I8`, `I16`, `I32`, `I64`, `U8`, `U16`, `U32`, or `U64` selects the corresponding plain fixed-width integer equality or inequality relation;
- for `<`, exact `I8`, `I16`, `I32`, `I64`, `U8`, `U16`, `U32`, or `U64` selects plain fixed-width integer strict less-than; and
- every other operator/type combination rejects before speculative operand validation.

Evidence availability therefore does not itself authorize a comparison relation. Two exact same-type record, safe-reference, raw-pointer, or floating operands may establish `T`, but every represented comparison rejects those selected types; exact `Bool` admits only equality/inequality and rejects strict less-than.

Only after outer Bool-result admission, effect-free evidence selection, and operation admission all succeed does source validation validate the complete eager binary producer as one transaction:

1. clone the current source producer-validation state;
2. validate the complete left producer with required source type exactly selected `T` against that clone;
3. only after complete left validation succeeds, validate the complete right producer with required source type exactly the same `T` against the resulting speculative state;
4. commit the final speculative state to the containing source-validation state only when both complete operands are source-valid; and
5. add no further source-state transition for the comparison itself.

A left validation failure therefore commits no speculative producer-state consequence. A right validation failure also commits no speculative consequence from the otherwise-valid left producer. Ordinary producer validity—including ownership availability, safe-authority compatibility, raw unsafe admission where applicable, literal representability, nested call/constructor/field-value validity, and every nested producer transaction—remains enforced at this stage and rolls back with the complete comparison transaction if either complete operand fails.

Existing literal rules compose without extension. For example, an exact `I32` binding compared with an in-range decimal integer literal selects `T = I32`, after which that literal materializes under its existing required-type relation; this works symmetrically for `==`, `!=`, or `<`. The literal-on-left/binding-on-right form selects the same `I32`. Two decimal integer literals remain unanchored because both top-level producers are contextual. A result-bearing call or resolved field-value producer may anchor `T` from static facts without executing the call or receiver during evidence classification. In `(x + 1) == y`, top-level addition is contextual and exact `y` may select `T`, after which the complete addition validates under that `T`; `(x + 1) == (y + 2)` remains unanchored because both complete top-level additions are contextual. For nested ordering, `(x < 1) == flag` may classify the grouped left operand as `Exact(Bool)` when exact integer `x` anchors and admits that inner `<`; `(1 < 2) == flag` rejects because the inner `<` is unanchored and therefore `Invalid`, and an outer exact-Bool `flag` cannot retroactively supply the inner integer type.

Dynamic execution is eager and exactly left-to-right under the already selected exact operand type `T`; evidence classification/selection itself has no runtime execution step:

1. evaluate the complete left producer exactly once under its existing producer semantics with required source type `T`;
2. preserve every source-state consequence completed by left evaluation;
3. if left evaluation yields defined fault `F`, produce no comparison result, do not begin right evaluation, and continue the same `F` through the existing producer/receiving cleanup and propagation relations;
...
5. after left success, hold its one produced owned `T` value as the in-progress binary operator's **held left operand**;
6. evaluate the complete right producer exactly once with required source type exactly `T`, regardless of the semantic value held on the left;
7. preserve every source-state consequence completed by right evaluation;
8. if right evaluation yields defined fault `F`, clean the held left operand exactly once, produce no comparison result, and continue the same `F` through the existing producer/receiving cleanup and propagation relations;
9. if right evaluation diverges, retain ownership of the held left operand as part of the suspended in-progress operation and perform no cleanup merely because execution remains suspended;
10. after both operands succeed, consume the held left `T` value and the successfully produced right `T` value exactly once by applying the selected Boolean equality/inequality, fixed-width integer equality/inequality, or fixed-width integer strict-less-than semantic value relation from `operators.md`; and
11. transfer the resulting distinct owned Bool exactly once to the surrounding receiving position.

The held left operand is one bounded operation-owned transient value of exact comparison-selected type `T`. It is not a source binding, place, lvalue, address, reference, argument transient, construction transient, field-receiver transient, pattern transient, condition transient, or source-visible storage identity. It exists only after left producer success and ends by cleanup on right fault, by successful operator consumption after right success, or remains owned while right evaluation diverges.

After both operand values have been produced successfully, the selected comparison itself is finite, deterministic, non-faulting, and non-diverging and adds no source-visible side effect, structural transition, storage identity, cleanup phase, or runtime state.

When a represented comparison is used as an `if` or bounded-`while` condition, its intrinsic successful result already has exact source type `Bool`; the condition's exact-`Bool` requirement therefore admits that result but never supplies or alters comparison operand type `T`. The successful post-condition producer-validation state is exactly the sequential successful state after right producer evaluation. The comparison transformation changes only the two successfully produced values into one Bool result and introduces no successful path-dependent structural state, source state set, join, meet, widening, implicit restoration, or normalization rule.

Nested represented comparisons apply their own complete relations recursively. A completed nested equality/inequality producer retains its accepted intrinsic `Exact(Bool)` evidence behavior when it is one complete top-level operand of an outer comparison; that static evidence fact does not validate or execute the nested equality/inequality operands during the outer evidence pass. A nested strict-less-than producer instead supplies `Exact(Bool)` only after its own effect-free evidence selection and fixed-width-integer operation admission have succeeded as defined above; an unanchored or statically inadmissible nested `<` supplies `Invalid`. In either case the nested comparison performs its complete transactional operand validation and dynamic execution only when its producer is later validated/evaluated under the outer selected required type.
...
The complete field-receiver transient lifecycle finishes before an enclosing local receives the result, before whole-binding or bounded field-assignment old-value cleanup begins, before reference-replacement old-value cleanup begins, before a direct-call argument transient is established for that result, before a return begins activation cleanup, before an enclosing construction initializer holds its construction transient, before a represented control-flow construct owns its Bool condition transient, and before a producer-backed record pattern establishes its separate pattern scrutinee transient.
...
The completed record value may be transferred into ordinary local initialization, whole-binding or bounded field-assignment RHS, reference-replacement RHS, direct-call argument, return result, enclosing construction field, a bounded producer-backed field-value receiver, or a producer-backed recursive record-pattern scrutinee. Those receiving relations keep their existing outer ordering and exact-type requirements. A qualified construction therefore composes through these existing receiving relations without defining a second execution category.
...
For a Shared or replacement-capable field-root, or a Shared or replacement-capable field-relative child argument, that complete target is its exact selected structural region rather than an ancestor binding/reference region. A held `SharedRef(T)` therefore requires full Shared capability over that exact complete target. A held `ExclusiveReplaceRef(T)` requires full replacement-capable exclusive capability over its exact complete target. A parent reference with any active child that reduces or suspends the required complete capability cannot itself satisfy that call-entry obligation. There is no implicit call-site reborrow: retaining a replacement-capable parent across a nested call requires explicit `&mut *parent` or bounded `&mut *parent.field...` for a matching replacement-capable parameter, or explicit `&*parent` / bounded `&*parent.field...` for a Shared parameter whose exact referent matches the selected target.
...
For Shared-reference type, transfer moves the transient's produced carrier into the callee parameter binding; ordinary prior Shared binding use may already have duplicated that carrier at argument production, but the call boundary itself creates neither another carrier nor another root/child authority. For replacement-capable reference type, transfer moves the one existing non-copyable carrier into the parameter. Reborrow arguments already carry their fresh child authority and exact target; field-root arguments already carry their exact selected root target; parameter transfer does not replace either with an ancestor authority or widen a projected target to an ancestor region. For a replacement-capable parameter the callee's external referent structural root has complete domain exactly equal to that transported target, including when the argument is a projected replacement-capable child; it does not rediscover or widen the caller-side backing path.
...
On normal callee completion, every incoming replacement-capable external referent has already satisfied the normal restoration obligation below before activation cleanup. Every callee-owned safe-reference carrier not preserved as a valid contract-bearing Shared result is removed exactly once by the ordinary lifetime/cleanup relation of its then-current owner. A valid Shared-reference result carrier is instead preserved outside activation-local cleanup and transferred to the caller under the direct-call result relation below. Under `SharedDirectChild`, preserving the child result does not preserve any carrier for the designated replacement-capable parent authority: if such a carrier remains callee-owned when activation cleanup begins, ordinary cleanup removes it before caller control resumes; it may instead have been moved and ended earlier by ordinary transport or nested-call completion. Ending a reborrowed child branch restores its parent's retained capability according to `references.md` when that was the final descendant branch. On defined fault, there is no external-referent restoration obligation; no result is produced and ordinary fault cleanup removes then-owned callee carriers before the same fault continues into the caller. If the callee diverges, the caller remains suspended, then-live safe authorities/carriers and external referent states remain live, and no result or synthetic restoration is produced. These rules compose identically for nested and recursive calls.
...
For a target whose callable signature has result type `SharedRef(T)`, `callables.md` has already established exactly one advertised safe-reference result contract. `references.md` owns the contract's exact authority/target validity and caller-side provenance relation; this execution owner fixes when the already-valid result becomes the caller's owned result relative to callee cleanup.

For `SharedIdentity { origin: j }`, every source-valid normal callee result preserves the exact dynamic target and same authority identity established by the value transferred to slot `j`. Caller-side result provenance is the caller-side provenance of the successfully produced argument value supplied to that slot. The call/result boundary creates no Shared root borrow, authority, reborrow, additional reference carrier, or semantic reference `Copy`; the returned carrier is the carrier produced by valid callee result evaluation and preserved outside callee activation cleanup. A fresh callee root or reborrow, including a field-relative child, cannot satisfy this identity contract merely because it reaches the same or an overlapping referent. A caller-created Shared child passed to slot `j` may round-trip only when the callee preserves that exact incoming child authority.

For `SharedDirectChild { origin: j }`, slot `j` has exact type `ExclusiveReplaceRef(T)`. At successful call entry `references.md` establishes the designated incoming activation authority `A` and complete target `R` independently of the later location of its carrier. Every source-valid normal callee result has already produced and preserved one Shared authority/carrier `C` with the exact direct-parent/target relation required by that owner. Successful-call completion accounts for `C` and the active `A -> C` relation before ordinary end-of-call carrier consequences are applied. Before caller control resumes, every callee-owned carrier for `A` has ended: if the transferred carrier remains in a callee binding at activation cleanup, lexical/parameter cleanup releases it in ordinary order; it may already have ended earlier after being moved through ordinary transport or a completed nested call. Only after normal callee cleanup does caller control resume and receive the same already-preserved `C`; the call boundary does not create, reborrow, or synthesize a child after parent-carrier release. `A` may therefore remain active carrierless while `C` or descendants remain, and every pre-existing ancestor relation remains unchanged under `references.md`.

Consequences include:

- under `SharedIdentity`, when slot `j` receives an ordinary use of an existing Shared-reference binding, argument production has already duplicated the caller's stored carrier; the caller's original carrier remains owned there and the distinct returned carrier may coexist with it for the same authority;
- under `SharedIdentity`, when slot `j` receives a temporary Shared root `&x` or `&x.field...`, successful return preserves a carrier for that same exact root authority and target after the callee parameter carrier would otherwise be cleaned, so the caller may transfer the result into an immutable Shared-reference local while the containing binding remains in its valid lexical extent;
- under `SharedIdentity`, when slot `j` receives a caller-created Shared child `&*parent` or `&*parent.field...`, valid return preserves that exact child authority and selected target rather than deriving a new child in the callee;
- under `SharedDirectChild`, the designated replacement-capable argument carrier transfers into the callee, the valid result child is produced within the callee's represented execution or by a nested result producer, that child survives cleanup, and no carrier for the designated parent authority returns to the caller through this result; a non-empty field-relative child cannot satisfy this contract because its target is a projected/subregion descendant rather than the exact complete origin target; and
- a nested or recursive contract-bearing call exposes its result summary from callable structure alone, allowing a surrounding call or Return to compose without expanding the nested callee body. Identity forwarding of a previously derived child preserves that child's existing authority; a nested direct-child result satisfies an enclosing direct-child contract only when `references.md` proves the same exact direct parent and target rather than a grandchild or unrelated descendant.

If callee execution yields defined fault, the direct-call producer yields that same fault and produces no result value, result carrier, result provenance, or direct-child result relation. A surrounding local/whole-binding assignment/bounded field assignment/reference-replacement/return/argument receiving position is therefore not initialized or transferred a result on that path.
...
This whole-binding relation does not itself define field/place assignment, compound assignment, assignment expression, reference-relative assignment other than the bounded complete-referent replacement below, source interior mutability, raw pointee replacement through ordinary assignment, or destructuring assignment. The bounded binding-root field relation below is a distinct non-empty structural-path operation. Unsafe raw pointee replacement remains the distinct operation after the safe-reference replacement relation below.

## Bounded binding-root field assignment and replacement

A source-valid bounded binding-root field assignment consumes from `local-bindings.md` one active assignment-mutable parameter/local root binding and its binding lifecycle, from `field-access.md` one already resolved and directly accessible non-empty structural field path `p` rooted at that binding plus exact final source type `T = type(p)`, from `references.md` the canonical Exclusive direct-authority compatibility relation over that exact selected region, and from `structural-ownership.md` the bounded non-empty subpath-installation admission, `frontier(p)`, and successful consumed-path transition. It does not redefine any of those relations.

The root/path/accessibility/type/mutability facts are established before RHS validation can commit producer state. Source validation of the complete statement is one transaction:

1. require canonical Exclusive direct-authority compatibility over exact target path `p` in the pre-RHS source state;
2. clone the current complete source producer-validation state;
3. validate the complete RHS producer exactly once against that clone with required source type exactly `T`;
4. if RHS validation fails, reject the statement and commit none of the speculative RHS producer-validation state;
5. on the speculative normal successful post-RHS state, require the bounded non-empty subpath-installation admission from `structural-ownership.md`, equivalently that no consumed path is a strict ancestor of `p`;
6. on that same speculative post-RHS state, re-establish canonical Exclusive direct-authority compatibility over exact target path `p`;
7. if either post-RHS requirement fails, reject the statement and commit none of the speculative statement transaction;
8. determine the then-current source `frontier(p)` from that admitted post-RHS structural state and apply the accepted successful subpath-installation state transition, removing exactly consumed paths equal to or strictly below `p` while preserving every structurally disjoint consumed path; and
9. commit the resulting successful source producer-validation state to the containing statement sequence.

The pre-RHS authority requirement prevents an already active overlapping safe authority from being bypassed merely because the RHS might later change authority state. The post-RHS requirement proves the actual replacement point remains Exclusive-compatible after all successful producer consequences. The relation is over exact region `p`: an authority over a structurally disjoint sibling remains governed by the existing non-overlap rule and is not rejected merely because another region of the same root is being replaced. Root assignment mutability is independently required and is never supplied by Exclusive authority.

Dynamic execution is **source-first** and exactly:

1. identify the already validated root binding, exact non-empty path `p`, and exact target type `T` without reading, duplicating, moving, or otherwise producing the target value merely to select it;
2. evaluate the complete RHS producer exactly once with required source type `T` under its existing producer semantics;
3. preserve every source-state and producer-transient consequence completed by RHS evaluation;
4. if RHS evaluation yields defined fault `F`, perform no outer target-frontier cleanup, field installation, consumed-path reset, or target restoration and propagate the same `F` through the existing receiving/fault relation;
5. if RHS evaluation diverges, perform no outer target-frontier cleanup, field installation, consumed-path reset, or target restoration and remain suspended in that producer;
6. after successful RHS production, preserve the produced exact-`T` value outside the target's old-value cleanup set; the statement's source validity has already proved the post-RHS no-strict-consumed-ancestor and canonical Exclusive requirements for this successful state;
7. select and clean exactly the target's then-current `frontier(p)` in canonical frontier order;
8. transfer the preserved exact-`T` value into exactly structural path `p` without duplication; and
9. establish the successful bounded subpath-installation structural state from `structural-ownership.md`.

The target root remains in scope during RHS evaluation. Every RHS producer follows its ordinary semantics, including uses that consume the exact target, descendants of the target, or structurally disjoint regions when independently valid. There is no special field self-assignment rule.

Consequently:

- if `p` remains fully available, `frontier(p)` contains exactly `p`; replacement cleans that complete old target value once and installs the new value;
- if the RHS has consumed exactly `p`, `frontier(p)` is empty; assignment reinitializes that exact vacant source subvalue and does not clean it again;
- if the RHS has consumed one or more strict descendants of `p` while no strict ancestor of `p` is consumed, `p` is partially available; assignment cleans exactly the maximal still-owned disjoint members of `frontier(p)` and reconstructs complete ownership of `p` without re-cleaning any consumed descendant;
- any consumed path structurally disjoint from `p` remains consumed after successful installation; and
- if a strict ancestor of `p` is consumed on the successful post-RHS state, source validation rejects the statement rather than splitting that ancestor into synthetic complement/sibling consumed paths or reconstructing ownership from below.

After successful RHS production, the admitted replacement tail is finite, non-faulting, and non-diverging under the represented source model. The required target-state and authority facts are source-validity facts; this relation introduces no runtime moved-state test, alias oracle, target-state fault, or dynamic authority check.

This bounded relation does not create a general place/lvalue category, field-level mutability metadata, producer-backed assignment target, qualified assignment root, reference-relative projected replacement, raw field/path assignment, destructuring assignment, compound assignment, or assignment expression. Safe-reference and raw-pointer record fields remain outside represented record value shape under their existing owners, and no safe-reference/raw-pointer target broadening follows from structural subpath installation.

## Complete-referent replacement execution

A source-valid complete-referent replacement statement `*r = Value;` consumes from `references.md` one active binding `r` of exact type `ExclusiveReplaceRef(T)`, its exact complete referent domain, the pre-RHS and post-RHS destination carrier/authority liveness and full replacement-capable authority requirements, exact RHS type `T`, source-first ordering, the applicable then-current remaining-frontier cleanup, and the complete-root or canonical non-empty subpath installation transition selected by that referent domain. It does not redefine those validity rules.

Execution is exactly:

1. resolve `r` and identify its exact complete referent domain without consuming, copying, or otherwise disabling the destination reference carrier; source validity has already required that live carrier/authority to retain full replacement-capable exclusive authority over that exact target before RHS producer consequences may commit;
2. evaluate the complete RHS producer exactly once with required source type `T` under its existing producer semantics;
3. preserve every source-state and producer-transient consequence completed by that evaluation;
4. if RHS evaluation yields defined fault `F`, perform no outer referent cleanup, replacement, structural installation/reset, or reference-carrier change and propagate the same `F` through the existing receiving/fault relation;
5. if RHS evaluation diverges, perform no outer referent cleanup, replacement, structural installation/reset, or synthetic reference change and remain suspended in that producer;
6. after successful RHS production, require `r` still to own a live destination carrier and its authority to retain full replacement-capable exclusive reference-relative authority over the exact complete referent;
7. preserve the successfully produced RHS value outside the old-referent cleanup set;
8. identify the target's exact backing structural root/path `q`; when `q` is empty, select and clean that backing root's then-current complete-root remaining ownership frontier in canonical frontier order; when `q` is non-empty in either a local-binding root or a replacement-capable external-referent root, require the canonical bounded non-empty subpath-installation admission and select and clean exactly the then-current `frontier(q)`;
9. transfer the produced exact-`T` value into exactly the complete referent selected by the reference; and
10. when `q` is empty, establish fresh complete structural ownership for that backing root; when `q` is non-empty in either backing-root category, apply the canonical successful bounded subpath-installation transition at `q`, removing exactly consumed paths equal to or below `q` while preserving every structurally disjoint consumed path.

The destination reference carrier is neither consumed nor replaced by this statement. The reference's exact complete target may be backed by a current-activation local binding root or by an incoming replacement-capable parameter's external referent root, at either an empty or non-empty structural path `q`. Non-empty external paths arise when a projected replacement-capable child is formed from an incoming replacement-capable parent; child formation creates no second external root. For any non-empty `q`, the post-RHS structural admission may accept a fully available, exactly consumed, or partially available descendant-consumed target, but rejects a consumed strict ancestor exactly as owned by `references.md` and `structural-ownership.md`. The RHS may itself ownership-move the referent—for example through `*r` when `T` is non-duplicable—so long as it does not consume, move away, or otherwise disable the destination reference carrier/authority required at step 6. This relation deliberately rejects both bypass of the pre-RHS authority admission and any design that snapshots a dereference destination, lets the RHS consume the destination reference, and then commits through stale authority. Ending a projected child authority alone never repairs structural ownership consumed through that child.

After the post-source destination-authority and applicable structural-admission checks and RHS success, the replacement tail is finite, non-faulting, and non-diverging under the represented source model. A failed authority/structural precondition is source invalidity rather than defined fault.
...
For the root function body and each represented `BlockStatement`, the applicable `BodyStatement` sequence is validated and executes strictly in concrete source order while a local normal continuation remains present. Every ordinary source-valid local declaration, record-destructuring declaration, whole-binding assignment, bounded binding-root field assignment, complete-referent replacement, raw replacement, no-result call statement, and normally completing ordinary or unsafe nested block preserves one local normal continuation. A represented explicit `fault;`, admitted `break;`, and admitted `continue;` body statement has no local normal continuation. A represented terminal return has no local normal continuation. A represented conditional exposes the local normal-continuation presence and, when present, the definite normal environment established by `control-flow.md`. A represented bounded `while` always exposes its statically represented false local normal continuation and the definite post-condition environment established by `control-flow.md`, including when its condition is the literal `true` and when some body paths transfer.
...
For bounded binding-root field assignment, complete source-first RHS production, post-RHS non-empty subpath-installation and exact-path Exclusive validity, then-current `frontier(p)` cleanup, exact-field-path replacement transfer, and successful structural subpath-installation state transition before continuing.

For complete-referent replacement, first satisfy the pre-RHS live-destination/full-authority admission owned by `references.md`, then complete source-first RHS production, the post-RHS live-destination/full-authority recheck, the applicable then-current referent-frontier cleanup, exact-`T` replacement, and either complete-root reset or canonical non-empty subpath-installation according to the stored reference's exact target domain before continuing.
...
The held left operand of an in-progress represented eager binary operator is a scalar operation-owned transient rather than a structural-frontier transient. For a represented comparison it has the comparison-selected exact operand type `T`, which is `Bool` only for the Boolean equality/inequality relation or one admitted fixed-width integer type for the integer equality/inequality/strict-less-than relations; for integer multiplication/addition/subtraction/exclusive-or/bitwise OR it has the selected fixed-width integer type `T`; for same-format floating multiplication, division, addition, and subtraction it has the selected exact `F16`/`F32`/`F64` type `T`. Cleaning it on right-operand fault ends that one owned value exactly once. If right evaluation succeeds, the applicable operator relation consumes it instead; if right evaluation diverges, it remains owned by the suspended operation. A floating operation's selected contract is not part of the held value and adds no cleanup-bearing state. Unary Boolean negation, integer negation, and integer complement create no held-left transient or other separately cleanup-bearing operator transient. Boolean short-circuit conjunction likewise creates no held-left transient across right evaluation because the successful left Bool is consumed for selection before the right producer can begin.
...
A fully available binding frontier contains only its complete root. An unavailable complete root has an empty frontier. A partial root cleans exactly the maximal still-owned disjoint subvalues and never re-cleans a consumed path. Shared-reference locals are duplicable and immutable and therefore normally retain complete binding structural ownership until lexical cleanup; replacement-capable reference locals are immutable but non-duplicable and may have had their carrier moved, in which case their binding root is consumed and has no carrier to clean there. Raw-pointer locals are duplicable and may be mutable, but pointer cleanup remains target-neutral regardless of the stored exact origin.
...
Function parameters belong to the root activation but are not local declarations. On activation termination, after root lexical locals, process parameters in **reverse callable-signature parameter-slot order**, each using its then-current binding frontier. A safe-reference parameter removes a carrier at parameter cleanup only when its then-current binding frontier still owns one; a moved replacement-capable parameter binding is already consumed and has no carrier to clean there. Under `SharedIdentity`, a valid result may already own a separately produced carrier for the same exact incoming Shared authority outside activation cleanup. Under `SharedDirectChild`, the valid result instead owns its already-produced Shared child carrier; any still-callee-owned carrier for the designated `ExclusiveReplaceRef(T)` parent authority ends through the ordinary cleanup of its current owner before caller continuation, while the authority may already be carrierless because earlier transport or nested-call completion ended that carrier. Raw-pointer parameters do not exist in this source slice. A loop transfer is not activation termination and therefore does not process parameters merely because it exits loop-body child scopes.

For normal activation termination only, every incoming replacement-capable external referent must already have passed the fully-available restoration check before this parameter cleanup begins. Parameter cleanup does not itself restore or clean that external referent; it ends the reference carrier/authority branch when one is still owned there. Fault termination has no corresponding restoration precondition and cleans the actual carrier state without synthesizing target repair.

This cleanup order is semantic and independent of physical stack layout, ABI passing, compiler/Core local numbering, or backend strategy. Together with the source safe-reference and raw-pointer lexical validity rules, it ensures every callee-owned safe-reference carrier that must end before continuation has ended before control returns/faults into a caller whose target storage remains live, ensures a reference local declared after its root/parent is cleaned before the applicable parent extent/branch may end, and ensures a valid raw-pointer local ends before the target extent that was required to contain it. A valid contract-bearing Shared-reference result cannot target callee-local storage: `SharedIdentity` preserves the designated incoming Shared authority/target, while `SharedDirectChild` preserves a Shared child with the exact designated incoming replacement-capable parent authority and complete target under `references.md`.
...
For a source function with one result type, that result type is the required type supplied to the return-value producer. Represented decimal integer and decimal floating literal returns therefore materialize under the declared result type through `literals.md`. When the declared result is bounded `SharedRef(T)`, `callables.md` supplies the enclosing callable's advertised safe-reference result contract and `references.md` owns the exact contract-specific provenance/activation-origin, dynamic target/authority, and direct-parent relation. `ExclusiveReplaceRef(T)` and `RawPtr(T)` are not result-admissible, so no source-valid normal return can export either a replacement-capable carrier or a raw-pointer value/origin from the activation.
...
For a function whose result is contract-bearing `SharedRef(T)`, successful result evaluation is additionally source-valid only when the exact advertised contract is satisfied after all result-producer effects and before activation cleanup:

- for `SharedIdentity { origin: i }`, `references.md` requires provenance exactly `ParameterOrigin(i)` and a carrier naming the exact dynamic target and same Shared authority identity established by the incoming value for slot `i`; a fresh root, another parameter's provenance, a local derived from a fresh root, or any fresh child reborrow including a field-relative child cannot satisfy this identity contract merely because its type or target matches;
- for `SharedDirectChild { origin: i }`, let `A` and `R` be the activation result-origin authority and complete target captured from the designated incoming `ExclusiveReplaceRef(T)` slot at call entry. `references.md` requires the produced result authority `C` to be active with full Shared capability, `C.target == R`, and `C.parent == A` exactly. The current carrier location of `A` does not redefine that origin. A fresh root, wrong parent, grandchild or arbitrary descendant, non-empty projected field-relative child, another projected/subregion target, or replacement-capable result cannot satisfy this contract. Identity-preserving transport or forwarding of an already-valid `C`, including a nested direct-call result with the same exact direct parent and target, remains valid under that owner.

After successful result evaluation and the applicable safe-reference result-contract check, normal return additionally requires every incoming replacement-capable external referent structural root to be fully available. This check observes the state after all return-value producer effects. No cleanup edge, implicit replacement, reset, or other repair is synthesized to satisfy it.
...
2. for `SharedDirectChild`, account for the preserved child `C` and its exact active direct-parent relation to `A` before final activation cleanup proceeds; `A` may already be carrierless, and no cleanup treats `C` as part of parent-carrier cleanup;
3. clean all active lexical scopes innermost through root;
4. process parameters in reverse slot order, cleaning each current binding frontier. Under `SharedIdentity`, parameter cleanup does not clean the separately preserved result carrier. Under `SharedDirectChild`, ordinary cleanup removes any still-callee-owned carrier for `A` through its current owner; if earlier transport or nested-call completion already left `A` carrierless, no synthetic carrier is removed or recreated. Cleanup never removes the separately preserved child result carrier, and `references.md` keeps `A` active carrierless while that child or descendants remain;
5. terminate the callee activation normally; and
6. deliver the same preserved owned transient result to the caller only after parameter cleanup has completed.

Transfer to the caller does not duplicate the result. For `SharedIdentity`, an `IdentifierUse` or another permitted producer may already have created a duplicate carrier as part of that producer's ordinary semantics. For `SharedDirectChild`, the result child must already have been created by a source-valid zero-selector reborrow or supplied by a source-valid nested result producer before the contract check; a non-empty field-relative child is projected and therefore cannot satisfy this contract. Neither Return nor the call boundary creates a fresh child, reborrow, authority, target, or semantic reference `Copy` after cleanup.
...
If return-value production yields a defined fault before successful result production, or if a nested producer faults, no normal return or normal external-referent restoration check occurs and no Shared result or direct-child result relation is established. The existing defined-fault cleanup/propagation relation below handles the then-current active scopes exactly once. If return-value production diverges, no normal return cleanup, result transfer, direct-child result relation, or synthetic restoration occurs merely because execution remains suspended.
...
A safe-reference result contract is a normal-return contract only. A defined-fault path produces no result value, Shared result carrier, result provenance, or `SharedDirectChild` result relation and performs no transfer/initialization of the surrounding result receiving position. A whole-binding assignment, bounded field assignment, safe-reference replacement, or raw replacement whose RHS faults has not performed its outer replacement; the then-current structural/authority/pointer-origin state is simply the state produced before the fault.
...
Argument transients produced before a later argument fault are cleaned in reverse production order before the fault continues in the caller. When an argument transient has safe-reference type, cleaning it removes exactly its carrier and applies ordinary authority/child-branch ending. After successful transfer into a callee parameter, that carrier is callee-owned; its later owner and lifetime follow ordinary parameter/local/call transport, and it is cleaned exactly once by whichever callee-owned location still owns it when that branch ends. `SharedIdentity` may separately preserve a result carrier naming the same exact incoming Shared authority. `SharedDirectChild` may separately preserve a distinct already-created Shared child carrier, but no callee-owned carrier for the designated replacement-capable parent authority survives into caller continuation. Raw-pointer argument transients do not arise because raw-pointer parameters are invalid.

For each represented eager binary operator, successful left production establishes exactly one held left operand value before right evaluation begins. If right evaluation faults, that held value is cleaned exactly once before the same fault continues. If right evaluation diverges, the held value remains owned by the suspended operation. After right success, both operand values are consumed by the selected operator relation and neither remains independently cleanup-bearing. This relation applies to the held comparison-selected exact `T` of represented comparisons—`Bool` only for Boolean equality/inequality or one admitted fixed-width integer type for integer equality/inequality/strict-less-than—the held exact fixed-width integer value of plain multiplication/addition/subtraction/exclusive-or/bitwise OR, and the held exact `F16`/`F32`/`F64` value of same-format floating multiplication, division, addition, or subtraction under that operation occurrence's independently established numeric contract. Unary Boolean negation, plain fixed-width integer negation, and plain fixed-width integer bitwise complement establish no held-left transient and consume their one produced operand immediately after successful operand production. Boolean short-circuit conjunction also establishes no held-left transient across right evaluation: its successful left Bool is consumed for selection first, so right fault/divergence has no conjunction-owned left value to clean or retain.
...
An owned transient return result is outside callee activation-local cleanup after successful result evaluation because ownership has been separated for caller transfer. For a bounded contract-bearing Shared-reference result, that transient owns exactly the produced result carrier whose contract-specific target/authority/provenance or direct-parent evidence has already satisfied `references.md`; that evidence is validation state and not an additional cleanup-bearing runtime object. No replacement-capable or raw-pointer return transient is source-valid in this slice.

A successfully produced whole-binding assignment, bounded field assignment, complete-referent replacement, or raw-replacement RHS is transferred into its selected target and is not independently remaining after successful replacement. For complete-referent replacement, the destination reference carrier remains independently owned throughout and after RHS transfer. When a whole-binding RHS type is raw-pointer, its exact origin transfers with that value. If RHS production faults before success, producer-specific transient cleanup remains controlling.
...
A transient value produced by consuming a non-duplicable binding-root field is owned by its current transient position after production; its former binding path remains consumed and does not re-enter that binding's frontier if a later producer faults. A non-duplicable `*r` result analogously owns the complete referent value after the exact structural region denoted by that reference becomes consumed; it does not re-enter that region if a later producer faults. For a producer-backed field-value use, the selected transferred path likewise does not re-enter the completed field-receiver transient after its result is preserved.
...
A temporary Shared root `&x` or `&x.field...`, a temporary replacement-capable root `&mut x` or `&mut x.field...`, and a temporary reborrow `&*r`, `&*r.field...`, `&mut *r`, or `&mut *r.field...`, is ordinary producer ownership containing the produced safe-reference carrier until it transfers into its receiving local/argument position or is cleaned by that receiving relation. Root formation carries a fresh root authority targeting its exact selected region; reborrow carries a fresh child authority and a field-relative child retains its exact selected descendant target. Under `SharedIdentity`, a valid Shared root or child passed to the advertised origin slot may round-trip only when the callee preserves that exact incoming authority and target. Under `SharedDirectChild`, the advertised origin slot is replacement-capable; a valid callee result may expose an exact complete-target Shared direct child already created from that designated activation authority, or an identity-preserving transport/forwarding of such a child, but a non-empty field-relative child is projected and cannot satisfy that contract. Neither the Return boundary nor call boundary creates a synthetic child. `*r` produces an ordinary referent value; when its referent is non-duplicable through a replacement-capable reference it additionally consumes exactly the complete structural region denoted by that reference, including a non-empty projected backing path, but creates no new reference carrier category.

This revision defines no general temporary lifetime extension, expression-statement discard, or arbitrary temporary cleanup. Only transient values required by represented record construction, direct-call argument/result transfer, represented eager-binary-operator held-left execution, producer-backed field-value receivers, whole-binding/bounded-field/reference-replacement/raw-replacement transfer, producer-backed record destructuring, and the bounded safe-reference/raw-pointer producer/receiving relations are owned here. The successful Bool condition transient used by represented conditional or bounded-`while` selection is owned and ended by `control-flow.md` after this document's existing producer relation yields it. Boolean conjunction's successful operand results are consumed inside the conjunction relation and add no separate transient category.
...
If a directly called callee diverges, the caller remains suspended at that call and performs no return/fault/loop-transfer cleanup merely because time passes. Safe-reference carriers and corresponding authorities remain live according to their then-current transferred/child relations; every replacement-capable external referent remains in its then-current structural state, which may be unavailable after a callee `*r` Move. No synthetic restoration occurs. A contract-bearing Shared-reference result is normal-return-only, so divergence produces no result carrier/provenance or `SharedDirectChild` result relation and initializes no receiving position. In particular, any still-live designated parent carrier remains with its current suspended owner; if earlier completed transport or nested-call semantics already left the activation-origin authority carrierless, divergence does not recreate a carrier. Divergence neither synthesizes a result child nor performs new cleanup/restoration. Raw pointers cannot be transferred to or returned from that callee in this slice.

If the right operand of a represented eager binary operator diverges after left success, the operator remains suspended in that right producer. The held left value and completed source-state consequences remain owned/effective; no operator result or held-left cleanup occurs merely because time passes. If the left operand diverges, right evaluation never begins and no held left operand exists. This applies equally to represented comparisons under their comparison-selected Bool or fixed-width integer `T`, plain fixed-width integer multiplication/addition/subtraction/exclusive-or/bitwise OR, and same-format floating multiplication/division/addition/subtraction under any selected contract.
...
Safe root formation, explicit bounded reborrow, and an already admitted complete-referent dereference have no divergence point after source validation. A non-duplicable complete-referent dereference performs its Move at that producer position and then yields the owned value. Divergence may occur in a surrounding receiving producer after such a value or reference carrier has already become a transient; in that case its ownership/authority remains exactly where transfer had reached and is not implicitly restored or shortened. For a projected replacement-capable child, ending or suspending the child branch does not synthesize structural restoration of a path already consumed through the child.
...
If the RHS of bounded binding-root field assignment diverges, the operation remains suspended in that source producer. No target `frontier(p)` cleanup, field installation, consumed-path reset, or synthetic target restoration occurs merely because time passes. Any source-state transitions already completed within the RHS remain effective.

If the RHS of complete-referent replacement diverges, the operation remains suspended in that source producer. The destination reference carrier remains owned at the replacement statement, but no applicable referent-frontier cleanup, replacement, structural installation/reset, or synthetic target restoration occurs merely because time passes. Any source-state transitions already completed within the RHS remain effective. This applies identically when the stored replacement-capable reference denotes a non-empty local-backed or external-backed projected target.
...
Left-to-right constructor evaluation, left-to-right argument evaluation plus post-production safe-reference call-entry validation, source-first whole-binding-assignment RHS evaluation, source-first bounded binding-root field-assignment RHS evaluation, source-first complete-referent-replacement RHS evaluation, source-first raw-replacement RHS evaluation, raw address formation and raw ownership move at their producer positions, safe root formation, complete-referent dereference and explicit bounded reborrow at their producer positions, Boolean logical-negation, plain fixed-width integer-negation, and plain fixed-width integer-complement operand evaluation before their respective result transformations, eager left-to-right plain integer-multiplication/addition/subtraction/exclusive-or/bitwise-OR, same-format floating-multiplication/floating-division/floating-addition/floating-subtraction under each occurrence's selected contract, effect-free comparison operand-type selection followed by eager comparison operand evaluation, Boolean short-circuit conjunction left evaluation and selection before any permitted right evaluation, producer-backed field receiver evaluation before selected-field production, producer-before-pattern evaluation, **depth-first explicit pattern binding-leaf source order**, and concrete body/block statement sequencing fix relative source ordering for any effects that future accepted operation owners make observable. A node-local rest marker contributes no effect position or binding-leaf order item.

Safe root formation creates one fresh source authority/carrier at its producer position without reading, moving, mutating, or consuming the target value. Complete-referent dereference accesses the referent under the retained capability selected by `references.md`; for replacement-capable non-duplicable referents it performs the structural ownership Move at that producer position. Explicit bounded reborrow creates exactly one fresh child authority/carrier at its producer position and delegates capability only over that child's exact target without moving/copying the parent carrier or accessing the target value. A non-empty Shared or replacement-capable selector path changes only the child's selected target/delegation region and exact backing path; it creates no field-value read or ownership transition. Under `SharedIdentity`, result transfer preserves a previously produced authority/carrier relationship across the Return/call boundary and introduces no additional producer/effect position. Under `SharedDirectChild`, the child authority is created only at its actual source-valid zero-selector Shared reborrow producer position, or arrives as the already-produced result of a nested call whose own contract has completed; a non-empty projected child does not satisfy that contract. Return and the outer call boundary only preserve and expose an already-existing valid result child across cleanup. They do not create a child after parent-carrier cleanup, whether that carrier ended earlier through ordinary execution or during final activation cleanup. Those are semantic alias/value-production consequences but introduce no source-visible physical addressing, runtime pointer arithmetic, or source effect system.

Bounded binding-root field assignment first evaluates its RHS after the pre-RHS target/mutability/path/accessibility/Exclusive facts are established. Only after successful RHS production and the post-RHS subpath-installation plus exact-path Exclusive facts are satisfied does it clean the then-current `frontier(p)` and install the new exact field value. It does not snapshot a stale field value, generalize the target into a place/lvalue, or move cleanup before source production.

Complete-referent replacement begins only after the destination reference carrier is live and its authority retains full replacement-capable exclusive capability over the reference's exact complete target. It then evaluates its RHS source-first while retaining that destination carrier, revalidates live/full authority after normal RHS success, and only then performs the applicable then-current referent-frontier cleanup plus complete-root reset or canonical non-empty subpath installation. For a projected replacement-capable child, that non-empty installation occurs at the exact path in the existing local or external backing structural root and does not create or normalize a second root. It is not authority to bypass an initially incompatible child delegation, restore structural ownership merely because a child ends, or use a stale snapshotted destination after the RHS disables the destination reference.
...
Literal evaluation has no source-visible side effect under `literals.md`; adding literals to represented ordinary value positions therefore adds no competing effect-order relation. Represented operators similarly add no operator-local side effect after all operand values required by the selected successful semantic path have been produced under `operators.md`. Every represented eager binary operator evaluates its right producer after left success regardless of the left semantic value. Boolean short-circuit conjunction is distinct: after left success it consumes the left Bool for selection, skips the right producer when left is `false`, and evaluates the right producer exactly once only when left is `true`. Represented comparisons under their selected Boolean equality/inequality, fixed-width integer equality/inequality, or fixed-width integer strict-less-than relation, plain integer negation, integer complement, multiplication, addition, subtraction, exclusive-or, bitwise OR, and same-format floating multiplication, division, addition, and subtraction are non-faulting/non-diverging after their required operand values exist, regardless of a governed floating occurrence's selected numerical result set. For floating division, this includes signed-zero divisors and NaN-class outcomes: those are ordinary numerical results, not fault/effect branches.
...
`concrete-syntax.md` owns represented concrete grammar, including bounded Shared `&T`, replacement-capable `&mut T`, Shared root `&x`/`&x.field...`, replacement-capable root `&mut x`/`&mut x.field...`, complete-referent `*r`, explicit Shared children `&*r`/`&*r.field...`, explicit replacement-capable children `&mut *r`/`&mut *r.field...`, whole-binding `x = Value;`, bounded binding-root field assignment `x.field... = Value;`, and complete-referent replacement `*r = Value;`; activation-local `raw T`, `raw &x`, contextual `raw move p`, contextual `raw assign p = Value;`, and `unsafe { ... }`; the existing `-> &T` result spelling consumed by bounded safe-reference result-contract admission while `-> &mut T` remains semantically result-invalid; bounded contextual ordinary/conditional grouping; the operation-local `@fast(Value)` numeric-contract-selected value; the bounded Boolean logical-negation, plain fixed-width integer-negation, and plain fixed-width integer-complement prefix placements with signed-literal priority for `-`; the bounded multiplicative tier whose binary `*` form selects plain integer multiplication or same-format floating multiplication by exact required type and whose `/` form selects same-format floating division only under exact floating required type; the bounded additive tier whose `+` form selects plain integer addition or same-format floating addition and whose binary `-` form selects plain integer subtraction or same-format floating subtraction by exact required type; the bounded plain integer-exclusive-or tier; the bounded plain integer-bitwise-OR tier; the bounded non-associative comparison tier whose `==`, `!=`, and `<` semantic relation is selected only after effect-free comparison evidence establishes exact operand type `T`; the bounded Boolean short-circuit-conjunction tier; unqualified and qualified record-construction targets; record-pattern heads and bounded node-local rest marker; the bounded statement-level `while`; bounded unlabeled `break;`/`continue;`; and the payload-free explicit-fault statement. The comparison grammar itself is type-neutral and introduces no type-driven parse decision. `references.md` owns exact safe-reference type identity/admission, root and parent-relative target/authority/carrier/lifetime facts, canonical compatibility, source-validation origin provenance, complete-referent dereference semantics, explicit bounded reborrow/delegation, complete-referent replacement preconditions/effects, parameter/result-call transfer obligations, external-referent structural roots, normal restoration, bounded safe-reference result-contract Return validity/caller summary, and exact source/Core reference refinement. `raw-pointers-unsafe.md` owns raw-pointer type/value/origin validity, address formation, RawMove/RawAssign preconditions and target effects, lexical unsafe admission, and exact source/Core raw refinement. `operators.md` owns represented operator operand/result typing, selected numeric-contract facts, comparison operation selection, semantic value transformations, operator-local ownership consequences, and operation-specific source/Core refinements. `literals.md` owns boolean/integer/decimal floating materialization and the preserved signed-literal semantic boundary. `structural-ownership.md` owns structural paths/state/availability/frontiers plus bounded non-empty subpath installation for binding and external-referent roots. `field-access.md` owns binding-root and bounded producer-backed receiver selection, bounded binding-root field-assignment target selection, direct field accessibility consumed by field selection, construction initializers, safe-reference selector resolution, and explicitly selected recursive pattern fields, source-selected final-field duplicate-or-consume production, and producer-receiver remaining-frontier facts. `patterns.md` owns recursive record-pattern head resolution, explicit field/rest structure, binding-leaf facts/order, direct-root ownership production, producer-transient ownership transitions, omission consequences, and pattern-transient frontier selection. `local-bindings.md` owns binding identity/scope/lookup/mutability/lifecycle, whole-binding use/assignment legality, and bounded binding-root field-assignment legality. `control-flow.md` owns represented conditional selection/arm validation/definite normal binding/external-referent structural ownership/raw-pointer origin, bounded-`while` condition selection/backedge-state admission/post-loop state, and nearest-loop break/continue target/state admission.

Additional operators beyond Boolean logical negation, plain fixed-width integer negation/bitwise-complement/multiplication/addition/subtraction/exclusive-or/bitwise-OR/equality/inequality/strict-less-than, same-format floating multiplication/division/addition/subtraction, Boolean equality/inequality, and Boolean short-circuit conjunction, general expressions beyond the bounded safe-reference/raw-pointer producers, grouping/selected-value wrappers and prefix/multiplicative/additive/exclusive-or/bitwise-OR/comparison/logical-conjunction tiers, arbitrary assignment places beyond the represented whole-binding and bounded binding-root field targets, additional loop forms, labeled transfers, transfer values, unrestricted nonterminal-within-block return, arbitrary-receiver members, additional refutable/shorthand pattern categories, additional producer-backed scrutinee families, broader raw-pointer operations, unsafe callable contracts, and other source forms remain outside this execution relation.

The represented operators, bounded safe-reference producers/reborrow/replacement/result transfer, bounded raw-pointer producers/replacement/unsafe admission, grouping wrapper, numeric-contract-selected wrapper, construction, bounded producer-backed field-value, bounded binding-root field assignment, recursive pattern including bounded node-local omission, partial-ownership cleanup, return, explicit-fault, bounded-loop-transfer cleanup, bounded-`while` execution sequencing, and existing producer execution relations are defined entirely by source identities, structural ownership, safe authority/carrier/delegation identity, source Shared-result provenance and activation-origin evidence where applicable, exact raw-pointer origin provenance where applicable, owned values, source order, transfer, transient ownership, local normal-continuation presence, fault-reason identity, operator-local value semantics, selected numeric-contract facts, and cleanup. Grouping, numeric-contract selection, rest, and unsafe-block admission add no new dynamic item to that list beyond their specifically owned static semantic facts. This execution owner does not redefine Core operation, destruction, reference, pointer, or unsafe semantics. Plain integer negation and plain integer complement each consume one existing Core `IntegerSub` refinement after complete operand production through `operators.md`; plain integer multiplication consumes the one represented Core `IntegerMul` relation, same-format floating multiplication consumes the distinct represented Core `FloatMul` relation with its explicit selected contract, same-format floating division consumes the distinct represented Core `FloatDiv` relation with its explicit selected contract, plain integer addition consumes the distinct represented Core `IntegerAdd` relation, same-format floating addition consumes the distinct represented Core `FloatAdd` relation with its explicit selected contract, plain integer subtraction consumes the distinct represented Core `IntegerSub` relation, same-format floating subtraction consumes the distinct represented Core `FloatSub` relation with its explicit selected contract, plain integer exclusive-or consumes the distinct represented Core `IntegerXor` relation, and plain integer bitwise OR consumes the distinct represented Core `IntegerOr` relation through `operators.md`; Boolean equality/inequality retain their existing accepted comparison CFG, fixed-width integer equality consumes one accepted Core `IntegerEq` after complete source operand lowering with its explicit retained exact comparison-selected operand type, fixed-width integer inequality consumes that same one `IntegerEq` followed by the accepted Boolean-negation refinement without a second source operand evaluation, and fixed-width integer strict less-than consumes one accepted Core `IntegerLt` after complete source operand lowering with the same explicit retained exact comparison-selected operand type. Safe-reference forms consume exactly the accepted Core Shared/ExclusiveReplace reference and bounded identity-preserving/direct-child Shared-result/external-referent relations through `references.md`. Raw forms consume exactly the accepted Core raw-pointer/unsafe/storage/alias relations through `raw-pointers-unsafe.md`. Any source-to-Core lowering must refine these source requirements and the separately owned reference/raw/operator/conditional/loop/transfer requirements through accepted Core semantics rather than use Core representation behavior as source authority.
...
After source validation, the bounded safe-reference forms refine only through the exact mapping owned by `references.md`: `SharedRef(T)` to canonical Core Shared reference of `lower(T)`; `ExclusiveReplaceRef(T)` to canonical Core `ExclusiveReplace` reference of `lower(T)`; Shared root `&x`/`&x.field...` to the existing Core `ReferenceRoot` relation with Shared permission over direct storage representing `x` followed by the retained exact resolved field-projection sequence; replacement-capable root `&mut x`/`&mut x.field...` to the same existing Core root-reference relation with `ExclusiveReplace` permission over direct storage representing `x` followed by the retained exact resolved field-projection sequence; ordinary Shared duplication to Core `Copy` of the carrier-bearing reference value; replacement-capable ordinary use to ownership-transferring Move of the non-copyable carrier; bounded `*r` to reference-relative Core `Copy` for Shared/duplicable access or Core `Move` for admitted replacement-capable non-duplicable complete-referent access over the reference's exact selected target; zero-selector `&*r` and zero-selector `&mut *r` to the existing Core child-authority construction over a zero-projection `ReferenceAccess`; bounded `&*r.field...` and `&mut *r.field...` to the same Core `ReferenceReborrow` relation with respectively Shared and `ExclusiveReplace` permission and the exact retained relative field-projection sequence on the existing parent-carrier `ReferenceAccess`; `*r = Value;` to source-first reference-relative replacement using retained full `ExclusiveReplace` authority and the existing projected `ReferenceAccess` for the stored reference's exact target; safe-reference parameters to existing Core parameter slots; each replacement-capable parameter to the accepted Core external-referent call-entry/postcondition relation; and the three source safe-reference result-contract states to the corresponding Core `SafeReferenceResultContract::{None, SharedIdentity, SharedDirectChild}` variant with the same designated origin slot where present. For either a Shared or replacement-capable field root, retained source field identity/order and final source type map directly to the Core direct-place projection sequence. For either a Shared or replacement-capable field-relative child, retained relative field identity/order and final selected source type map directly to the Core `ReferenceAccess` projection sequence rooted at the existing stored parent carrier with the exact child permission. Lowering MUST NOT reconstruct an originating root place from parent provenance or materialize a field value, target load, parent reference `Copy`/`Move`, synthetic root borrow/reference, raw pointer/address operation, authority detach/re-root, or result-contract operation merely to realize either projected target. A source-valid `SharedRef(T)` result refines to the same Core Shared-reference result type while preserving that exact contract variant and slot. Source cleanup/result preservation refines to existing Core carrier-aware cleanup and result transfer. Source origin provenance, direct-parent derivation, activation result-origin evidence, exact root/relative target path, structural backing root/path relation, and external-referent structural state remain source validation/refinement evidence and need not become separate runtime value fields. Source binding identity, root/relative-target eligibility, field accessibility, referent admission, lexical validity, contextual type admission, target-relative delegation, call-entry/full-capability validity, normal restoration, and advertised result-contract validity MUST be established before lowering and MUST NOT be reconstructed from Core `LocalId`, `StorageRegion`, `ReferenceAuthorityId`, scalar liveness, path state, or runtime machine behavior. No Core semantic, IR, validator, or reference-runtime extension is required for this bounded projected-root/relative-reborrow refinement.
...
A source return may refine to the existing Core `Return` terminator from whichever lower block represents that return point. For a bounded contract-bearing `SharedRef(T)` result, this refinement is valid only after source validation has proved the exact safe-reference result contract required by `references.md`; the source callable's three-state descriptor maps unchanged to the corresponding Core `SafeReferenceResultContract` variant and designated origin slot. `SharedIdentity` therefore remains identity-preserving, while `SharedDirectChild` remains exact direct-child-bearing rather than being collapsed into identity. Lowering MUST preserve an already-produced direct-child result and MUST NOT introduce a synthetic reborrow, fresh authority, or return-boundary `Copy` to satisfy either contract. Replacement-capable and raw-pointer results are not source-admitted. Ordinary normal lexical-scope cleanup and ordinary normal `Goto` continuation are emitted only for source paths that actually have a local normal continuation; a returning path does not require a synthetic normal join/backedge. Source local normal-continuation presence, source cleanup selection, safe-reference result-contract validity, replacement-capable external-referent restoration, and any raw-pointer origin validity MUST be established before lowering and MUST NOT be reconstructed from Core reachability, path-state worklists, scalar liveness, initialization state, pointer metadata, or coincidental runtime aliasing.
...
This revision does not define other literal semantics beyond the represented boolean, decimal integer, and decimal floating families; arithmetic beyond plain same-type fixed-width integer negation/multiplication/addition/subtraction and same-format floating multiplication/division/addition/subtraction, binary bitwise operations or shifts beyond the represented unary fixed-width integer bitwise complement and binary fixed-width integer exclusive-or/bitwise-OR, floating unary negation or floating remainder, standalone fused arithmetic, unary plus, increment/decrement, integer ordering beyond represented same-exact-type fixed-width integer strict less-than, another comparison beyond represented equality/inequality/strict-less-than, `<=`, `>`, `>=`, three-way comparison, floating equality/inequality/ordering, nominal-record or structural equality/inequality/ordering, safe-reference or raw-pointer equality/inequality/ordering, function or generic equality/comparison/ordering, comparison operators beyond exact-Bool equality/inequality and same-exact-type fixed-width integer equality/inequality/strict-less-than relations, operator forms beyond Boolean negation/plain integer negation/integer complement/integer multiplication/floating multiplication/floating division/integer addition/same-format floating addition/integer subtraction/floating subtraction/integer exclusive-or/integer bitwise OR/plain fixed-width integer equality/inequality/strict-less-than/Boolean equality/inequality/Boolean short-circuit conjunction, source `standard` or `reproducible` selector spellings, block/function/module numeric-contract scopes or defaults, lexical/dynamic contract inheritance, caller-to-callee contract propagation, generic annotations/attributes/pragmas, additional numeric-contract-selected operation families beyond represented floating multiplication/division/addition/subtraction, short-circuit logical operators beyond represented conjunction, compound assignment, assignment-as-value, conditional expressions, unequal-state/path-dependent two-normal-outcome conditional joins, unrestricted nonterminal-within-block return or arbitrary unreachable tails, additional loop forms (`loop`, `for`, do/while), loop `else`, labels or a label namespace, labeled break/continue, transfer values, loop values, general loop fixed-point inference, refutable-match control flow, field/place assignment or partial-field reinitialization beyond the represented bounded binding-root field relation, arbitrary value/expression field-assignment receivers, arbitrary value/expression field-value receivers beyond the bounded direct-call/record-construction receiver set, general postfix/member/method access, additional refutable/shorthand/wildcard/literal/guard/alternative patterns, producer-backed pattern scrutinees beyond direct calls/record constructions/field-value uses, grouped or other general expression scrutinees, destructuring assignment, qualified binding leaves/qualified field names/nested module paths beyond the represented alias-member pair, additional field accessibility classes beyond the represented module-private/exported relation, safe-reference semantics beyond bounded Shared and replacement-capable complete-root/binding-field root references, bounded Shared and replacement-capable complete/field-relative reborrow, and complete-referent replacement forms (including plain Core `Exclusive`, direct reference-relative field access such as `*r.field`, producer/transient reference targets, explicit source reference Drop, interior assignment beyond bounded complete-referent replacement, reference-containing record fields or aggregate results, replacement-capable results, nested references, mutable/rebindable reference locals, projected/subregion or arbitrary-descendant Shared result contracts, multiple/explicit/static Shared result origins, explicit result-contract/result-origin selector syntax, named lifetime/outlives syntax, or non-lexical authority shortening), raw-pointer semantics beyond the bounded activation-local `RawPtr(T)` / `raw &x` / `raw move p` / `raw assign p = Value;` / lexical `unsafe` relation (including raw-pointer parameters/results/aggregates, pointer-to-pointer/reference types, null/fabricated/numeric pointers, RawRead source exposure, non-consuming owned raw load, field/path raw addresses, pointer arithmetic/comparison/conversion, target-sized integers, physical layout/address/alignment/stability/pinning, heap/global/static raw storage, unsafe function/callable/caller contracts, user proof contracts, or reference-to-raw conversion), indirect calls/function values/closures, generics/traits/coherence, async/tasks or Exec call semantics, effect-system completion, fault payload/message/code values, panic/throw syntax, catch/recovery, ABI/calling convention/FFI/linkage, parser/HIR/Core MIR production code, or backend behavior.
