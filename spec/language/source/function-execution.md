...
If callee execution yields defined fault, the direct-call producer yields that same fault and produces no result value, result carrier, result provenance, or direct-child result relation. A surrounding local/assignment/reference-replacement/return/argument receiving position is therefore not initialized or transferred a result on that path.
