...
A safe root-reference producer receives its surrounding exact required safe-reference type and applies the root-target, referent-admission, availability, mutability where applicable, lexical-extent, and canonical authority-compatibility rules from `references.md`. For a Shared root form, source validation completes the root lookup and, when present, every field-selector resolution/accessibility step, exact final referent type/admission, selected-path full availability, containing binding extent, and canonical Shared compatibility before committing any fresh authority/carrier. `&x` is the zero-selector case; `&x.field...` produces one fresh Shared root authority/carrier targeting exactly the resolved structural path. Root `&mut x` remains complete-root-only and produces one fresh replacement-capable exclusive authority/carrier only for an admitted mutable ordinary local. Successful root formation reads, copies, moves, replaces, or otherwise accesses no target value, changes no structural ownership state, and is non-faulting/non-diverging after source validation.
