...
- delegates to the accepted external-call relation in `external-calls.md`, which may normally resume at its represented continuation or diverge without creating a Core function activation;
...
- a represented external call contributes its normal continuation edge after external-call argument effects and normal-result initialization facts under `external-calls.md`;
...
A represented Core control-flow graph MAY contain cycles formed by `Goto`, `Branch`, normal represented-call continuation edges including external-call continuation edges, or combinations of those edges where the applicable owner permits them.
