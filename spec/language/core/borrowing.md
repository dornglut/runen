...
`Init` and the direct destination of represented `IntegerAdd` are also exclusive-authority direct-storage operations.

`IntegerAdd` has no loan-relative destination form in this revision. Its `dst: Place` is authorized only by the direct-access rule below. Its `left` and `right` operands separately retain whatever shared/exclusive authority their existing `Operand` forms require; classifying the destination as exclusive does not upgrade, replace, or duplicate operand access authority.
...
- `Init(p, ...)`, `IntegerAdd { dst: p, ... }`, `Move(p)`, ordinary `Assign(p, ...)`, and `Drop(p)` are permitted only when no active loan of either kind overlaps `p`, in addition to their existing value/storage preconditions.

The direct-access rules apply structurally. Loan delegation never weakens these constraints.

The `IntegerAdd` rule above applies only to its direct destination. It does not make an active shared or exclusive loan a legal `IntegerAdd` destination, because this revision defines no loan-relative destination form for that operation. A left or right `IntegerAdd` operand that uses an existing `Operand` place access is checked independently under that operand form's ordinary direct or loan-relative authority rules after destination admission.
