...
Because `dst` was wholly vacant at admission, the result write has an empty destruction domain and performs no replacement destruction. The write has the same stored-value-lifetime and storage-instance consequences as successful `Init`, `IntegerAdd`, `IntegerSub`, and `IntegerMul`: it begins the destination stored-value lifetime without changing the destination storage extent or storage-instance identity.
...
This semantic spelling identifies the represented Core operation. It does not require one particular implementation enum, generic numeric opcode, instruction encoding, backend opcode, or physical machine instruction. The `contract` field is one semantic fact of the represented operation; it is not a backend hint, ambient mode, or runtime floating-environment state.

Let `D` be the exact Core type identity of `dst`. A valid `FloatAdd` requires all of the following static facts before execution:

- `D` denotes a scalar type whose scalar kind is exactly one of `F16`, `F32`, or `F64`;
- `contract` is exactly one member of the accepted Core floating numeric-contract domain `Standard | Reproducible | Fast`;
- every place-derived left or right operand produces a value whose selected Core type identity is exactly `D`;
- a constant operand is admitted only when its semantic value matches `D` under the existing type-table value-matching relation; and
- no equality of scalar kind alone makes two distinct Core type identities interchangeable for an operand.
...
`FloatAdd` is governed by the numeric-contract authority in [Core floating-point semantics](numerics/floating-point.md). Its explicit `contract` field establishes the selected contract for exactly this represented operation before realization. There is no absent, implicit, or parallel default-only contract state on represented `FloatAdd` after this cutover. Independently authored Core may select any accepted member of the complete contract domain; a source language may expose only a strict subset of those selections. Any result-changing restructuring involving multiple represented floating-operation occurrences remains governed by the mixed-contract composition authority in `numerics/floating-point.md`; this storage owner adds no second composition rule.
...
3. evaluate `right` completely under its existing `Operand` semantics with required Core type identity `D` in the resulting state;
4. preserve all state consequences of that operand evaluation;
5. consume the two produced semantic binary-floating operand values and select exactly one result permitted by the accepted same-format floating-addition relation for `contract` in `numerics/floating-point.md` for the scalar kind of `D`;
...
The floating owner remains the sole numerical authority for every result permitted by the selected contract, including finite normal/subnormal results, signed-zero selection, binary rounding, finite/infinity boundaries, signed infinities, and NaN-class outcomes. This storage relation does not restate, narrow, or extend any contract-specific numerical permission. When that owner requires only that a result belong to `D`'s NaN value class, `FloatAdd` may store any semantic NaN member permitted by that relation. This storage relation does not select a canonical member, payload, sign, quiet/signaling identity, physical encoding, equality identity, or another member-sensitive property.
...
After both operand values have been produced, the floating-addition/result-write step is finite, non-faulting, and non-diverging under the represented Core semantics. Any numerical result selected by the floating owner under `contract`, including NaN, signed infinity, signed zero, subnormal, or normal outcomes where applicable, is an ordinary numerical result rather than a Core `Fault`, undefined behavior, panic, exception, or alternate control-flow outcome. The operation introduces no new borrow interval, reference value, pointer provenance, cleanup category, storage identity, assignment-mutability rule, interior-mutability rule, fault reason, control-flow edge, floating exception/status state, layout/ABI promise, runtime numeric mode, or backend-visible semantic fact.

The operation consumes only the already accepted same-format floating-addition result relation for its explicit `contract` and the existing numeric-contract authority from `numerics/floating-point.md`. It does not represent floating subtraction, multiplication, division, negation, comparison, conversion, source numeric-contract syntax or scoping, a NaN literal, a generic floating operation family, constant folding, or a physical floating representation.
...
Represented integer-operation result values are determined by their separately owned integer relations. A represented `FloatAdd` selects exactly one numerical result from the separately owned same-format floating-addition relation for its explicit `contract`. The exact numerical result need not be unique where `numerics/floating-point.md` explicitly permits a result set, including its existing class-only NaN latitude and any selected-contract numerical latitude. Such permitted variation is numerical semantic latitude owned exclusively by `numerics/floating-point.md`; it is not hidden storage, loan, scheduling, backend, physical-encoding, host-floating, or container-iteration state. Operation admission, operand sequencing, result-write occurrence, stored-value lifetime transition, and abnormal-completion behavior remain deterministic independently of which permitted numerical result is selected.
