...
`Init` and represented `IntegerAdd`/`IntegerSub`/`IntegerMul`/`IntegerXor`/`IntegerOr`/`IntegerEq`/`IntegerLt`/`FloatAdd`/`FloatSub`/`FloatMul`/`FloatDiv` result initialization may begin either the first stored-value lifetime or a later stored-value lifetime in vacant storage. Ordinary `Assign` and `InteriorAssign` may end old stored-value lifetimes and begin replacement lifetimes in the same storage extent and the same dynamic storage instance.
...
## Plain fixed-width integer equality result initialization

The represented Core proving relation contains one distinct fixed-width integer-equality operation, normatively written here as:

```text
IntegerEq {
    dst: Place,
    operand_type: exact Core type identity,
    left: Operand,
    right: Operand,
}
```

This semantic spelling identifies the represented Core operation. It does not require one particular implementation enum, field name, integer-predicate family, instruction encoding, backend opcode, or physical machine instruction. The mandatory `operand_type` field denotes one exact per-program Core type identity under the type relation defined above; it is not an inferred scalar kind, a source type, a runtime value, or optional metadata.

Let `B` be the exact Core type identity of `dst` and let `I` be the exact Core type identity carried by `operand_type`. A valid `IntegerEq` requires all of the following static type facts before execution:

- `B` denotes a scalar type whose scalar kind is exactly `Bool`;
- `I` denotes a scalar type whose scalar kind is exactly one of `I8`, `I16`, `I32`, `I64`, `U8`, `U16`, `U32`, or `U64`;
- every place-derived left or right operand produces a value whose selected Core type identity is exactly `I`;
- a constant operand is admitted only when its semantic value matches `I` under the existing type-table value-matching relation; and
- no equality of scalar kind alone makes two distinct Core type identities interchangeable for an operand.

The explicit operand type is semantically necessary because the Bool destination type `B` cannot supply the integer type required by the operands, while the current constant-value representation carries no independent exact type identity. Two constant operands are therefore admitted only relative to the explicit `I`; a place/constant or constant/place pair uses exactly the same required `I`. The operation does not infer `I` from the first place operand, from a constant's scalar variant, from `B`, or from implementation convenience.

An `AddressOf` operand therefore cannot satisfy this operation's fixed-width integer operand requirement. Any applicable `Move`, `Copy`, `RawMove`, `ReferenceMove`, `ReferenceCopy`, or other represented operand form retains its own existing access, liveness, copyability, provenance, reference-authority, and ownership-transfer requirements while also producing the exact required type `I`. This operation introduces no conversion, promotion, coercion, widening, narrowing, defaulting, overload relation, generic numeric type, or operand-derived type inference.

`IntegerEq` reuses the non-replacing destination lifecycle of `Init` and the existing represented result operations rather than defining another storage model. Before either operand is evaluated:

1. resolve `dst` as direct storage under the existing place/type rules and establish exact destination type `B`;
2. establish the explicit exact operand type identity `I` and require the static destination/operand type facts above;
3. require the exclusive direct-storage authority that the existing non-replacing initialization/result operations require under `borrowing.md`; and
4. require `dst` to be wholly vacant.

These destination and exact-type preconditions are fixed at operation admission before left-operand evaluation. Operand evaluation cannot make an initially Live, insufficiently authorized, non-Bool, or otherwise invalid destination retrospectively admissible, and it cannot change the selected `I` for that same operation. Ordinary assignment mutability is not required, exactly as for the existing non-replacing result destinations.

After destination admission, execution is exactly:

1. evaluate `left` completely under its existing `Operand` semantics with required exact Core type identity `I`;
2. preserve all state consequences of that operand evaluation;
3. evaluate `right` completely under its existing `Operand` semantics with required exact Core type identity `I` in the resulting state;
4. preserve all state consequences of that operand evaluation;
5. consume the two produced semantic fixed-width integer operand values to compute the exact equality result owned by `numerics/integers.md` for the scalar kind of `I`;
6. write exactly that one semantic Bool result into `dst`; and
7. mark the written scalar destination Live, beginning a new stored-value lifetime there.

The left operand is therefore evaluated before the right operand. `Move`, `Copy`, raw-pointer operand access, safe-reference operand access, and all other operand-local effects remain exactly those of the existing operand relation; `IntegerEq` does not duplicate or weaken them. A source place moved by the left operand is already Dead when the right operand is evaluated. A right operand cannot read, copy, or move such a place unless an independent represented operation has legally restored its value beforehand.

Because `dst` was wholly vacant at admission, the result write has an empty destruction domain and performs no replacement destruction. The write has the same stored-value-lifetime and storage-instance consequences as successful non-replacing `Init` and the existing represented result operations: it begins the Bool destination stored-value lifetime without changing the destination storage extent or storage-instance identity.

After both operand values have been produced, the equality/result-write portion is finite, deterministic, non-faulting, and non-diverging under the represented Core semantics. The operation introduces no new borrow interval, reference value, pointer provenance, cleanup category, storage identity, assignment-mutability rule, interior-mutability rule, fault reason, control-flow edge, overflow classification, numeric contract, layout/ABI promise, runtime flag, or backend-visible semantic fact.

The operation consumes only the fixed-width same-type integer equality relation from `numerics/integers.md`. It is a value-producing statement, not a terminator or an alternate form of `Branch`. It does not represent integer inequality, ordering, three-way comparison, floating comparison, pointer/reference comparison, structural comparison, conversion, a generic comparison/predicate operation family, source operator syntax, or a constant-folding requirement.

## Plain fixed-width integer strict-order result initialization

The represented Core proving relation contains one distinct fixed-width integer strict-less-than operation, normatively written here as:
...
Let `B` be the exact Core type identity of `dst` and let `I` be the exact Core type identity carried by `operand_type`. A valid `IntegerLt` requires all of the following static type facts before execution:
...
The explicit operand type is semantically necessary for the same reason as `IntegerEq`: the Bool destination type `B` cannot supply the integer type required by the operands, while the current constant-value representation carries no independent exact type identity. Two constant operands are therefore admitted only relative to the explicit `I`; a place/constant or constant/place pair uses exactly the same required `I`. The operation does not infer `I` from the first place operand, from a constant's scalar variant, from `B`, or from implementation convenience.

An `AddressOf` operand therefore cannot satisfy this operation's fixed-width integer operand requirement. Any applicable `Move`, `Copy`, `RawMove`, `ReferenceMove`, `ReferenceCopy`, or other represented operand form retains its own existing access, liveness, copyability, provenance, reference-authority, and ownership-transfer requirements while also producing the exact required type `I`. This operation introduces no conversion, promotion, coercion, widening, narrowing, defaulting, overload relation, generic numeric type, or operand-derived type inference.

`IntegerLt` reuses the non-replacing destination lifecycle of `Init`, `IntegerEq`, and the existing represented result operations rather than defining another storage model. Before either operand is evaluated:
...
3. evaluate `right` completely under its existing `Operand` semantics with required exact Core type identity `I` in the resulting state;
4. preserve all state consequences of that operand evaluation;
5. consume the two produced semantic fixed-width integer operand values to compute the exact strict-order result owned by `numerics/integers.md` for the scalar kind of `I`;
6. write exactly that one semantic Bool result into `dst`; and
7. mark the written scalar destination Live, beginning a new stored-value lifetime there.

The left operand is therefore evaluated before the right operand. `Move`, `Copy`, raw-pointer operand access, safe-reference operand access, and all other operand-local effects remain exactly those of the existing operand relation; `IntegerLt` does not duplicate or weaken them. A source place moved by the left operand is already Dead when the right operand is evaluated. A right operand cannot read, copy, or move such a place unless an independent represented operation has legally restored its value beforehand.

Because `dst` was wholly vacant at admission, the result write has an empty destruction domain and performs no replacement destruction. The write has the same stored-value-lifetime and storage-instance consequences as successful non-replacing `Init`, `IntegerEq`, and the existing represented result operations: it begins the Bool destination stored-value lifetime without changing the destination storage extent or storage-instance identity.

After both operand values have been produced, the strict-order/result-write portion is finite, deterministic, non-faulting, and non-diverging under the represented Core semantics. The operation introduces no new borrow interval, reference value, pointer provenance, cleanup category, storage identity, assignment-mutability rule, interior-mutability rule, fault reason, control-flow edge, overflow classification, numeric contract, layout/ABI promise, runtime flag, or backend-visible semantic fact.

The operation consumes only the fixed-width same-type integer strict-order relation from `numerics/integers.md`. It is a value-producing statement, not a terminator or an alternate form of `Branch`. It does not represent non-strict less-than, greater-than, greater-than-or-equal, three-way comparison, floating comparison, pointer/reference comparison, structural comparison, conversion, a generic comparison/predicate operation family, source operator syntax, a range relation, or a constant-folding requirement.

## Binary floating addition result initialization
...
`FloatAdd` is governed by the numeric-contract authority in [Core floating-point semantics](numerics/floating-point.md). Its explicit `contract` is the already-established selected contract for this operation occurrence and is consumed exactly as semantic input by that owner. This storage relation does not redefine the contract's baseline, refinement, reassociation, contraction, subnormal, NaN, rounding, or other numerical rules. Independently authored Core may represent any accepted `Standard`, `Reproducible`, or `Fast` FloatAdd whether or not current source syntax can select all three.
...
3. evaluate `right` completely under its existing `Operand` semantics with required Core type identity `D` in the resulting state;
4. preserve all state consequences of that operand evaluation;
5. consume the two produced semantic binary-floating operand values and select exactly one result permitted by the accepted same-format floating-addition relation in `numerics/floating-point.md` for scalar kind `D` under this operation's explicit `contract`;
...
## Binary floating multiplication result initialization

The represented Core proving relation contains one distinct binary floating-multiply operation, normatively written here as:
...
This semantic spelling identifies the represented Core operation. It does not require one particular implementation enum, generic numeric opcode, fused opcode, instruction encoding, backend opcode, or physical machine instruction. `NumericContract` denotes exactly the accepted semantic contract identity domain `Standard | Reproducible | Fast` owned by `numerics/floating-point.md`. Every represented `FloatMul` carries exactly one such explicit contract. This field is semantic input to the floating numerical owner; it is not an ambient numeric mode, target capability, backend flag, runtime floating-environment setting, or second storage-state component.

Let `D` be the exact Core type identity of `dst`. A valid `FloatMul` requires all of the following static type facts before execution:
...
`FloatMul` is governed by the numeric-contract authority in [Core floating-point semantics](numerics/floating-point.md). Its explicit `contract` is the already-established selected contract for this operation occurrence and is consumed exactly as semantic input by that owner. This storage relation does not redefine the contract's baseline, refinement, reassociation, contraction, subnormal, NaN, signed-zero, infinity, rounding, or other numerical rules. Independently authored Core may represent any accepted `Standard`, `Reproducible`, or `Fast` FloatMul whether or not current source syntax can select all three.

`FloatMul` reuses the non-replacing destination lifecycle of `Init` and the represented integer-result, `FloatAdd`, and `FloatSub` operations rather than defining another storage model. Before either operand is evaluated:
...
After destination admission, ordinary `FloatMul` execution is exactly:

1. evaluate `left` completely under its existing `Operand` semantics with required Core type identity `D`;
2. preserve all state consequences of that operand evaluation;
3. evaluate `right` completely under its existing `Operand` semantics with required Core type identity `D` in the resulting state;
4. preserve all state consequences of that operand evaluation;
5. consume the two produced semantic binary-floating operand values and select exactly one result permitted by the accepted same-format floating-multiplication relation in `numerics/floating-point.md` for scalar kind `D` under this operation's explicit `contract`;
6. write that one semantic result into `dst`; and
7. mark the written scalar destination Live, beginning a new stored-value lifetime there.

The left operand is therefore evaluated before the right operand. `Move`, `Copy`, raw-pointer operand access, and all other operand-local effects remain exactly those of the existing operand relation; `FloatMul` does not duplicate or weaken them. A place moved by the left operand is already Dead when the right operand is evaluated. A right operand cannot read, copy, or move such a place unless an independent represented operation has legally restored its value beforehand.

The floating owner remains the sole numerical authority for finite normal/subnormal multiplication results, signed-zero selection, binary rounding, finite/infinity boundaries, signed infinities, NaN-class outcomes including zero-times-infinity, and any selected-contract-specific permitted result variation. When that owner requires only that a result belong to `D`'s NaN value class, `FloatMul` may store any semantic NaN member permitted by that relation. This storage relation does not select a canonical member, payload, sign, quiet/signaling identity, physical encoding, equality identity, or another member-sensitive property.

A semantic NaN value produced at runtime is an ordinary storable value of `D`. Its existence does not create a NaN form in the current constant-value representation and does not make `Operand::Constant` capable of fabricating a NaN. A reference or verification implementation may represent the accepted class-only result with a class-level observation when that abstraction preserves every represented semantic operation; such an assurance carrier is not a semantic NaN member identity.

Because `dst` was wholly vacant at admission, the ordinary result write has an empty destruction domain and performs no replacement destruction. The write has the same stored-value-lifetime and storage-instance consequences as successful non-replacing `Init` and the existing represented result operations: it begins the destination stored-value lifetime without changing the destination storage extent or storage-instance identity.

After both operand values have been produced, the ordinary floating-multiplication/result-write step is finite, non-faulting, and non-diverging under the represented Core semantics. Operation admission, operand sequencing, and the one ordinary result-write occurrence remain deterministic; the selected numerical relation may admit more than one result only where `numerics/floating-point.md` explicitly permits that variation. NaN, signed infinity, signed zero, subnormal, normal, and other contract-permitted outcomes selected by the floating owner are ordinary numerical results rather than a Core `Fault`, undefined behavior, panic, exception, or alternate control-flow outcome. The operation introduces no new borrow interval, reference value, pointer provenance, cleanup category, storage identity, assignment-mutability rule, interior-mutability rule, fault reason, control-flow edge, floating exception/status state, layout/ABI promise, runtime numeric mode, or backend-visible semantic fact.

The ordinary operation consumes only its explicit selected-contract same-format floating-multiplication result relation and existing numeric-contract authority from `numerics/floating-point.md`. That numerical owner separately and exclusively defines any result-changing permission to reassociate an eligible pure finite multiplication region or to contract an eligible finite `FloatMul` result consumed by `FloatAdd`, including the requirement that every participating or affected operation occurrence named by those rules be `Fast` and the rule that `Standard` or `Reproducible` occurrences remain semantic boundaries. This storage relation neither widens nor suppresses those permissions and does not turn them into an alternate per-operation result rule. It defines no fused operation, multiply-subtract form, negated fused variant, exact-zero or special-value contraction, reciprocal replacement, permutation, omission, duplication, approximation, reduced precision, backend aggregate fast-math authority, or ambient numeric state.

## Binary floating division result initialization

The represented Core proving relation contains one distinct binary floating-divide operation, normatively written here as:
...
This semantic spelling identifies the represented Core operation. It does not require one particular implementation enum, generic numeric opcode, reciprocal-multiply lowering, instruction encoding, backend opcode, or physical machine instruction. `NumericContract` denotes exactly the accepted semantic contract identity domain `Standard | Reproducible | Fast` owned by `numerics/floating-point.md`. Every represented `FloatDiv` carries exactly one such explicit contract. This field is semantic input to the floating numerical owner; it is not an ambient numeric mode, target capability, backend flag, runtime floating-environment setting, or second storage-state component.

Let `D` be the exact Core type identity of `dst`. A valid `FloatDiv` requires all of the following static type facts before execution:
...
An `AddressOf` operand therefore cannot satisfy this operation's binary-floating operand requirement. Any applicable `Move`, `Copy`, `RawMove`, or other represented operand form retains its own existing access, liveness, copyability, provenance, and ownership-transfer requirements while also producing the exact required type `D`. This operation introduces no integer division, cross-format conversion, promotion, widening, narrowing, coercion, defaulting, overload relation, generic numeric type, or operand-derived result-type inference.

`FloatDiv` is governed by the numeric-contract authority in [Core floating-point semantics](numerics/floating-point.md). Its explicit `contract` is the already-established selected contract for this operation occurrence and is consumed exactly as semantic input by that owner. This storage relation does not redefine the contract's baseline, refinement, subnormal, NaN, signed-zero, infinity, rounding, or other numerical rules. It grants no division reassociation or reciprocal-replacement permission. Independently authored Core may represent any accepted `Standard`, `Reproducible`, or `Fast` FloatDiv whether or not current source syntax can select all three.

`FloatDiv` reuses the non-replacing destination lifecycle of `Init` and the represented integer-result, `FloatAdd`, `FloatSub`, and `FloatMul` operations rather than defining another storage model. Before either operand is evaluated:
...
3. evaluate `right` completely under its existing `Operand` semantics with required Core type identity `D` in the resulting state;
4. preserve all state consequences of that operand evaluation;
5. consume the two produced semantic binary-floating operand values and select exactly one result permitted by the accepted same-format floating-division relation in `numerics/floating-point.md` for scalar kind `D` under this operation's explicit `contract`;
6. write that one semantic result into `dst`; and
7. mark the written scalar destination Live, beginning a new stored-value lifetime there.

The left operand is therefore evaluated before the right operand. `Move`, `Copy`, raw-pointer operand access, and all other operand-local effects remain exactly those of the existing operand relation; `FloatDiv` does not duplicate or weaken them. A place moved by the left operand is already Dead when the right operand is evaluated. A right operand cannot read, copy, or move such a place unless an independent represented operation has legally restored its value beforehand.

The floating owner remains the sole numerical authority for finite quotient selection and binary rounding, signed-zero results, finite/infinity boundaries, signed infinities, NaN-class outcomes including signed-zero divided by signed-zero and infinity divided by infinity, and any selected-contract-specific permitted result variation. Division by signed zero is therefore a numerical result path under that owner: for the accepted nonzero-finite or infinite numerators it selects the corresponding signed infinity result, while signed-zero divided by signed-zero selects the result NaN class. None of those cases is a Core `Fault`, panic, undefined behavior, physical exception/status observation, or alternate control-flow outcome.

When the floating owner requires only that a result belong to `D`'s NaN value class, `FloatDiv` may store any semantic NaN member permitted by that relation. This storage relation does not select a canonical member, payload, sign, quiet/signaling identity, physical encoding, equality identity, or another member-sensitive property. A semantic NaN value produced at runtime is an ordinary storable value of `D`; its existence does not add a NaN form to the current constant-value representation.

Because `dst` was wholly vacant at admission, the result write has an empty destruction domain and performs no replacement destruction. The write has the same stored-value-lifetime and storage-instance consequences as successful non-replacing `Init` and the existing represented result operations: it begins the destination stored-value lifetime without changing the destination storage extent or storage-instance identity.

After both operand values have been produced, the floating-division/result-write step is finite, non-faulting, and non-diverging under the represented Core semantics. Operation admission, operand sequencing, and the one result-write occurrence remain deterministic; the selected numerical relation may admit more than one result only where `numerics/floating-point.md` explicitly permits that variation. Normal, subnormal, signed-zero, signed-infinity, and NaN-class outcomes selected by the floating owner are ordinary numerical results. The operation introduces no new borrow interval, reference value, pointer provenance, cleanup category, storage identity, assignment-mutability rule, interior-mutability rule, fault reason, control-flow edge, floating exception/status state, layout/ABI promise, runtime numeric mode, or backend-visible semantic fact.

The operation consumes only its explicit selected-contract same-format floating-division result relation and existing numeric-contract authority from `numerics/floating-point.md`. It does not represent integer division, floating remainder, floating negation, reciprocal approximation or replacement, division reassociation, fused divide, comparison, conversion, source selector syntax or scoping, a NaN literal, a generic floating operation family, constant folding, a physical floating representation, or an ambient numeric environment. A realization may use a different physical mechanism only when it independently preserves the selected Runen-observable FloatDiv semantics.
