...
`Init` and represented `IntegerAdd`/`IntegerSub`/`IntegerMul`/`IntegerXor`/`IntegerOr` result initialization may begin either the first stored-value lifetime or a later stored-value lifetime in vacant storage. Ordinary `Assign` and `InteriorAssign` may end old stored-value lifetimes and begin replacement lifetimes in the same storage extent and the same dynamic storage instance.
...
The interior-mutability marker is static semantic type metadata. `InteriorAssign` introduces no hidden runtime borrow state and no new path-state component beyond the storage transitions it already performs. `IntegerAdd`, `IntegerSub`, `IntegerMul`, `IntegerXor`, and `IntegerOr` likewise introduce no hidden storage-state component beyond their two operand consequences and one non-replacing result initialization.
