...
## Multi-operation fast-permission composition

For the finite multiply-add contraction and reassociation permissions defined below, an **affected operation occurrence** is a represented floating operation occurrence whose operand grouping, required intermediate result, or consumed-result relationship would be changed or bypassed by the proposed result-changing transformation.

A result-changing permission granted by those sections under `fast` applies across multiple affected operation occurrences only when every affected occurrence has selected `fast`. An affected occurrence under `standard` or `reproducible` is a semantic boundary; selecting `fast` for another occurrence does not weaken that boundary.

This rule does not prohibit a physical transformation across any contract combination when the realization independently proves that every affected operation still has behavior permitted by its own selected contract. Such a transformation uses ordinary realization freedom, not the result-changing `fast` latitude defined below.

An operation whose operand grouping, required intermediate result, and consumed-result relationship are all unchanged is not affected merely because it occurs in the same larger computation.
...
When both the participating multiplication operation and participating addition operation have selected `fast`, a realization MAY instead contract an eligible occurrence. Contracted evaluation computes the exact real quantity `(x * y) + z` as if with unbounded intermediate range and precision. No Runen rounding, overflow-result selection, underflow-result selection, or finite-format truncation occurs between multiplication and addition. The one exact result is then explicitly supplied as a nonzero exact finite real quantity to the binary floating rounding relation below for the established addition result type exactly once.

Therefore an eligible `fast`/`fast` contraction occurrence explicitly permits either the ordinary uncontracted result or the contracted one-round result. Choosing whether to contract is permitted numerical variation under the selected `fast` contracts; backend behavior is not additional semantic input.

If either participating operation has selected `standard` or `reproducible`, this result-changing contraction permission does not apply. A realization may nevertheless physically fuse such an operation pair when it independently proves that the resulting Runen-observable behavior remains permitted by both participating selected contracts.
...
The multiplication result value may occupy either operand position of the addition. This permission does not reorder the addition operands and does not authorize reassociation, operand permutation, omission, duplication, reciprocal replacement, reduced precision, approximation, or changes to the evaluation or effects that establish `x`, `y`, or `z`. The multiplication evaluation may still have other semantic consumers; contracting this use does not by itself remove or alter obligations for those other consumers. An independently required ordinary multiplication result remains governed by the multiplication operation's own selected contract. This rule does not manufacture a contraction opportunity by regrouping a mixed arithmetic expression. Any other transformation composed with contraction requires independent Runen authority.

`standard` and `reproducible` gain no result-changing contraction permission from this section. Backend fused instructions, LLVM-style contraction flags, WGSL fusion latitude, or another physical realization mechanism do not widen the eligible set and are not semantic input.
...
For ordinary uncontracted evaluation, these choices remain occurrence-local. A `fast` multiplication can flush only the operand occurrences it consumes; a neighboring `standard` or `reproducible` operation gains no input-flushing latitude merely from that multiplication's selected contract, and the converse is likewise true for a `fast` addition.

For a use considered for finite multiply-add contraction under the `fast`/`fast` rule above, the input-flush choices affecting multiplication operands `x` and `y` are choices of the participating `fast` multiplication occurrence. The input-flush choice affecting the other addition operand `z` is a choice of the participating `fast` addition occurrence. Those choices are made before finite contraction eligibility is tested, and the contraction rule consumes the resulting post-substitution `x`, `y`, and `z` values. Because the accepted contraction rule requires all three values to be nonzero finite, replacing any of them by signed zero makes that contraction choice ineligible. Either operation may instead leave its own subnormal operand occurrence unflushed when every other contraction condition holds.

If the multiplication evaluation has other semantic consumers, its `x`/`y` input-flush choices are the same choices that govern its independently required ordinary multiplication result. Contracting one addition use does not authorize that contracted use to recover pre-flush `x` or `y` values that the multiplication evaluation did not consume.

For an ordinary basic operation after its own input substitution, the existing arithmetic/result rules determine its result first; the separate `fast` subnormal result-flushing rule below may then apply when that result is nonzero subnormal. For a permitted contracted use, no separately rounded multiplication result is formed for that use, so there is no multiplication-result flush and no addition-input flush of such an unformed intermediate. The contraction rule determines one final addition result, and result flushing may then apply to that final result when subnormal.
...
This relaxation applies independently to each basic-operation result occurrence. In an uncontracted multiply-add sequence, the multiplication result and the later addition result are separate operation results; each result may be flushed only when that operation occurrence has selected `fast` and the result is subnormal. For a use contracted under the permitted `fast`/`fast` rule, that contracted use has no separately rounded intermediate multiplication result, so this rule can flush only the one final contracted addition result for that use. Any independently required ordinary multiplication result, including a result consumed elsewhere, remains governed by the multiplication operation's own selected contract and may separately be flushed only when that contract is `fast`.
...
Under `fast`, result-changing reassociation is permitted only within a pure finite tree region whose affected internal operation occurrences have all selected `fast` and are all addition, or whose affected internal operation occurrences have all selected `fast` and are all multiplication. Reassociation may change only the grouping of those affected operations: the ordered leaf-value sequence and operation kind MUST remain unchanged.

An affected `standard` or `reproducible` internal operation is a reassociation boundary. A `fast` subregion inside a larger mixed-contract tree MAY be reassociated only when the transformation leaves every enclosing or neighboring non-`fast` operation's operand grouping, required intermediate result, and consumed-result relationship unchanged. Conversely, a physical restructure that crosses a non-`fast` boundary remains legal when it independently proves that every affected operation continues to exhibit behavior permitted by its own selected contract; such a restructure does not rely on this result-changing `fast` permission.
