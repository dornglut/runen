...
- an already-permitted ordinary state change to a Buffer atomic location that is semantically ordered before an admitted exchange supplies the exchange's logical read pre-state and therefore its prior value;
- an accepted exchange that is semantically ordered before a later permitted ordinary read of the same Buffer atomic location supplies that read's logical pre-state and therefore its observed value;
...
- ordered mixed ordinary/atomic Buffer coherence in both represented directions: ordinary state change before exchange supplies the exchange prior value, and exchange before ordinary read supplies the read value, while mixed conflict classification remains unchanged;
