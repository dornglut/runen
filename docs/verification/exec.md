...
- a foreign fixture-local atomic location is rejected before Buffer state is changed;
...
- exact Buffer atomic element-location identity plus transactional composition of the generic atomic-exchange realization with the one `LogicalBufferState`, including singleton-region overlap and rejection without state mutation;
