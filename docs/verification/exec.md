...
- a group-scoped exchange is admitted only from a validated `HierarchyFixture` containing its producer iteration, and its selected `GroupId` is derived from that producer's validated membership rather than accepted as an independent label;
- an iteration absent from the supplied validated hierarchy, including a foreign-`each` iteration with an equal private token, cannot produce admitted group-scope evidence;
- distinct sibling producers in the same established group select compatible group-cohort scope and may synchronize when the other direct release/acquire requirements hold;
- producers in distinct groups of one hierarchy select incompatible group-cohort scopes;
- equal private group tokens under distinct hierarchy identities remain distinct semantic groups and therefore incompatible group scopes;
- unscoped, root-cohort-scoped, and group-cohort-scoped exchanges represented on one atomic location participate in the same location-local modification order rather than creating scope-specific atomic locations or order partitions;
...
- a mixed pair drawn from different represented scope forms is reported as an open scope interaction rather than a defined incompatible pair; this includes unscoped/root, unscoped/group, and root/group pairs;
- root- or group-cohort scope identity does not infer sibling `each` order or legalize an otherwise-conflicting ordinary sibling access;
...
- exchange identities from different atomic locations do not synchronize through this relation even when their represented cohort scope identity is compatible;
...
These obligations do not define subgroup atomic or fence scope, collectives beyond the unordered reduction covered separately below, group-local storage, broadcast, shuffle, scans, or another hierarchy-sensitive operation. Root- and group-cohort atomic exchange scope are covered separately above; group scope consumes an already-validated hierarchy membership but does not define hierarchy establishment itself. Other hierarchy-sensitive operations require their own normative contracts before executable evidence is extended to cover them.
...
These obligations do not define source barrier syntax, dynamic divergent-barrier validation, atomic order semantics beyond direct release/acquire, subgroup atomic scope or fence semantics, additional collectives, group-local storage, or physical barrier implementation. Root- and group-cohort atomic exchange scope are covered separately above.
