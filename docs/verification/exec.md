...
- a named immediate-predecessor query exposes only that relation and does not enumerate the accepted modification order;
...
- validated atomic-exchange occurrence identity, exchange semantics classification, exact candidate modification-order coverage, location-local semantic-order constraints, prior-value observation, immediate-predecessor identity, direct release/acquire synchronization, and final-value computation;
