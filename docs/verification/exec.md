...
The normative ordinary-access and structured-iteration rules exercised here are owned by `spec/language/exec/memory-model.md` and `spec/language/exec/parallelism.md`. Structured task lifetime, detachment, dynamic task-scope identity, explicit normal task join, and cooperative cancellation observation are owned by `spec/language/exec/tasks.md`. Physical execution-agent identity and realization placement preservation are owned by `spec/language/exec/realization.md`. Buffer-specific identity, region, view-access, logical-coherence, atomic element-location/state, and address-free typed-mapping facts are owned by `spec/language/exec/resources/buffers.md`; physical allocation identity, allocation extent, and execution-agent accessibility are owned by `spec/language/exec/resources/allocations.md`. Core storage, overlap, borrowing, interior-mutability, and function-termination cleanup facts remain owned by their Core specifications.
...
- a group-scoped exchange and a subgroup-scoped exchange are scope-compatible when the subgroup's containing `GroupId` is exactly the group selected by the group-scoped exchange and the group-scoped producer's validated `SubgroupId` is exactly the subgroup selected by the subgroup-scoped exchange;
- that mixed group/subgroup relation is symmetric, so either release→acquire orientation may synchronize when the other direct release/acquire requirements hold;
- a same-group group/subgroup pair is scope-incompatible when the group-scoped producer belongs to another subgroup;
- a group/subgroup pair with distinct semantic group identities is scope-incompatible, including equal private group/subgroup tokens under distinct hierarchy identities;
...
- a root-scoped exchange and a group-scoped exchange are scope-compatible exactly when the exact hierarchy already bound by the group scope proves that the root producer belongs to that selected semantic group; the relation is symmetric and may synchronize in either release→acquire orientation when the other direct-relation requirements hold;
- a root-scoped exchange and a subgroup-scoped exchange are scope-compatible exactly when the exact hierarchy already bound by the subgroup scope proves that the root producer belongs to that selected semantic subgroup; the relation is symmetric and may synchronize in either release→acquire orientation when the other direct-relation requirements hold;
- a valid root/group or root/subgroup pair is scope-incompatible when the exact narrower hierarchy proves that the root producer belongs outside the selected narrower cohort, including producers belonging to distinct dynamic `each` identities;
- a distinct same-`each` hierarchy with equal private cohort tokens cannot retarget the narrower scope; unrepresented identities or insufficient exact hierarchy evidence, including missing root membership, produce no verification relation rather than being reclassified as semantic incompatibility or as a normatively open scope interaction;
- a mixed pair involving unscoped/root, unscoped/group, or unscoped/subgroup remains reported as an open scope interaction, and the focused synchronization predicate supplies no current edge for such an open pair without proving future incompatibility;
...
`AtomicLocationId`, location-scoped `AtomicExchangeId`, `AtomicValueToken`, `AtomicExchangeSemantics`, `AtomicGroupScope`, `AtomicSubgroupScope`, `AtomicExchangeScope`, `AtomicScopeRelation`, `AtomicExchange`, `AtomicExchangeFixture`, and `AtomicExchangeRealization` are verification representation only. `Base`, `Release`, `Acquire`, and `AcquireRelease` are verification classifications of the normative semantic classes rather than a frozen source memory-order enumeration. `Unscoped`, `Root(IterationId)`, `Group(AtomicGroupScope)`, and `Subgroup(AtomicSubgroupScope)` are verification classifications of the currently represented synchronization-scope forms. Root scope derives root-cohort identity from the scoped exchange's verification-only producer iteration and remains hierarchy-independent. `AtomicGroupScope` and `AtomicSubgroupScope` are constructed only from a represented exact established hierarchy fixture and retain that producer's validated full `HierarchyMembership`; neither can publicly pair a producer with an unrelated free cohort label. That retained membership is focused verification evidence for the exact group/subgroup and root-to-narrower relations, not a source hierarchy handle, hierarchy-establishment mechanism, cross-hierarchy participant set, or reusable participant-domain abstraction. The focused hierarchy-evidence query consumes the narrower scope's exact established hierarchy only to classify the represented root/group or root/subgroup pair; it does not bind root scope to a hierarchy, create an implicit current hierarchy, or generalize to a participant-domain relation. These forms are not a frozen source memory-scope enumeration or backend scope lattice. `Compatible`, `Incompatible`, and `Open` are verification evidence for the currently defined-or-open pairwise scope relationship, not source values or a general scope lattice. Candidate modification-order slices are supplied as realization evidence and are not exposed by an accepted realization as semantic scheduler order. Verification-only `(before, after)` constraint pairs stand for order facts already owned elsewhere; they do not define a generic Runen execution graph. Exact immediate-predecessor and per-exchange scope metadata remain private after realization; the accepted realization exposes only the focused scope-relation and synchronization predicates required by this verification boundary.

These obligations do not define atomic load/store, compare-exchange, fetch operations, release sequences, sequentially-consistent semantics, fences, broader atomic scope, the remaining mixed interoperability between unscoped exchange and root/group/subgroup cohort-scoped forms, cross-hierarchy participant inclusion, source/lowering syntax for identifying the current iteration or establishing/selecting a hierarchy, Buffer-specific mixed ordinary/atomic interaction beyond the separately covered conflict and ordered-coherence boundaries below, source atomic syntax/types/order/scope enums, storage layout, addresses, progress guarantees, or backend atomic instructions.
...
- an already-permitted ordinary state change to a Buffer atomic location that is semantically ordered before an admitted exchange is included in the exchange's logical read pre-state; in the focused case with no intervening state-changing access, the exchange therefore returns that ordinary changed value as its prior value;
- an accepted exchange that is semantically ordered before a later permitted ordinary read of the same Buffer atomic location is included in that read's logical pre-state; in the focused case with no intervening state-changing access, the read therefore observes the exchange's final value;
- those ordered mixed pairs remain conflicts under the mixed conflict predicate; semantic ordering does not erase conflict classification or insert the ordinary access into atomic modification order;
- fixture call order for the two focused mixed cases stands only for semantic order already supplied by an applicable contract and is not host-call-order authority for Runen order;
...
These cases compose the Buffer-owned logical element identity/state contract in `spec/language/exec/resources/buffers.md` with the generic atomic-exchange contract in `spec/language/exec/memory-model.md`. They exercise only an atomic exchange that an applicable contract has already admitted. They do not define atomic access authority or source atomic capability; mixed ordinary/atomic conflict and source-unordered legality are owned separately by the memory-model boundary below, while ordered mixed logical pre-state consequences are consumed from the Buffer coherence boundary above.

Required cases:

- one `BufferAtomicLocation` is identified exactly by one `BufferId` and one `PositionId`;
- equal Buffer and position identities denote the same Buffer atomic location, while another position or another Buffer denotes a distinct location;
- one `BufferAtomicExchangeId` is structurally scoped by its exact `BufferAtomicLocation`, so equal private occurrence tokens under the same location denote the same represented occurrence identity while the same token under another Buffer atomic location remains distinct;
- one bridge fixture constructs `BufferAtomicExchange` evidence only for its exact Buffer atomic location and rejects an exchange, candidate-order identity, or order constraint belonging to another Buffer atomic location before Buffer state is changed;
- the location's logical footprint is the singleton `BufferRegion` containing that exact position, and its overlap relation is the existing Buffer-region relation rather than address or allocation overlap;
- the bridge initializes the existing generic `AtomicExchangeFixture` from the current value of that exact position in the one `LogicalBufferState`;
- one admitted exchange returns the current logical Buffer value as its prior value and commits the accepted atomic final value back to that same logical position;
- several admitted exchanges reuse the generic location-local candidate modification-order semantics for prior/final values, and only the accepted final location value is committed back to the one Buffer state;
- a successful atomic realization does not change a disjoint Buffer position;
...
- the generic `AtomicLocationId` used by the composed atomic oracle is synthesized privately only after every Buffer-side exchange/order identity is validated against the exact `BufferAtomicLocation`; callers cannot supply or observe that internal identity, and it is not an alternative resource identity.
...
These cases exercise the mixed conflict and source-unordered legality relation owned by `spec/language/exec/memory-model.md` using the singleton logical footprint of a Buffer atomic location owned by `spec/language/exec/resources/buffers.md`. This section is verification evidence only; it does not redefine either owner.

Required cases:

- an ordinary non-atomic read whose Buffer region overlaps the exact singleton region of an admitted Buffer atomic location conflicts with the atomic exchange under the mixed relation;
- an ordinary non-atomic state-changing access whose Buffer region overlaps that exact atomic-location region also conflicts;
- an ordinary read or state-changing access to a disjoint logical position of the same Buffer does not conflict with that atomic location under the mixed relation;
- an equal private `PositionId` under another `BufferId` remains disjoint from the atomic location because Buffer identity participates in the resource-owned overlap relation;
- mixed conflict classification consumes only the ordinary Buffer region and the resource-owned atomic-location footprint; atomic exchange class, synchronization scope, modification-order position, mapping identity, allocation identity, execution-agent identity, and physical schedule are not conflict inputs;
- the mixed relation does not change the existing ordinary/ordinary conflict predicate;
- a source-unordered overlapping mixed pair remains conflicting even when host fixture calls are serialized; host call order is not semantic permission or synchronization;
- the ordinary access does not enter the atomic location's modification order and does not become atomic through conflict classification.
...
These cases exercise the physical execution-agent identity owned by `spec/language/exec/realization.md`, the allocation identity/extent/accessibility relation owned by `spec/language/exec/resources/allocations.md`, and the typed physical-accessibility relation owned by `spec/language/exec/resources/buffers.md`. Mapping does not become logical access permission, synchronization, environment admission, or an address/representation model.
...
- a mapping construction naming an execution agent not admitted by that allocation is rejected before a mapping occurrence is registered or its occurrence token is consumed;
- after such an accessibility rejection, the same unused occurrence token may still form a legal mapping for an admitted agent;
...
- duplicate or previously used mapping occurrence identity within one allocation fixture is rejected;
- a mapping's selected execution agent is fixed for that occurrence and is not silently replaced by another agent;
...
## Task verification boundary

Structured task lifetime and detachment, explicit normal task join, and cooperative cancellation assurance obligations are owned by the [Exec task verification contract](exec-tasks.md).

## Executable oracle coverage

The current `runen-exec-oracle` executable subset covers only relations already defined above and in the linked task verification contract:
...
- validated atomic-exchange occurrence identity, exchange semantics and producer-derived root/group/subgroup-scope classification, exact candidate modification-order coverage across scope forms, location-local semantic-order constraints, prior-value observation, private immediate-predecessor/exact-scope evidence, same-form plus exact-hierarchy group/subgroup and root-to-hierarchical compatible/incompatible relations, deliberately open unscoped-to-structured scope evidence, direct scope-compatible release/acquire synchronization, and final-value computation;
